...
mCompleter::mCompleter(QLineEdit *parent,
                       char _delimiter /*,mSettings::defaultFont defaultfont*/)
    : QCompleter(parent),
      delimiter(_delimiter),
      line_edit(qobject_cast<QLineEdit *>(parent))  //,defaultfont(defaultfont)
{
...
mCompleter::mCompleter(
    QLineEdit *parent, char _delimiter,
    /*mSettings::defaultFont defaultfont,*/ QStringListModel *model)
...
mCompleter::mCompleter(
    QLineEdit *parent, char _delimiter,
    /* mSettings::defaultFont defaultfont,*/ const QStringList &list)
...
QString mCompleter::pathFromIndex(const QModelIndex &index) const {
  ...
  if (cursor_pos != originalPos) line_edit->setCursorPosition(cursor_pos);
  //   else
   ...
   * NOTE
   *
   * prev_delimiter_index should actually point at final white space AFTER the
   * delimiter.
   */
  ...
  // Get index of first delimiter after current position (or EOL if no delimiter
  // after cursor)
  ...
  // QString pre = str.mid(prev_delimiter_index + 1, cur_index -
...
QStringList mCompleter::splitPath(const QString &path) const {
  ...
  int index =
      path.mid(0, line_edit->cursorPosition()).lastIndexOf(delimiter) + 1;
  QString str = path.mid(index, line_edit->cursorPosition() - index).trimmed();
  string_list << str;
  return string_list;
}

void mCompleter::putModel(QStringListModel *model) { setModel(model); }

void mCompleter::connectSignals() {
  connect(this, SIGNAL(activated(const QString &)), this,
          SLOT(onActivated(const QString &)));

  connect(line_edit, SIGNAL(cursorPositionChanged(int, int)), this,
          SLOT(onCursorPositionChanged(int, int)));

  connect(line_edit, SIGNAL(removeFromCompleter(int)), this,
          SLOT(removeItemFromModel(int)));
}

void mCompleter::onActivated(const QString &) { cursor_pos = originalPos; }

void mCompleter::onCursorPositionChanged(int old_pos, int new_pos) {
  if (old_pos != cursor_pos) cursor_pos = new_pos;
}
