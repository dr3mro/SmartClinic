...
PatientList::PatientList(QWidget *parent)
    : mDialog(parent), ui(new Ui::PatientList) {
  ui->setupUi(this);
  ...
  connect(ui->searchWidgetx, SIGNAL(loadPatient(int)), parent->window(),
...
void PatientList::keyPressEvent(QKeyEvent *e) {
  ...
  if (e->key() == Qt::Key_F2) ui->searchWidgetx->focusFilter();
  QWidget::keyPressEvent(e);
}

PatientList::~PatientList() { delete ui; }

void PatientList::on_closeButton_clicked() { this->close(); }
