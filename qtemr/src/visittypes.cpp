...
VisitTypes::VisitTypes() {
  ...
  ui_VisitTypes << _NewVisit << _Requests << _Follow1 << _Follow2 << _Follow3
                << _Follow4 << _Free << _Undefined;
  algo_VisitTypes << _NewVisit << _Follow1 << _Follow2 << _Follow3 << _Follow4
                  << _Free << _Requests << _Undefined;
}
...
VisitTypes::n_visitsType VisitTypes::advance(int _vt) const {
  ...
  bool autoSetNewAfterMaxIsReached =
      settings.autoSetNewAfterMaxFollowUpsPerProblem();
  ...
  switch (_vt) {
    ...
    case Follow4:
      ...
      if (_vt == maxFollowUpsPerProblem) {
        return autoSetNewAfterMaxIsReached
                   ? NewVisit
                   : (n_visitsType)maxFollowUpsPerProblem;
      } else {
...
VisitTypes::t_visitsType &VisitTypes::getVisitTypesByUiIndex(
    const int &index) const {
  return const_cast<VisitTypes::t_visitsType &>(ui_VisitTypes.at(index));
}

VisitTypes::t_visitsType &VisitTypes::getVisitTypesByAlgoIndex(
    const int &index) const {
...
QString VisitTypes::getVisitSymbole(const int &_i, const bool &isCaseClosed,
                                    const bool &noNextDateSpecified) const {
  n_visitsType vt;
