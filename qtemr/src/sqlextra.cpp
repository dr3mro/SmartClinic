...
sqlExtra::sqlExtra(QObject *parent, QString connectionName, bool cTable)
  ...
  if (!doesCustomDrugsTableExists()) {
    QString coreconName =
        QString("qt_sql_core_coreDrugList_%1")
...
sqlExtra::~sqlExtra() { delete stdDosesModel; }

bool sqlExtra::createTables() {
  QStringList tables = QStringList() << "placeOfBirth"
                                     << "allergies"
                                     << "diagnoses"
                                     << "dictionary"
                                     << "doses"
  ...
  a = executeQuery(
      "CREATE TABLE IF NOT EXISTS defaultDoses ( tradeName varchar UNIQUE, "
      "dose varchar )");
  if (!a) {
  ...
  foreach (QString table, tables) {
    b = executeQuery(
        QString("CREATE TABLE IF NOT EXISTS %1 ( STRING varchar UNIQUE )")
            .arg(table));
    if (!b) {
      break;
    }
  ...
  c = executeQuery(
      "CREATE TABLE IF NOT EXISTS diet ( diet varchar UNIQUE, description "
      "varchar )");
  if (!c) {
...
bool sqlExtra::addToAutoComplete(QString table, QString phrase,
                                 bool bypassCheck) {
  if (!bypassCheck && phrase.length() > 2) {
    if (!getAutoCompleteList(table).contains(phrase, Qt::CaseInsensitive))
      return executeQuery(QString("INSERT INTO '%1' (STRING) VALUES(\"%2\")")
                              .arg(table)
                              .arg(phrase.simplified()));
  }
  if (bypassCheck) {
    return executeQuery(QString("INSERT INTO '%1' (STRING) VALUES(\"%2\")")
                            .arg(table)
...
bool sqlExtra::addDefaultDrugDose(QString tradeName, QString dose) {
  bool q = executeQuery(
      QString(
          "INSERT INTO defaultDoses (tradeName, dose) VALUES (\"%1\",\"%2\")")
          .arg(tradeName)
          .arg(dose));
  if (!q) {
...
bool sqlExtra::deleteDefaultDrugDose(QString tradeName) {
  bool q =
      executeQuery(QString("DELETE FROM defaultDoses WHERE tradeName=\"%1\"")
                       .arg(tradeName));
  if (!q) {
...
QString sqlExtra::getDefaultDrugDose(QString tradeName) {
  ...
  query->clear();
  bool q = query->exec(
      QString("SELECT dose FROM defaultDoses WHERE tradeName = \"%1\"")
          .arg(tradeName));
  if (!q) {
...
QStringList sqlExtra::getFavourites() {
  ...
  if (!p) {
    mDebug() << "ERROR WHILE CREATING AUTOCOMPLETE LIST FVOURITES: "
...
bool sqlExtra::removeFromAutoComplete(QString table, QString phrase) {
  QString query;
  if (table == "favourites")
    query = QString("DELETE FROM defaultDoses WHERE TradeName = \"%1\";")
                .arg(phrase);
  else
    query = QString("DELETE FROM '%1' WHERE STRING = \"%2\";")
                .arg(table)
                .arg(phrase);
...
QStringList sqlExtra::getAutoCompleteList(QString table) {
  ...
  if (!p) {
    mDebug() << "ERROR WHILE CREATING AUTOCOMPLETE LIST : "
             << query->lastError().text();
    mDebug() << query->lastQuery();
  }
  QStringList sList;
...
bool sqlExtra::createServicesTable() {
  query->clear();
  bool x = query->exec(
      "CREATE TABLE IF NOT EXISTS SERVICES (service varchar UNIQUE, price "
      "integer)");
  if (!x) {
    mDebug() << "Failed to create services table : ?"
             << query->lastError().text();
  }
...
bool sqlExtra::addService(QString service) {
  query->clear();
  bool x = query->exec(
      QString("INSERT INTO SERVICES (service) VALUES (\"%1\")").arg(service));
  if (!x) {
    mDebug() << "Failed to create services table : ?"
             << query->lastError().text();
  }
...
bool sqlExtra::deleteSrvice(QString service) {
  query->clear();
  bool x = query->exec(
      QString("DELETE FROM SERVICES WHERE service=\"%1\"").arg(service));
  if (!x) {
    mDebug() << "Failed to delete service from services table : ?"
             << query->lastError().text();
  }
...
void sqlExtra::updateServiceModel() { tableModel->select(); }

QSqlTableModel *sqlExtra::readServicesModel() {
  tableModel = new serviceModel(this, db);
  connect(tableModel, SIGNAL(dataChanged(QModelIndex, QModelIndex)), this,
          SLOT(tableModelUpdateValidator(QModelIndex, QModelIndex)));
...
// QSqlTableModel *sqlExtra::getSqlTable(QString table, QString col1, QString
// col2)
//{
...
QStringList sqlExtra::getPaidServicesList() {
  ...
  if (!x) {
    mDebug() << "Failed to GET LIST OF service from services table : ?"
             << query->lastError().text();
  }
  while (query->next()) {
...
double sqlExtra::getPaidServicePrice(QString service) {
  ...
  query->clear();
  bool x = query->exec(
      QString("SELECT price FROM SERVICES WHERE service=\"%1\"").arg(service));
  if (!x) {
    mDebug() << "Failed to GET  service Price from services table : ?"
             << query->lastError().text();
  }
  while (query->next()) {
...
bool sqlExtra::createExpanderTable() {
  bool c = executeQuery(
      "CREATE TABLE IF NOT EXISTS expander ( name varchar UNIQUE, string "
      "varchar )");
  if (!c) {
...
bool sqlExtra::addNewExpand(QString name) {
  if (getAutoCompleteList("drugs").contains(name)) return false;

  bool b = executeQuery(
      QString("INSERT INTO expander (name) VALUES (\"%1\")").arg(name));
  if (!b) {
...
bool sqlExtra::saveExpand(QString name, QString expand) {
  query->clear();
  query->prepare(QString("UPDATE expander SET string=:string where name=\"%1\"")
                     .arg(name));
...
bool sqlExtra::deleteExapnd(QString name) {
  bool c =
      executeQuery(QString("DELETE FROM expander Where name=\"%1\"").arg(name));
  if (c)
...
QStringList sqlExtra::getExpandList() {
  ...
  if (!p) {
    mDebug() << "ERROR WHILE CREATING EXAPNDER LIST : "
             << query->lastError().text();
  }
  QStringList sList;
  while (query->next()) {
...
QString sqlExtra::getExpand(QString name) {
  ...
  query->clear();
  bool x = query->exec(
      QString("SELECT string FROM expander WHERE name=\"%1\"").arg(name));
  if (!x) {
    mDebug() << "Failed to GET expander from expander table : ?"
             << query->lastError().text();
  }
  while (query->next()) {
...
bool sqlExtra::isExpandExists(QString name) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM expander where name=\"%1\")")
          .arg(name);
  query->clear();
  QString value;
...
bool sqlExtra::isDefaultDoseExists(QString name) {
  QString sqlCmd =
      QString(
          "SELECT EXISTS(SELECT * FROM defaultDoses where tradeName=\"%1\")")
          .arg(name);
  query->clear();
  QString value;
  if (!query->exec(sqlCmd) || !query->first()) {
...
QString sqlExtra::getDiet(QString diet) {
  query->clear();
  bool p = query->exec(
      QString("SELECT description FROM diet Where diet=\"%1\"").arg(diet));
  if (!p) {
    mDebug() << "ERROR WHILE GETTING DIET DESCRIPTION: "
             << query->lastError().text();
    mDebug() << query->lastQuery();
  }
...
//{
//     executeQuery("DELETE FROM defaultDoses WHERE tradeName IS NULL OR
//     trim(tradeName) = '';"); executeQuery("DELETE FROM defaultDoses WHERE
//     dose IS NULL OR trim(dose) = '';");
// }
