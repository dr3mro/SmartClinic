...
mPatientFilter::mPatientFilter(QWidget *parent)
{
    ...
    QAction *filterToggleAction =
        this->addAction(QIcon(":/ionicons/more"), QLineEdit::ActionPosition::TrailingPosition);

    filterToggleAction->setCheckable(true);

    connect(filterToggleAction,SIGNAL(toggled(bool)),parent, SLOT(showHideFilters(bool)));
    connect(this,SIGNAL(toggleFilters(bool)),filterToggleAction, SLOT(setChecked(bool)));
