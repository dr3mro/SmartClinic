...
vitalBox::vitalBox(QWidget *parent)
    : QLineEdit(parent), validator(new QRegularExpressionValidator(this)) {
  connect(parent->window(), SIGNAL(setReadWrite(bool)), this,
          SLOT(makeEditable(bool)));
  connect(parent->window(), SIGNAL(styleVitals(mSettings &)), this,
          SLOT(createStyles(mSettings &)));
}

vitalBox::~vitalBox() { delete validator; }
  ...
  switch (vitaltype) {
    case (0):
      connect(this, SIGNAL(textChanged(QString)), this,
              SLOT(pulseRate(QString)));
      setRegExpValidator("^([0-9][0-9]|[1-3][0-9][0-9]?)$");
      break;
    case (1):
      connect(this, SIGNAL(textChanged(QString)), this,
              SLOT(bloodpressure(QString)));
      setRegExpValidator(
          "^([1-9][0-9]|[1-2][0-9][0-9]|300?)\\/"
          "([1-9][0-9]|[1-2][0-9][0-9]?)$");
      break;
    case (2):
      connect(this, SIGNAL(textChanged(QString)), this,
              SLOT(respiratoryRate(QString)));

      setRegExpValidator("^(100|[1-9]?[0-9])$");
      break;
    case (3):
      connect(this, SIGNAL(textChanged(QString)), this,
              SLOT(temperature(QString)));
...
void vitalBox::pulseRate(QString pulserate) {
  if (speciality != dataHelper::Speciality::InternalMedicine) return;
  ...
  if (pulseRate >= 60 && pulseRate <= 100) {
    setStyleSheet(style.normalStylesheet);
    return;
  } else if ((pulseRate < 60 && pulseRate > 45) ||
             (pulseRate > 100 && pulseRate <= 150)) {
...
void vitalBox::bloodpressure(QString bloodpressure) {
  ...
  if (systolic == 0 || diastolic == 0) {
    setStyleSheet(style.normalStylesheet);
    return;
  } else if ((systolic >= 100 && systolic <= 120) ||
...
void vitalBox::makeEditable(bool b) { setReadOnly(!b); }

void vitalBox::mousePressEvent(QMouseEvent *event) {
  if (!this->isReadOnly() && this->text().isEmpty() &&
      event->button() == Qt::MiddleButton && !this->placeholderText().isEmpty())
