...
settingsClass::settingsClass(QWidget *parent)
  ...
  exConName = QString("qt_sql_extra_settings_connection_%1")
                  .arg(QRandomGenerator::global()->bounded(0, 9999999));
  bsConName = QString("qt_sql_base_settings_connection_%1")
                  .arg(QRandomGenerator::global()->bounded(0, 9999999));
  ...
  ui->servicesTable->horizontalHeader()->setSectionResizeMode(
      0, QHeaderView::Stretch);
  ui->servicesTable->setItemDelegate(spinboxdelegate);

  initiateWatcher();

  connect(ui->createStartMenuShortcut, SIGNAL(clicked(bool)), this,
          SLOT(createStartMenuShortcut(bool)));
  connect(ui->createDesktopShortcut, SIGNAL(clicked(bool)), this,
          SLOT(createDesktopShortcut(bool)));
  connect(ui->encryptionSwitch, SIGNAL(clicked(bool)), this,
          SLOT(toggleEncryption(bool)));
  connect(ui->stylerWidget, SIGNAL(applyTheme()), this, SIGNAL(reloadTheme()));
  connect(ui->stylerWidget, SIGNAL(saveSelectedTheme(QString)), &msettings,
          SLOT(setSelectedTheme(QString)));
...
mSettings::pSettings settingsClass::getValuesFromUI() {
  ...
  _settings.autoSetnewAfterMaxPerProblemIsReached =
      ui->checkBoxAutoNewVisit->isChecked();
  ...
  if (ui->monday->isChecked()) _settings.workingdays |= VisitHelper::Monday;
  //    mDebug() <<_settings.workingdays;
  if (ui->tuesday->isChecked()) _settings.workingdays |= VisitHelper::Tuesday;
  //    mDebug() <<_settings.workingdays;
  if (ui->wednesday->isChecked())
    _settings.workingdays |= VisitHelper::Wednesday;
  //    mDebug() <<_settings.workingdays;
  if (ui->thursday->isChecked()) _settings.workingdays |= VisitHelper::Thrusday;
  //    mDebug() <<_settings.workingdays;
  if (ui->friday->isChecked()) _settings.workingdays |= VisitHelper::Friday;
  //    mDebug() <<_settings.workingdays;
  if (ui->saturday->isChecked()) _settings.workingdays |= VisitHelper::Saturday;
  //    mDebug() <<_settings.workingdays;
  if (ui->sunday->isChecked()) _settings.workingdays |= VisitHelper::Sunday;
  //    mDebug() <<_settings.workingdays;
  return _settings;
}
...
void settingsClass::setValuesToUI(const mSettings::pSettings &_settings) {
  ...
  ui->defaultFontSize->setCurrentText(
      QString::number(_settings.defaultFontSize));
  ui->defaultFontBold->setChecked(_settings.defaultFontBold);
  ui->textboxFont->setCurrentText(_settings.textboxFont);
  ui->textboxFontSize->setCurrentText(
      QString::number(_settings.textboxFontSize));
  ...
  ui->checkBoxAutoNewVisit->setChecked(
      _settings.autoSetnewAfterMaxPerProblemIsReached);
...
void settingsClass::closeEvent(QCloseEvent *event) {
  ...
  bool isThemeSelectionChanged =
      (storedSettings.selectedTheme != uiSettings.selectedTheme);
  bool isThemeModified = (ui->stylerWidget->save() || isThemeSelectionChanged);
  isSettingsModified = !(uiSettings == storedSettings);

  if (isThemeModified) emit reloadTheme();

  if (isSettingsModified) msettings.saveSettings(uiSettings);

  if (encryptionToggled) emit reloadMainWindow();
...
void settingsClass::on_setPasswdButton_clicked() { set_clear_pass(); }
...
void settingsClass::on_passwordLine_returnPressed() { set_clear_pass(); }
...
void settingsClass::initiateWatcher() {
  ...
  lnkStartMenuPath = QString("%1\\%2.lnk")
                         .arg(QString(QStandardPaths::standardLocations(
  ...
  connect(&portabilityWatcher, SIGNAL(fileChanged(QString)), this,
          SLOT(portabilityChecker()));
  connect(&linkWatcher, SIGNAL(fileChanged(QString)), this,
          SLOT(shortcutChecker()));
}
...
void settingsClass::portabilityChecker() {
  ...
  ui->portability_status->setText(portability ? "Portabel mode is enabled"
                                              : "Portabel mode is disabled");
...
void settingsClass::on_closeButton_clicked() { close(); }

void settingsClass::on_resetDrugsButton_clicked() {
  int reply = QMessageBox::question(nullptr, tr("Are you sure?"),
                                    tr("Are you sure that you want to reset "
                                       "autocompleter for drugs to default ? "
                                       "This can not be undone."),
                                    QMessageBox::Yes, QMessageBox::No);
  if (reply == QMessageBox::Yes) {
    if (sqlextra->resetDrugsTable()) {
      emit refreshCompleter();
      newMessage(tr("System Message"),
                 tr("Drugs autocompleter was reset successfuly."));
    } else {
      newMessage(tr("System Message"),
                 tr("Drugs autocompleter reset operation was failed."));
    }
  }
  QSqlDatabase::removeDatabase("qt_sql_core_resetDrugs");
}

void settingsClass::on_buttonAddService_clicked() { addNewService(); }

void settingsClass::on_buttonDeleteService_clicked() {
  int row = ui->servicesTable->currentIndex().row();
  QString selectedService =
      ui->servicesTable->currentIndex().sibling(row, 0).data().toString();
  sqlextra->deleteSrvice(selectedService);
  sqlextra->updateServiceModel();
  ui->buttonDeleteService->setDisabled(true);
}

void settingsClass::on_serviceLine_returnPressed() { addNewService(); }
...
void settingsClass::addNewService() {
  ...
  for (int row = 0; row < rows; row++) {
    items << ui->servicesTable->model()
                 ->data(ui->servicesTable->model()->index(row, 0))
                 .toString();
  }
...
void settingsClass::on_comboBoxSpeciality_currentIndexChanged(int index) {
  bool b = (index == 4);  // this will disable common conditions if obgyn is
                          // used
...
void settingsClass::createDesktopShortcut(bool b) { mklnk(lnkDesktopPath, b); }
...
void settingsClass::on_selectDirButton_clicked() {
  int reply = QMessageBox::question(
      nullptr, "Message",
      QString("%1 will exit after you change data path . Are you sure? please "
              "make sure you have no unsaved data.")
          .arg(APPNAME),
      QMessageBox::Yes, QMessageBox::No);
...
void settingsClass::on_cb_portability_clicked(bool checked) {
  ...
  if (operation_status) ui->portability_msg->setText("- restart is needed");

  if (checked) portabilityWatcher.addPath(dataIOhelper::PortabilityLockFile());
