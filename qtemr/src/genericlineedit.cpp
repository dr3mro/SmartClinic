...
genericLineEdit::genericLineEdit(QWidget *parent)
    : mLineEdit(parent), worker(new wm_CompleterLoader) {
  if (windowName != "") {
  ...
  if (windowName == "visitsBox") {
    connect(parent->window(), SIGNAL(onVisitBoxClose()), this, SLOT(clear()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(reportCompleterPopUpVisibility(bool)),
            parent->window(), SLOT(isCompleterPopupVisible(bool)));
    connect(this, SIGNAL(isAutoCompleteByWord()), parent->window(),
            SLOT(isCompleteByWord()));
  }
  if ((windowName == "MainWindow") || (windowName == "visitsBox")) {
    connect(parent->window(), SIGNAL(updateTextFont()), this,
            SLOT(setDefaultFont()));
    connect(this, SIGNAL(removeItemFromCompleter(QString, QString)),
            parent->window(), SLOT(removeItemFromCompleter(QString, QString)));
    connect(parent->window(), SIGNAL(insertUniqueItem(QString, QString)), this,
            SLOT(addUniqueItem2Model(QString, QString)), Qt::QueuedConnection);
  }
...
genericLineEdit::~genericLineEdit() { delete worker; }
...
void genericLineEdit::mUpdateCompleter() {
  ...
  if (!ignoredObjects.contains(objectName())) createCompleter();
}
