...
void PageMetrics::update(QPageSize::PageSizeId _pageFormat,
                         QMarginsF _mmPageMargins) {
  ...
  m_pxPageSize =
      QSizeF(mmToPx(m_mmPageSize.width(), x), mmToPx(m_mmPageSize.height(), y));
  m_pxPageMargins = QMarginsF(
      mmToPx(m_mmPageMargins.left(), x), mmToPx(m_mmPageMargins.top(), y),
      mmToPx(m_mmPageMargins.right(), x), mmToPx(m_mmPageMargins.bottom(), y));
}

QPageSize::PageSizeId PageMetrics::pageFormat() const { return m_pageFormat; }

QSizeF PageMetrics::mmPageSize() const { return m_mmPageSize; }

QMarginsF PageMetrics::mmPageMargins() const { return m_mmPageMargins; }

QSizeF PageMetrics::pxPageSize() const {
  return QSizeF(m_pxPageSize.width(), m_pxPageSize.height());
}

QMarginsF PageMetrics::pxPageMargins() const {
  return QMarginsF(m_pxPageMargins.left(), m_pxPageMargins.top(),
                   m_pxPageMargins.right(), m_pxPageMargins.bottom());
}
