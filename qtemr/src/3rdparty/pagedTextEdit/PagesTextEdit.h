 ...
 */
class PagesTextEdit : public TextEdit {
  Q_OBJECT
 ...
 public slots:
  ...
  /**
   * @brief Установить значение необходимости добавления дополнительной
   * прокрутки снизу
   */
 ...
 protected:
  /**
   * @brief Переопределяется для корректировки документа и прорисовки оформления
   * страниц
   */
 ...
 private:
   ...
   */
  void paintPageNumber(QPainter* _painter, const QRectF& _rect, bool _isHeader,
                       int _number);
 ...
 private slots:
   ...
   * @note Стандартная реализация QTextEdit такова, что она всё время сбрасывает
   *установленный размер документа, что приводит к нежелательным последствиям
   */
