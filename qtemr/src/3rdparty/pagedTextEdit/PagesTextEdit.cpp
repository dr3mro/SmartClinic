...
PagesTextEdit::PagesTextEdit(QWidget* parent)
  ...
  connect(verticalScrollBar(), SIGNAL(rangeChanged(int, int)), this,
          SLOT(aboutVerticalScrollRangeChanged(int, int)));
...
bool PagesTextEdit::usePageMode() const { return m_usePageMode; }
...
void PagesTextEdit::updateViewportMargins() {
  ...
  if (m_usePageMode) {
    ...
    const int DEFAULT_BOTTOM_MARGIN = 20;
    {
      ...
      //
      // Если ширина редактора больше ширины страницы документа, расширим
      // боковые отступы
      //
      if (width() > pageWidth) {
        ...
        leftMargin = rightMargin =
            (width() - pageWidth - VERTICAL_SCROLLBAR_WIDTH - BORDERS_WIDTH) /
            2;
      }
      ...
      if ((height() - documentHeight) >
        ...
        const int HORIZONTAL_SCROLLBAR_HEIGHT =
            horizontalScrollBar()->isVisible() ? horizontalScrollBar()->height()
                                               : 0;
        bottomMargin = height() - documentHeight - HORIZONTAL_SCROLLBAR_HEIGHT -
                       DEFAULT_TOP_MARGIN - BORDERS_HEIGHT;
      }
      ...
      //
      viewportMargins =
          QMargins(leftMargin, topMargin, rightMargin, bottomMargin);
    }
  }
...
void PagesTextEdit::updateVerticalScrollRange() {
  ...
  //
  // В обычном режиме просто добавляем немного дополнительной прокрутки для
  // удобства
  //
  else {
    const int SCROLL_DELTA = 800;
    int maximumValue = document()->size().height() -
...
void PagesTextEdit::paintPagesView() {
  //
  // Оформление рисуется только тогда, когда редактор находится в постраничном
  // режиме
  //
  ...
  if (m_usePageMode) {
    ...
    qreal curHeight =
        pageHeight - (verticalScrollBar()->value() % (int)pageHeight);
    //
    ...
    while (curHeight <= height()) {
      ...
      // ... левая
      p.drawLine(0 - horizontalDelta, curHeight - pageHeight,
                 0 - horizontalDelta, curHeight - 8);
      // ... правая
      p.drawLine(x - horizontalDelta, curHeight - pageHeight,
                 x - horizontalDelta, curHeight - 8);
    ...
    if (curHeight >= height()) {
      ...
      // ... левая
      p.drawLine(0 - horizontalDelta, curHeight - pageHeight,
                 0 - horizontalDelta, height());
      // ... правая
      p.drawLine(x - horizontalDelta, curHeight - pageHeight,
                 x - horizontalDelta, height());
    }
  }
}
...
void PagesTextEdit::paintPageNumbers() {
  ...
  if (m_usePageMode && !m_pageMetrics.pxPageMargins().isNull() &&
    ...
    //
    // The current height and width displayed on the screen
    //
    qreal curHeight = pageSize.height() -
                      (verticalScrollBar()->value() % (int)pageSize.height());

    //
    // The beginning of the field must take into account the offset of the
    // scrollbar
    //
    qreal leftMarginPosition =
        pageMargins.left() - horizontalScrollBar()->value();
    //
    // Total field width
    //
    qreal marginWidth =
    ...
    //
    // Number of the first page visible on the screen
    //
    int pageNumber = verticalScrollBar()->value() / pageSize.height() + 1;

    //
    // Top margin of the first page on the screen when the previous page is not
    // visible
    //
    if (curHeight - pageMargins.top() >= 0) {
      QRectF topMarginRect(leftMarginPosition, curHeight - pageSize.height(),
    ...
    while (curHeight < height()) {
      //
      // Define the bottom margin rectangle
      //
      QRect bottomMarginRect(leftMarginPosition,
                             curHeight - pageMargins.bottom(), marginWidth,
                             pageMargins.bottom());
      ...
      //
      // Define the top margin rectangle of the next page
      //
      QRect topMarginRect(leftMarginPosition, curHeight, marginWidth,
                          pageMargins.top());
      paintPageNumber(&p, topMarginRect, true, pageNumber);
...
void PagesTextEdit::paintPageNumber(QPainter* _painter, const QRectF& _rect,
                                    bool _isHeader, int _number) {
  //
  // Верхнее поле
  //
  if (_isHeader) {
    ...
    if (m_pageNumbersAlignment.testFlag(Qt::AlignTop)) {
      _painter->drawText(
          _rect, Qt::AlignVCenter | (m_pageNumbersAlignment ^ Qt::AlignTop),
  ...
  //
    ...
    if (m_pageNumbersAlignment.testFlag(Qt::AlignBottom)) {
      _painter->drawText(
          _rect, Qt::AlignVCenter | (m_pageNumbersAlignment ^ Qt::AlignBottom),
          QString::number(_number));
    }
  }
}

void PagesTextEdit::aboutVerticalScrollRangeChanged(int _minimum,
                                                    int _maximum) {
  Q_UNUSED(_minimum);
  ...
  // значит текстэдит сам обновил интервал, применяем собственную функцию
  // коррекции
  //
...
void PagesTextEdit::aboutDocumentChanged() {
  if (m_document != document()) {
    ...
    //
    connect(document()->documentLayout(), SIGNAL(update()), this,
            SLOT(aboutUpdateDocumentGeometry()));
  }
}
