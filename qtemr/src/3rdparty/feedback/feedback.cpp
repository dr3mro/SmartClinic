...
FeedBack::FeedBack(QWidget *parent) : QWidget(parent), ui(new Ui::FeedBack) {
  ui->setupUi(this);
}
...
const bool &FeedBack::get_isBusy() { return isBusy; }

FeedBack::~FeedBack() { delete ui; }
...
void FeedBack::on_sendEmail_clicked() {
  ...
  for (int i = 0; i < ui->attachments->count(); ++i) {
    message.addPart(
        new MimeAttachment(new QFile(ui->attachments->item(i)->text())));
  }
...
void FeedBack::sendMailAsync(const MimeMessage &msg) {
  ...
  for (auto srv : m_aServers) {
    if (srv->host() == host &&
        ((srv->port() == port) & (srv->connectionType() == ct))) {
      server = srv;
      break;
    }
  }
  ...
  connect(reply, &ServerReply::finished, this, [=] {
    // qDebug() << "ServerReply finished" << reply->error() <<
    // reply->responseText();
    reply->deleteLater();
    if (reply->error()) {
