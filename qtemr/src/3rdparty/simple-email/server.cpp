...
Server::Server(QObject *parent)
    : QObject(parent), d_ptr(new ServerPrivate(this)) {
  Q_D(Server);
  d->hostname = QHostInfo::localHostName();
}

Server::~Server() { delete d_ptr; }
    ...
    case Server::SslConnection: {
      ...
      if (sslSock) {
        qCDebug(SIMPLEMAIL_SERVER)
            << "Connecting to host encrypted" << d->host << d->port;
...
void ServerPrivate::createSocket() {
...
#else
  q->connect(socket,
             static_cast<void (QTcpSocket::*)(QTcpSocket::SocketError)>(
                 &QTcpSocket::error),
             q, erroFn);
#endif
  ...
  q->connect(socket, &QTcpSocket::readyRead, q, [=] {
    ...
    switch (state) {
          ...
          if (!queue.isEmpty()) {
            ...
            if (cont.state == ServerReplyContainer::SendingCommands) {
              while (!cont.awaitedCodes.isEmpty() && socket->canReadLine()) {
                ...
                if (code != awaitedCode) {
                  ...
                  if (!cont.reply.isNull()) {
                    ServerReply *reply = cont.reply;
                    queue.removeFirst();
                    reply->finish(true, code,
                                  QString::fromLatin1(responseText));
                  } else {
                    queue.removeFirst();
                  }
                ...
                if (!capPipelining && !cont.awaitedCodes.isEmpty()) {
                  // Write next command
                  socket->write(cont.commands[cont.commands.size() -
                                              cont.awaitedCodes.size()]);
                }
              }
              ...
              if (!cont.reply.isNull()) {
                ServerReply *reply = cont.reply;
                queue.removeFirst();
                reply->finish(code != 250, code,
                              QString::fromLatin1(responseText));
              } else {
                queue.removeFirst();
              }
              qCDebug(SIMPLEMAIL_SERVER)
                  << "MAIL FINISHED" << code << queue.size()
                  << socket->canReadLine();
        ...
        if (socket->canReadLine()) {
          if (parseResponseCode(220)) {
            ...
            if (sslSock) {
              ...
              // This will be queued and sent once the connection get's
              // encrypted
      ...
      case WaitingForAuthLogin334_step1:
        if (socket->canReadLine()) {
          if (parseResponseCode(334, Server::AuthenticationFailedError)) {
            ...
            qCDebug(SIMPLEMAIL_SERVER)
                << "Sending authentication user" << username;
      ...
      default:
        qCDebug(SIMPLEMAIL_SERVER)
            << "readyRead unknown state" << socket->readAll() << state;
    }
    qCDebug(SIMPLEMAIL_SERVER) << "readyRead" << socket->bytesAvailable();
  });
}

void ServerPrivate::setPeerVerificationType(
    const Server::PeerVerificationType &type) {
  peerVerificationType = type;
  if (socket != Q_NULLPTR) {
    if (connectionType == Server::SslConnection ||
        connectionType == Server::TlsConnection) {
      switch (type) {
        case Server::VerifyNone:
          static_cast<QSslSocket *>(socket)->setPeerVerifyMode(
...
void ServerPrivate::login() {
  ...
  if (authMethod == Server::AuthPlain) {
    ...
    const QByteArray plain =
        '\0' + username.toUtf8() + '\0' + password.toUtf8();
...
void ServerPrivate::processNextMail() {
  while (!queue.isEmpty()) {
    ...
    if (cont.state == ServerReplyContainer::Initial) {
      ...
      qCDebug(SIMPLEMAIL_SERVER)
          << "Sending MAIL command" << capPipelining << cont.commands.size()
          << cont.commands << cont.awaitedCodes;
      if (capPipelining) {
...
bool ServerPrivate::parseResponseCode(int expectedCode,
  ...
  qCDebug(SIMPLEMAIL_SERVER)
      << "Got response" << responseText << "expected" << expectedCode;
