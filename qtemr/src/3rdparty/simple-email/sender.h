...
class Sender : public QObject {
 ...
 public:
  ...
  Sender(const QString &host, int port, ConnectionType ct,
  ...
  /**
   * Returns the response timeout when waiting for a message to be processed by
   * the SMTP server
   */
  int responseTimeout() const;

  /**
   * Defines the response timeout when waiting for a message to be processed by
   * the SMTP server
   */
  void setResponseTimeout(int msec);

  /**
   * Returns the timeout when waiting for a message data to be sent to the SMTP
   * server
   */
  int sendMessageTimeout() const;

  /**
   * Defines the timeout when waiting for a message data to be sent to the SMTP
   * server
   */
  ...
  /**
   * @brief setPeerVerificationType Defines how the mail-server's SSL
   * certificate should be examined
   * @param type  VerifyNone does not try to verify the identity, VerifyPeer
   * does check on server identity
   */
  void setPeerVerificationType(PeerVerificationType type);

  /**
   * @brief peerVerificationType Returns the type of verification done for the
   * mail-server's SSL certificate
