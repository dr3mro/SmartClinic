...
Sender::Sender(QObject *parent)
    : QObject(parent), d_ptr(new SenderPrivate(this)) {
  setConnectionType(TcpConnection);
}

Sender::Sender(const QString &host, int port, ConnectionType connectionType,
               QObject *parent)
    : QObject(parent), d_ptr(new SenderPrivate(this)) {
  Q_D(Sender);
  ...
  switch (connectionType) {
    ...
    case TlsConnection:
      ...
      connect(static_cast<QSslSocket *>(d->socket),
              ...
              this, &Sender::sslErrors, Qt::DirectConnection);
  }
  connect(d->socket, &QTcpSocket::stateChanged, this,
          &Sender::socketStateChanged);
...
SenderPrivate::SenderPrivate(Sender *parent) : q_ptr(parent) {}
...
bool SenderPrivate::connectToHost() {
  ...
  switch (connectionType) {
    ...
    case Sender::SslConnection: {
      ...
      if (sslSock) {
        qCDebug(SIMPLEMAIL_SENDER)
            << "Connecting to host encrypted" << host << port;
  ...
  if (connectionType == Sender::TlsConnection) {
    ...
    if (sslSock) {
      ...
      if (!sslSock->waitForEncrypted(connectionTimeout)) {
        qCDebug(SIMPLEMAIL_SENDER)
            << "Failed to encrypt connection" << sslSock->errorString();
        Q_EMIT q->smtpError(Sender::ConnectionTimeoutError);
        return false;
      }
    } else {
...
bool SenderPrivate::login() {
  ...
  if (authMethod == Sender::AuthPlain) {
    ...
    sendMessage(QByteArrayLiteral("AUTH PLAIN ") +
                userpass.toLatin1().toBase64());
...
void SenderPrivate::setPeerVerificationType(
    const Sender::PeerVerificationType &type) {
  peerVerificationType = type;
  if (socket != Q_NULLPTR) {
    if (connectionType == Sender::SslConnection ||
        connectionType == Sender::TlsConnection) {
      switch (type) {
