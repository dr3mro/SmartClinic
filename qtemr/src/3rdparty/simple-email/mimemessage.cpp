...
MimePart &MimeMessage::getContent() { return *d->content; }
...
bool MimeMessage::write(QIODevice *device) const {
  ...
  if (!d->listExtraHeaders.isEmpty()) {
    ...
    for (const QByteArray &header : listExtraHeaders) {
      data += MimeMessagePrivate::encodeData(
                  d->encoding, QString::fromLatin1(header), true) +
              QByteArrayLiteral("\r\n");
    }
  ...
  data = MimeMessagePrivate::encode(QByteArrayLiteral("From: "),
                                    QList<EmailAddress>() << d->sender,
                                    d->encoding);
  if (device->write(data) != data.size()) {
    return false;
  }
  ...
  if (d->replyTo.address().isEmpty() == false) {
    data = MimeMessagePrivate::encode(QByteArrayLiteral("Reply-To: "),
                                      QList<EmailAddress>() << d->replyTo,
                                      d->encoding);
    if (device->write(data) != data.size()) {
      return false;
    }
  ...
  data = MimeMessagePrivate::encode(QByteArrayLiteral("To: "), d->recipientsTo,
                                    d->encoding);
  if (device->write(data) != data.size()) {
    return false;
  }

  data = MimeMessagePrivate::encode(QByteArrayLiteral("Cc: "), d->recipientsCc,
                                    d->encoding);
  if (device->write(data) != data.size()) {
    return false;
  }
...
void MimeMessage::setSender(const EmailAddress &sender) { d->sender = sender; }
...
void MimeMessage::setSubject(const QString &subject) { d->subject = subject; }
...
void MimeMessage::addHeader(const QByteArray &headerName,
...
EmailAddress MimeMessage::sender() const { return d->sender; }

void MimeMessage::setReplyto(const EmailAddress &replyTo) {
  d->replyTo = replyTo;
}

QString MimeMessage::subject() const { return d->subject; }
...
MimeMessagePrivate::~MimeMessagePrivate() { delete content; }
...
QByteArray MimeMessagePrivate::encodeData(MimePart::Encoding codec,
  ...
  if (autoencoding) {
    ...
    const QByteArray result =
        QuotedPrintable::encode(simple, true, &printable, &encoded);
