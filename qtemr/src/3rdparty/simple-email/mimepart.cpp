  ...
  switch (d->contentEncoding) {
    ...
    case Base64:
      ret = QString::fromUtf8(
          QByteArray::fromBase64(d->contentDevice->readAll()));
      break;
    case QuotedPrintable:
      ret = QString::fromUtf8(
          QuotedPrintable::decode(d->contentDevice->readAll()));
      break;
  }
  return ret;
}
...
MimePartPrivate *MimePart::d_func() { return d_ptr.data(); }

MimePartPrivate::~MimePartPrivate() { delete contentDevice; }
...
bool MimePartPrivate::writeBase64(QIODevice *input, QIODevice *out) {
  ...
  while (!input->atEnd()) {
    ...
    // removed QByteArray::OmitTrailingEquals flag to generate ending == to
    // ensure compatability with Amazon SES
    QByteArray encoded =
        QByteArray(block, int(in)).toBase64(QByteArray::Base64Encoding);
...
bool MimePartPrivate::writeQuotedPrintable(QIODevice *input, QIODevice *out) {
  ...
  while (!input->atEnd()) {
    ...
    totalRead += in;
    QByteArray encoded =
        QuotedPrintable::encode(QByteArray(block, int(in)), false);
