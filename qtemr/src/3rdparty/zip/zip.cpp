...
#define S_IXUSR 0100
#endif
#define S_IRGRPZ 0040
#define S_IWGRPZ 0020
#define S_IXGRPZ 0010
#define S_IROTHZ 0004
#define S_IWOTHZ 0002
#define S_IXOTHZ 0001
#endif
...
static void writeMSDosDate(uchar *dest, const QDateTime &dt) {
  if (dt.isValid()) {
    quint16 time = (dt.time().hour() << 11)      // 5 bit hour //-V807
...
static quint32 permissionsToMode(QFile::Permissions perms) {
  quint32 mode = 0;
  if (perms & QFile::ReadOwner) mode |= S_IRUSR;
  if (perms & QFile::WriteOwner) mode |= S_IWUSR;
  if (perms & QFile::ExeOwner) mode |= S_IXUSR;
  if (perms & QFile::ReadUser) mode |= S_IRUSR;
  if (perms & QFile::WriteUser) mode |= S_IWUSR;
  if (perms & QFile::ExeUser) mode |= S_IXUSR;
  if (perms & QFile::ReadGroup) mode |= S_IRGRPZ;
  if (perms & QFile::WriteGroup) mode |= S_IWGRPZ;
  if (perms & QFile::ExeGroup) mode |= S_IXGRPZ;
  if (perms & QFile::ReadOther) mode |= S_IROTHZ;
  if (perms & QFile::WriteOther) mode |= S_IWOTHZ;
  if (perms & QFile::ExeOther) mode |= S_IXOTHZ;
  return mode;
}

static int inflate(Bytef *dest, ulong *destLen, const Bytef *source,
                   ulong sourceLen) {
  z_stream stream;
  int err;
...
static int deflate(Bytef *dest, ulong *destLen, const Bytef *source,
                   ulong sourceLen) {
  z_stream stream;
  int err;
...
static QFile::Permissions modeToPermissions(quint32 mode) {
  ...
  if (mode & S_IRGRPZ) ret |= QFile::ReadGroup;
  if (mode & S_IWGRPZ) ret |= QFile::WriteGroup;
  if (mode & S_IXGRPZ) ret |= QFile::ExeGroup;
  if (mode & S_IROTHZ) ret |= QFile::ReadOther;
  if (mode & S_IWOTHZ) ret |= QFile::WriteOther;
  if (mode & S_IXOTHZ) ret |= QFile::ExeOther;
  return ret;
}
...
static QDateTime readMSDosDate(const uchar *src) {
  ...
  return QDateTime(QDate(tm_year, tm_mon, tm_mday),
...
ZipReader::FileInfo::FileInfo(const FileInfo &other) { operator=(other); }
...
void QZipPrivate::fillFileInfo(int index, ZipReader::FileInfo &fileInfo) const {
  ...
  const quint32 mode =
      (qFromLittleEndian<quint32>(&header.h.external_file_attributes[0]) >>
       16) &
      0xFFFF;
...
class ZipWriterPrivate : public QZipPrivate {
 public:
  ...
  void addEntry(EntryType type, const QString &fileName,
                const QByteArray &contents);
};

LocalFileHeader CentralFileHeader::toLocalHeader() {
  LocalFileHeader h;
...
void ZipReaderPrivate::scanFiles() {
  ...
  if ((device->openMode() & QIODevice::ReadOnly) ==
      0) {  // only read the index from readable files.
  ...
  ZDEBUG("start_of_directory at %d, num_dir_entries=%d", start_of_directory,
         num_dir_entries);
  int comment_length = readUShort(eod.comment_length);
  if (comment_length != i) mDebug() << "QZip: failed to parse zip file.";
  ...
  for (i = 0; i < num_dir_entries; ++i) {
    ...
    if (readUInt(header.h.signature) != 0x02014b50) {
      mDebug() << "QZip: invalid header signature, index may be incomplete";
      break;
    }
    ...
    if (header.file_name.length() != l) {
      mDebug() << "QZip: Failed to read filename from zip index, index may be "
                  "incomplete";
      break;
    }
    ...
    if (header.extra_field.length() != l) {
      mDebug() << "QZip: Failed to read extra field in zip file, skipping "
                  "file, index may be incomplete";
      break;
    }
...
void ZipWriterPrivate::addEntry(
    EntryType type, const QString &fileName,
    const QByteArray
        &contents /*, QFile::Permissions permissions, QZip::Method m*/) {
#ifndef NDEBUG
  ...
  if (compression == ZipWriter::AlwaysCompress) {
    ...
    do {
      data.resize(len);
      res = deflate((uchar *)data.data(), &len,
                    (const uchar *)contents.constData(), contents.length());
      ...
      switch (res) {
        case Z_OK:
          ...
          break;
        case Z_MEM_ERROR:
          mDebug() << "QZip: Z_MEM_ERROR: Not enough memory to compress file, "
                      "skipping";
          data.resize(0);
          break;
        case Z_BUF_ERROR:
  ...
  // TODO add a check if data.length() > contents.length().  Then try to store
  // the original and revert the compression method to be uncompressed
  writeUInt(header.h.compressed_size, data.length());
  uint crc_32 = ::crc32(0, 0, 0);
  crc_32 =
      ::crc32(crc_32, (const uchar *)contents.constData(), contents.length());
  writeUInt(header.h.crc_32, crc_32);

  header.file_name = fileName.toLocal8Bit();
  if (header.file_name.size() > 0xffff) {
    mDebug() << "QZip: Filename too long, chopping it to 65535 characters";
...
*/
QIODevice *ZipReader::device() const { return d->device; }

/*!
    Returns true if the user can read the file; otherwise returns false.
*/
bool ZipReader::isReadable() const { return d->device->isReadable(); }
...
ZipReader::FileInfo ZipReader::entryInfoAt(int index) const {
  d->scanFiles();
  ZipReader::FileInfo fi;
  if (index >= 0 && index < d->fileHeaders.count()) d->fillFileInfo(index, fi);
  return fi;
}

/*!
    Fetch the file contents from the zip archive and return the uncompressed
   bytes.
*/
QByteArray ZipReader::fileData(const QString &fileName) const {
  ...
  uint skip =
      readUShort(lh.file_name_length) + readUShort(lh.extra_field_length);
  d->device->seek(d->device->pos() + skip);

  int compression_method = readUShort(lh.compression_method);
  // qDebug("file=%s: compressed_size=%d, uncompressed_size=%d",
  // fileName.toLocal8Bit().data(), compressed_size, uncompressed_size);
    ...
    do {
      ...
      switch (res) {
        case Z_OK:
          ...
          break;
        case Z_MEM_ERROR:
          mDebug() << "QZip: Z_MEM_ERROR: Not enough memory";
          break;
        case Z_BUF_ERROR:
          len *= 2;
          break;
        case Z_DATA_ERROR:
          mDebug() << "QZip: Z_DATA_ERROR: Input data is corrupted";
          break;
      }
    } while (res == Z_BUF_ERROR);
    return baunzip;
  }
  mDebug() << "QZip: Unknown compression method";
  return QByteArray();
}
...
bool ZipReader::extractAll(const QString &destinationDir) const {
  ...
  foreach (FileInfo fi, allFiles) {
    ...
    const QString absPath =
        destinationDir + QDir::separator() + fi.filePath;  //-V821
    if (fi.isSymLink) {
      ...
      if (!QFile::link(destination, absPath)) return false;
...
*/
ZipReader::Status ZipReader::status() const { return d->status; }

/*!
    Close the zip file.
*/
void ZipReader::close() { d->device->close(); }
...
/*!
    ...
    and directories. The files in the archive will be compressed in a way that
   is compatible with common zip reader applications.
*/

/*!
    Create a new zip archive that operates on the \a archive filename.  The file
   will be opened with the \a mode. \sa isValid()
*/
...
*/
QIODevice *ZipWriter::device() const { return d->device; }

/*!
    Returns true if the user can write to the archive; otherwise returns false.
*/
bool ZipWriter::isWritable() const { return d->device->isWritable(); }
...
*/
ZipWriter::Status ZipWriter::status() const { return d->status; }
...
/*!
    ...
    \value AutoCompress     A file that is added will be compressed only if that
   will give a smaller file.
*/
...
void ZipWriter::addFile(const QString &fileName, const QByteArray &data) {
  d->addEntry(ZipWriterPrivate::File, QDir::fromNativeSeparators(fileName),
              data);
}
...
void ZipWriter::addDirectory(const QString &dirName) {
  ...
  if (!name.endsWith(QLatin1Char('/'))) name.append(QLatin1Char('/'));
...
*/
void ZipWriter::addSymLink(const QString &fileName,
                           const QString &destination) {
  d->addEntry(ZipWriterPrivate::Symlink, QDir::fromNativeSeparators(fileName),
