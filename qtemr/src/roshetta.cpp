...
void Roshetta::setDocument(QTextDocument *doc) { mRoshetta = doc; }

QTextDocument *Roshetta::createRoshetta(
...
bool Roshetta::getIsDrugsOutOfRange() { return isDrugsOutOfRange; }

Roshetta::~Roshetta() { delete mRoshetta; }
...
void Roshetta::setRoshettaSize() {
  ...
  QPageSize pageSize = QPageSize(
      PageMetrics::pageSizeIdFromString(roshettaSettings.paperSizeId));
  mWidth = pageSize.sizePixels(qApp->primaryScreen()->logicalDotsPerInchX())
               .width() -
           xMargins;
  mHeight = pageSize.sizePixels(qApp->primaryScreen()->logicalDotsPerInchX())
                .height() -
            yMargins;
...
void Roshetta::setRootFrame() {
  ...
  if (roshettaData.diet.printRequired)
    rootFrameFormat.setHeight(
        PageMetrics::mmToPx(roshettaSettings.pageMargin * 2) * 2 + mHeight * 2);
  else
...
void Roshetta::stackFrames() {
  ...
  if (roshettaSettings.showBanner && roshettaSettings.prescriptionBannerStyle ==
  ...
  if (/*roshettaSettings.showHorizontalLineBelowHeader &&
       */
      roshettaSettings.prescriptionBannerStyle ==
  ...
  if (roshettaSettings.showSignaturePrintedOn) fillSignaturePrintedOn(cursor);
  ...
  if (roshettaData.diet.printRequired) fillDiet(cursor);
}

void Roshetta::makeHeader() {
  headerFormat.setWidth(mWidth);
  headerFormat.setHeight((mHeight * roshettaSettings.headerHeightPercent) /
                         100);
...
void Roshetta::makeBanner() {
  ...
  if (roshettaSettings.showBanner && roshettaSettings.prescriptionBannerStyle ==
                                         mSettings::bannerStyle::belowHeader) {
    bannerFrameFormat.setHeight(
        (mHeight * roshettaSettings.bannerHeightPercent) / 100);
    ...
    bannerFormat.setLayoutDirection(
        roshettaSettings.preferArabic ? Qt::RightToLeft : Qt::LeftToRight);

    QVector<QTextLength> bannertl =
        QVector<QTextLength>()
        << QTextLength(QTextLength::PercentageLength, 48)
...
void Roshetta::makeBody() {
  ...
  else if (roshettaSettings.prescriptionBannerStyle ==
           mSettings::bannerStyle::replaceLogo)
  ...
  QVector<QTextLength> bodytl =
      QVector<QTextLength>() << QTextLength(QTextLength::PercentageLength, 70)
...
void Roshetta::makeVitals() {
  QVector<QTextLength> vitalstl =
      QVector<QTextLength>() << QTextLength(QTextLength::PercentageLength, 30)
...
void Roshetta::makeFooter() {
  ...
  footerFormat.setHeight(
      (mHeight * roshettaSettings.footerHeightPercent / 100));
...
void Roshetta::fillHeader(QTextCursor &c) {
  ...
  if (roshettaSettings.showPrescriptionLogo &&
      roshettaSettings.prescriptionBannerStyle ==
          mSettings::bannerStyle::belowHeader)
    c.insertHtml(QString("<img src=\"logo.jpg\" width=%1 alt=\"logo\" >")
                     .arg(roshettaSettings.logoSize));
...
void Roshetta::fillBanner(QTextCursor &c) {
  QLocale loc = roshettaSettings.preferArabic
                    ? QLocale(QLocale::Arabic, QLocale::Egypt)
                    : QLocale(QLocale::English, QLocale::UnitedStates);
  QString datefmt =
      roshettaSettings.preferArabic ? "ddd yyyy/MM/dd" : "ddd dd/MM/yyyy";
  QString nextDate, visitDate;

  QString patient_age_sex = QString("%2%1").arg(
      roshettaData.sex == mSettings::mSex::male ? "m" : "f",
      dataHelper::julianToAge(
          QDate::currentDate().toJulianDay() - roshettaData.age,
          roshettaData.ageStyle));
  if (roshettaData.caseClosed ||
      roshettaData.printedinDate.date() == roshettaData.nextDate)
    nextDate =
        roshettaData.getNextFromJulian(roshettaData.nextDate.toJulianDay());
  else
  ...
  QString style =
      QString(" style=\"font-family:%1;font-size: %2px;font-weight: %3;\" ")
  ...
  c.beginEditBlock();
  c.insertHtml(
      QString("<div %2><b>Nx:</b>%1</div>").arg(roshettaData.name, style));
  c.movePosition(QTextCursor::NextCell);
  c.insertHtml(
      QString("<div %2><b>ASx:</b>%1</div>").arg(patient_age_sex, style));
  c.movePosition(QTextCursor::NextCell);
  c.insertHtml(QString("<div %2><b>eDt:</b>%1</div>").arg(visitDate, style));
  c.movePosition(QTextCursor::NextCell);

  c.insertHtml(
      QString("<div %2><b>Dx:</b>%1</div>").arg(roshettaData.Diagnosis, style));
  c.movePosition(QTextCursor::NextCell);
  c.insertHtml(
      QString("<div %3><b>ID:</b>%1</div>").arg(roshettaData.ID, style));
  c.movePosition(QTextCursor::NextCell);
  c.insertHtml(QString("<div %2><b>fDt:</b>%1 %3</div>")
                   .arg(nextDate, style, roshettaData.visitSymbole));
  c.endEditBlock();
}
...
void Roshetta::fillBody(QTextCursor &c) {
  ...
  if (roshettaSettings.showDrugs) {
    ...
    if (roshettaSettings.drugsPrintMode == mSettings::drugsPrintMode::both) {
      if (roshettaSettings.showOnlyNewlyModifiedAddedDrugs) {
        ...
        allDrugsCount = rows;
        rows += (roshettaSettings.showDrugsTitle &&
                 roshettaData.currentAlteredDrugsList.count() > 0)
                    ? 1
                    : 0;
        rows += (roshettaSettings.showDrugsTitle &&
                 roshettaData.baseAlteredDrugsList.count() > 0)
                    ? 1
                    : 0;
      } else {
        ...
        allDrugsCount = rows;
        rows += (roshettaSettings.showDrugsTitle &&
                 roshettaData.currentDrugsList.count() > 0)
                    ? 1
                    : 0;
        rows += (roshettaSettings.showDrugsTitle &&
                 roshettaData.baseDrugsList.count() > 0)
                    ? 1
                    : 0;
      }
    } else if (roshettaSettings.drugsPrintMode ==
               mSettings::drugsPrintMode::visitOnly) {
      if (roshettaSettings.showOnlyNewlyModifiedAddedDrugs) {
        rows += roshettaData.currentAlteredDrugsList.count();
        allDrugsCount = rows;
        rows += (roshettaSettings.showDrugsTitle &&
        ...
        rows += (roshettaSettings.showDrugsTitle &&
               ...
               mSettings::drugsPrintMode::baseOnly) {
      if (roshettaSettings.showOnlyNewlyModifiedAddedDrugs) {
        rows += roshettaData.baseAlteredDrugsList.count();
        allDrugsCount = rows;
        rows += (roshettaSettings.showDrugsTitle &&
    ...
    if (rows > 0) {
      ...
      if (roshettaSettings.drugsPrintMode == mSettings::drugsPrintMode::both) {
        ...
        if (roshettaSettings.drugsPrintMode ==
            mSettings::drugsPrintMode::visitOnly) {
          fillCurrentDrugs(c, "PRESCRIPTION");
          c.movePosition(QTextCursor::NextBlock, QTextCursor::MoveAnchor, 2);
        }
      ...
      int correction =
          roshettaSettings.showDrugsTableOutline ? allDrugsCount * 4 : 0;
  ...
  if (roshettaSettings.showInvestigations) fillRequests(c);
...
void Roshetta::fillDrugs(QTextCursor &c, QList<mSettings::drug> &drugs,
                         const QString &title) {
  ...
  QString tradeNameStyle =
      QString(" style=\"font-family:%1;font-size: %2px;font-weight: %3;\" ")
          .arg(roshettaSettings.roshettaFont.fontName,
               QString::number(roshettaSettings.roshettaFont.fontSize),
               (roshettaSettings.roshettaFont
                    .fontBold /*|roshettaSettings.showTradeNamesBold*/)
                   ? "bold"
                   : "normal");
  ...
  QString doseStyle =
      QString(" style=\"font-family:%1;font-size: %2px;font-weight: %3;\" ")
          .arg(roshettaSettings.doseFont.fontName,
               ...
               (roshettaSettings.doseFont.fontBold |
                (roshettaSettings.doseFont.fontBold &&
                 roshettaSettings.roshettaFont.fontBold))
                   ? "bold"
                   : "normal");

  if (!roshettaSettings.compactMode && parsedDrugsList.isEmpty() &&
      !roshettaSettings.showDrugsTableOutline &&
      allDrugsCount <= spacerfactor1 && !roshettaSettings.showDrugsTitle) {
    c.insertHtml("<br>");
  }
  ...
  if (roshettaSettings.showDrugsTitle) {
    ...
    c.currentTable()
        ->cellAt(CurrentDrugRow, 0)
        .firstCursorPosition()
        .setBlockFormat(drugsHeaderBlockFormat);
  ...
  foreach (const mSettings::drug &d, drugs) {
    if (!roshettaSettings.showDrugsTitle &&
        roshettaSettings.clearDuplicateDrugs &&
        parsedDrugsList.contains(d.TradeName)) {
    ...
    if (roshettaSettings.preferArabic) dataHelper::switchToEasternArabic(dose);
    ...
    if (roshettaSettings.showDrugsInitDate &&
        roshettaSettings.showDoseNewLine) {
      c.insertHtml(QString("<div align=left dir=LTR %4>%1 %2 <i "
                           "style=\"font-size:7px\"> %3 </i></div>")
    ...
    if (roshettaSettings.showDoseNewLine) {
      c.insertHtml("<br>");
      c.insertHtml(
          QString("<div align=left dir=RTL %2>%1</div>").arg(dose, doseStyle));
    } else {
      c.movePosition(QTextCursor::NextCell);

      if (!roshettaSettings.compactMode && parsedDrugsList.count() < 2 &&
          !roshettaSettings.showDrugsTableOutline &&
          allDrugsCount <= spacerfactor1 && !roshettaSettings.showDrugsTitle) {
        c.insertHtml("<br>");
      }
      ...
      c.insertHtml(
          QString("<div align=right dir=RTL %2>%1</div>").arg(dose, doseStyle));
    }
    ...
    if (!roshettaSettings.compactMode &&
      ...
      if (spacerFactor > 0)
        c.insertHtml(
            lineBreak.repeated(lessExpanded ? spacerFactor - 1 : spacerFactor));
    }
...
void Roshetta::fillRequests(QTextCursor &c) {
  if (roshettaData.requests.count() == 0) return;
  ...
  c.currentTable()->cellAt(0, 0).firstCursorPosition().setBlockFormat(
      requestsHeaderBlockFormat);
...
void Roshetta::fillSignaturePrintedOn(QTextCursor &c) {
  QString printedinDate =
      QDateTime::currentDateTime().toString("ddd dd/MM/yyyy hh:mm AP");
  QString style =
      QString(" style=\"font-family:%1;font-size: %2px;font-weight: %3;\" ")
  ...
  c.beginEditBlock();
  if (roshettaSettings.showSignaturePrintedOn) {
    c.insertHtml(QString("<div %2><b>printed on:</b> %1</div>")
                     .arg(printedinDate, style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(
        QString("<div %1><b>Physician's Signature:</b></div>").arg(style));
  } else {
    c.movePosition(QTextCursor::NextCell);
  }
  c.endEditBlock();
...
void Roshetta::fillVitals(QTextCursor &c) {
  ...
  if (roshettaData.vitals.weight != 0) {
    ...
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 KG</div>")
                     .arg(QString::number(roshettaData.vitals.weight), style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.height != 0) {
    c.insertHtml(QString("<div %1>H</div>").arg(style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 CM</div>")
                     .arg(QString::number(roshettaData.vitals.height), style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.sPo2 != 0) {
    c.insertHtml(QString("<div %1>sPo2</div>").arg(style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 %</div>")
                     .arg(QString::number(roshettaData.vitals.sPo2), style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.RBS != "") {
    ...
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(
        QString("<div %2> %1 mg/dl</div>").arg(roshettaData.vitals.RBS, style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.pulse != 0) {
    c.insertHtml(QString("<div %1>PR</div>").arg(style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 bpm</div>")
                     .arg(QString::number(roshettaData.vitals.pulse), style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.RR != 0) {
    c.insertHtml(QString("<div %1>RR</div>").arg(style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 cpm</div>")
                     .arg(QString::number(roshettaData.vitals.RR), style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.BP != "") {
    ...
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(
        QString("<div %2> %1 mmHg</div>").arg(roshettaData.vitals.BP, style));
    c.movePosition(QTextCursor::NextCell);
  }
  if (roshettaData.vitals.T != 0) {
    c.insertHtml(QString("<div %1>T</div>").arg(style));
    c.movePosition(QTextCursor::NextCell);
    c.insertHtml(QString("<div %2> %1 C</div>")
                     .arg(QString::number(roshettaData.vitals.T), style));
    c.movePosition(QTextCursor::NextCell);
  }
...
void Roshetta::fillAltBanner(QTextCursor &c) {
  ...
  if (!roshettaSettings.preferRTFBanner) {
    altBannerTemplate.replace(
        QRegularExpression("font-size: ?\\d{1,2}pt;"),
        QString("font-size: %1pt;")
            .arg(roshettaSettings.altBannerFont.fontSize));
    altBannerTemplate.replace(
        QRegularExpression("font-family:\\s*'([^']*)';"),
        QString("font-family:'%1';")
            .arg(roshettaSettings.altBannerFont.fontName));
    altBannerTemplate.replace(
        QRegularExpression("font-weight:?\\d{3};"),
        QString("font-weight:%1;")
            .arg(roshettaSettings.altBannerFont.fontBold ? 600 : 400));
  }
...
void Roshetta::drawHorizontalLineBelowHeader(QTextCursor &c) {
  ...
  drawHorizontalLineBelowHeaderFrameFormat.setBackground(
      roshettaSettings.enableBodyHeaderSeparator ? Qt::darkGray
                                                 : Qt::transparent);
