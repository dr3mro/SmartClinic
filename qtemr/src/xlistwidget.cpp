...
xListWidget::xListWidget(QWidget *parent)  //-V730
{
  ...
  connect(this, SIGNAL(popUpMessage(QString, QString)), parent->window(),
          SLOT(popUpMessage(QString, QString)));
  connect(this, SIGNAL(afterAddItemxListWidget(QString, QString)),
          parent->window(), SLOT(afterAddItemxListWidget(QString, QString)),
          Qt::QueuedConnection);
  connect(this, SIGNAL(itemSelectionChanged()), this,
          SLOT(selectedRowChanged()));
  connect(parent->window(), SIGNAL(updateTextFont()), this,
          SLOT(setDefaultFont()));
}

void xListWidget::setListVariables(QString ln) { listName = ln; }
...
void xListWidget::addItem(const QString &label) {
  ...
  for (int i = 0; i < count(); i++) {
    ...
    if (itemText.toUpper() == newItemText.toUpper()) {
      emit popUpMessage(
          "Warning",
          QString("%1 already exists,Please Recheck!").arg(newItemText));
      return;
    }
  }
