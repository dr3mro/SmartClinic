...
sqlBase::sqlBase(QObject *parent, QString connectionName, bool createTables)
    : msql(parent) {
  ...
  if (!dataHelper::doubleEqual(getDatabaseVersion(),
                               static_cast<double>(dbVersion)))
    upgradeDatabase();
...
QStandardItemModel *sqlBase::getPatientsTableModel() {
  ...
  if (!query->exec(
          "SELECT ID,Name,mobile,Age,Gender,Residence,maritalStatus,occupation "
          "FROM patients")) {
    ...
    while (query->next()) {
      ...
      id_Item->setData(QString::number(ID).rightJustified(5, '0'),
                       Qt::DisplayRole);
      ...
      if (deceasedList.contains(QString::number(ID)))
        name_Item->setData(QIcon(":/Graphics/ionicons/android-star.png"),
                           Qt::DecorationRole);
...
bool sqlBase::addSurgicalNote(int ID, QString surgeryID, int julianDate,
                              QString opName, QString opReport) {
  return bindAndExecSurgicalNoteQuery(createSurgicalNotesPrepareInsertPhrase(),
                                      ID, surgeryID, julianDate, opName,
                                      opReport);
}

bool sqlBase::addInvestigation(int ID, qint64 visitDate, qint64 visitTime,
                               QString invName, QString invPath, int invDate,
                               QString invTime, int invState, double price,
                               QString invResults) {
  query->clear();
  // db.transaction();
  bool p = query->prepare(
  ...
  bool e = query->exec();
  if (!e) {
    mDebug() << "Failed to exec insert investigations"
...
bool sqlBase::isVisitExists(int ID, QString visitDateTime) {
  QString sqlCmd = QString(
                       "SELECT EXISTS(SELECT * FROM visits where ID=%1 AND  "
                       "visitDateTime='%2')")
                       .arg(ID)
                       .arg(visitDateTime);
...
bool sqlBase::isPatientExists(int ID) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM patients where ID=%1)").arg(ID);

  return bool(sqlExec(sqlCmd).toInt());
}

void sqlBase::isServiceExistsInThisVisit(const int &ID, const int &visitDate,
                                         const QString &ServiceName,
                                         sqlBase::ServiceState &serviceState) {
  QString sqlCmd = QString(
                       "SELECT PRICE FROM investigations "
                       "WHERE ID=%1 AND "
                       "NAME=\"%2\" AND "
...
void sqlBase::isFollowVisitAttended(int ID, int julianDate,
                                    Attended &attended) {
  attended.clear();
  QString sql = QString(
                    "SELECT visitTime,visitType FROM visits WHERE ID=%1 AND "
                    "visitJulianDate=%2")
                    .arg(ID)
                    .arg(julianDate);
  query->clear();
...
bool sqlBase::updateVisit(int ID, QString visitDateTime, const Visit &visit) {
  ...
  QString sqlPhraseVitals =
      createVisitVitalsUpdatePreparePhrase(ID, visitDateTime);
...
bool sqlBase::saveVisit(int ID, QString visitDateTime, const Visit &visit,
                        double Price) {
  bool save = false;
  ...
  if (isVisitExists(ID, visitDateTime)) {
    ...
    if (save)
      updateVisitPrice(ID, static_cast<int>(dt.date().toJulianDay()),
                       dt.time().msecsSinceStartOfDay() / 1000, Price);
  } else {
    save = addVisit2myDataBase(visit);
    if (save)
      setVisitPrice(ID, static_cast<int>(dt.date().toJulianDay()),
                    dt.time().msecsSinceStartOfDay() / 1000, Price);
  }
  return save;
}

bool sqlBase::savePatientData(Patient &patient, DrugsItemModel *drugsModel,
                              QStandardItemModel *siblings,
                              QList<QPair<QString, int> > conditions,
                              development develop, perinatal pnatal, obGyn og) {
  db.transaction();
...
bool sqlBase::saveVisitData(sqlBase::visitData vdata) {
  db.transaction();
  bool v =
      saveVisit(vdata.ID, vdata.visitDateTime, vdata.visit, vdata.visitPrice);
  bool d =
      saveDrugs(vdata.ID, vdata.visitDate, vdata.visitTime, vdata.drugsModel);
  bool i = saveInvestigationsModel(vdata.ID, vdata.visitDate, vdata.visitTime,
                                   vdata.invModel);
  if (!v || !d || !i) {
    mDebug() << "<v:>" << v << " <d:>" << d << " <i:>" << i;
...
DrugsItemModel *sqlBase::readDrugs(int ID, qint64 visitDate, qint64 visitTime,
  ...
  query->exec(
      QString("select "
              "TRADENAME,GENERICNAME,DOSE,STARTDATE,STOPDATE,STATE,FORM,PRICE "
              "from drugs where id=%1 and visitdate=%2 and visittime=%3")
          .arg(ID)
          .arg(visitDate)
          .arg(visitTime));
  while (query->next()) {
...
QStandardItemModel *sqlBase::readSurgNotes(int ID, QStandardItemModel *model) {
  ...
  model->removeRows(0, model->rowCount());
  QStringList labels = QStringList() << "OPID"
                                     << "ID"
                                     << "DATE"
                                     << "Operation"
                                     << "Report";
  ...
  while (query->next()) {
    ...
    item2 = new QStandardItem(
        QDate::fromJulianDay(query->value(2).toInt()).toString("dd/MM/yyyy"));
    item3 = new QStandardItem(query->value(3).toString());
    item4 = new QStandardItem(query->value(4).toString());
...
bool sqlBase::saveDrugs(int ID, qint64 visitDate, qint64 visitTime,
  ...
  QString deletePhrase =
      QString("DELETE FROM drugs WHERE ID=%1 AND VISITDATE=%2 AND VISITTIME=%3")
          .arg(ID)
          .arg(visitDate)
          .arg(visitTime);
  // db.transaction();
  ...
  for (int i = 0; i < row_count; i++) {
    ...
    QString preparePhrase = QString(
        "INSERT INTO "
        "drugs "
        "(ID,VISITDATE,STATE,STARTDATE,STOPDATE,TRADENAME,GENERICNAME,FORM,"
        "DOSE,PRICE,VISITTIME) "
        "VALUES "
        "(:ID,:VISITDATE,:STATE,:STARTDATE,:STOPDATE,:TRADENAME,:GENERICNAME,:"
        "FORM,:DOSE,:PRICE,:VISITTIME) ");
    //
...
bool sqlBase::saveSiblings(int ID, QStandardItemModel *model) {
  QString dateOfBirth, Gender, TypeOfDelivery, PlaceOfBirth, Complications,
      term;

  bool b = false;

  QString deletePhrase = QString("DELETE FROM siblings WHERE ID=%1").arg(ID);
  // db.transaction();
  queryExec(deletePhrase);
  ...
  for (int i = 0; i < row_count; i++) {
    ...
    QString preparePhrase = QString(
        "INSERT INTO "
        "siblings "
        "(ID,DOB,TypeOfDelivery,GENDER,PLACEOFBIRTH,COMPLICATIONS,TERM)"
        "VALUES(:ID,:DOB,:TypeOfDelivery,:GENDER,:PLACEOFBIRTH,:COMPLICATIONS,:"
        "TERM)");
    query->clear();
    query->prepare(preparePhrase);
    query->bindValue(":ID", ID);
...
bool sqlBase::updateSurgicalNote(int ID, QString surgeryID, int julianDate,
                                 QString opName, QString opReport) {
  return bindAndExecSurgicalNoteQuery(
      createSurgicalNotesPrepareUpdatePhrase(ID, surgeryID), ID, surgeryID,
      julianDate, opName, opReport);
}

bool sqlBase::setVisitPrice(int ID, int visitDate, int visitTime,
                            double price) {
  QString query = QString(
                      "INSERT INTO visitPrices(ID,visitDate,visitTime,price) "
                      "VALUES (%1,%2,%3,%4)")
                      .arg(ID)
                      .arg(visitDate)
                      .arg(visitTime)
...
bool sqlBase::updateVisitPrice(int ID, int visitDate, int visitTime,
                               double price) {
  QString query = QString(
                      "UPDATE visitPrices SET price=%1 WHERE ID=%2 AND "
                      "visitDate=%3 AND visitTime=%4")
                      .arg(price)
                      .arg(ID)
                      .arg(visitDate)
                      .arg(visitTime);
...
bool sqlBase::deleteVisitPrice(int ID, int visitDate, int visitTime) {
  QString query = QString(
                      "DELETE FROM visitPrices WHERE ID=%1 AND visitDate=%2 "
                      "AND visitTime=%3")
                      .arg(ID)
                      .arg(visitDate)
                      .arg(visitTime);
...
sqlBase::Patient sqlBase::getPatientData(int ID) {
  ...
  while (query->next()) {
    ...
    patient.cigarretsPerDay =
        query->value(QString("cigarretsPerDay")).toString();
...
sqlBase::Visit sqlBase::getPatientVisitDataPre287(int ID,
                                                  QString dateTimeString) {
  query->clear();
  bool q = query->exec(
      QString("SELECT * FROM visits_old WHERE ID=%1 AND visitDateTime='%2'")
          .arg(ID)
          .arg(dateTimeString));
...
sqlBase::Visit sqlBase::getPatientVisitData(const int &ID,
                                            const QString &dateTimeString) {
  query->clear();
  bool q = query->exec(
      QString("SELECT * FROM visits WHERE ID=%1 AND visitDateTime='%2'")
          .arg(ID)
          .arg(dateTimeString));
...
mSettings::Vitals sqlBase::getPatientVisitVitals(
    const int &ID, const QString &dateTimeString) {
  query->clear();
  mSettings::Vitals mVitals;
  QLocale locale(QLocale::English, QLocale::UnitedStates);
  QDateTime mDate =
      locale.toDateTime(dateTimeString, "dd/MM/yyyy hh:mm AP ddd");
  int mVisitDate = mDate.date().toJulianDay();
  int mVisitTime = mDate.time().msecsSinceStartOfDay() / 1000;

  bool q = query->exec(QString("SELECT * FROM visitVitals WHERE ID=%1 AND "
                               "visitDate='%2' AND visitTime='%3'")
                           .arg(ID)
                           .arg(mVisitDate)
                           .arg(mVisitTime));
...
mSettings::Vitals sqlBase::getPatientVisitVitalsforPlaceholderText(
    const int &ID, const int &mVisitDate) {
  mSettings::Vitals mVitals;
  mVitals.pulse =
      sqlExec(
          QString(
              "SELECT pulse  FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND pulse  > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toInt();
  mVitals.BP = sqlExec(
      QString("SELECT BP     FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND BP     > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
          .arg(ID)
          .arg(mVisitDate));
  mVitals.RR =
      sqlExec(
          QString(
              "SELECT RR     FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND RR     > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toInt();
  mVitals.T =
      sqlExec(
          QString(
              "SELECT T      FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND T      > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toDouble();
  mVitals.weight =
      sqlExec(
          QString(
              "SELECT weight FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND weight > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toDouble();
  mVitals.height =
      sqlExec(
          QString(
              "SELECT height FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND height > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toDouble();
  mVitals.sPo2 =
      sqlExec(
          QString(
              "SELECT sPo2   FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND sPo2   > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
              .arg(ID)
              .arg(mVisitDate))
          .toInt();
  mVitals.RBS = sqlExec(
      QString("SELECT RBS    FROM visitVitals WHERE ID = %1 AND visitDate < %2 "
              "AND RBS    > 0  ORDER BY visitDate DESC, visitTime DESC limit 1")
          .arg(ID)
          .arg(mVisitDate));
  return mVitals;
}
...
int sqlBase::getPatientIndexLength() {
  ...
  if (!q) mDebug() << "get Index Length : " << query->lastError().text();

  while (query->next()) indexLength = query->value(0).toInt() + 1;
...
bool sortVisits(const sqlBase::visitItem &item1,
                const sqlBase::visitItem &item2) {
...
QVector<sqlBase::visitItem> sqlBase::getPatientVisits(int ID) {
  ...
  bool b = query->exec(
      QString(
          "SELECT visitType,visitDateTime,diagnosis,visitJulianDate,visitTime "
          "from visits WHERE ID=%1;")
          .arg(ID));
  if (!b) mDebug() << "Failed getting visits List" << query->lastError().text();
...
bool sqlBase::deletePatientRecord(int ID) {
  ...
  bool i =
      query->exec(QString("DELETE FROM investigations WHERE ID=%1").arg(ID));
  ...
  bool n =
      query->exec(QString("DELETE FROM surgicalNotes WHERE ID=%1").arg(ID));
  ...
  bool c = query->exec(
      QString("DELETE FROM conditions_patients WHERE ID=%1").arg(ID));
  bool dev =
      query->exec(QString("DELETE FROM development WHERE ID=%1").arg(ID));
...
bool sqlBase::copyVisit2ID(int fID, int tID, QStringList visitList) {
  ...
  foreach (QString visit, visitList) {
    int julianDate =
        static_cast<int>(locale.toDateTime(visit, "dd/MM/yyyy hh:mm AP ddd")
                             .date()
                             .toJulianDay());
    int visitTime =
        static_cast<int>(locale.toDateTime(visit, "dd/MM/yyyy hh:mm AP ddd")
                         ...
                         1000);
    bool v1 = query->exec(
        QString("UPDATE visits SET ID=%1 WHERE ID=%2 AND visitJulianDate=%3")
            .arg(tID)
            .arg(fID)
            .arg(julianDate));
    bool v2 = query->exec(
        QString("UPDATE investigations SET PATH=replace(PATH,'/%1/','/%2/') "
                "where ID=%1 AND VISITDATE=%3 AND PATH like '%data/media/%1/%'")
            .arg(fID)
            .arg(tID)
            .arg(julianDate));
    bool v3 = query->exec(
        QString("UPDATE investigations SET ID=%1 WHERE ID=%2 AND VISITDATE=%3")
            .arg(tID)
            .arg(fID)
            .arg(julianDate));
    bool v4 = query->exec(
        QString("UPDATE drugs SET ID=%1 WHERE ID=%2 AND VISITDATE=%3")
            .arg(tID)
            .arg(fID)
            .arg(julianDate));
    bool v5 = query->exec(
        QString("UPDATE visitPrices SET ID=%1 WHERE ID=%2 AND VISITDATE=%3")
            .arg(tID)
            .arg(fID)
            .arg(julianDate));
    bool v6 = query->exec(QString("UPDATE visitVitals SET ID=%1 WHERE ID=%2 "
                                  "AND VISITDATE=%3 AND VISITTIME='%4'")
                              .arg(tID)
                              .arg(fID)
                              .arg(julianDate)
                              .arg(visitTime));
...
bool sqlBase::deletePatientVisit(int ID, int julianDate, int mVisitTime) {
  ...
  bool v1 = query->exec(
      QString("DELETE FROM visits WHERE ID=%1 AND visitJulianDate=%2")
          .arg(ID)
          .arg(julianDate));
  if (!v1) mDebug() << query->lastError().text();
  bool v2 = query->exec(
      QString("DELETE FROM investigations WHERE ID=%1 AND VISITDATE=%2")
          .arg(ID)
          .arg(julianDate));
  if (!v2) mDebug() << query->lastError().text();
  bool v3 =
      query->exec(QString("DELETE FROM drugs WHERE ID=%1 AND VISITDATE=%2")
                      .arg(ID)
                      .arg(julianDate));
  if (!v3) mDebug() << query->lastError().text();
  bool v4 = query->exec(
      QString("DELETE FROM visitPrices WHERE ID=%1 AND VISITDATE=%2")
          .arg(ID)
          .arg(julianDate));
  if (!v4) mDebug() << query->lastError().text();
  bool v5 = query->exec(QString("DELETE FROM visitVitals WHERE ID=%1 AND "
                                "VISITDATE=%2 AND VISITTIME=%3")
                            .arg(ID)
                            .arg(julianDate)
                            .arg(mVisitTime));
  if (!v5) mDebug() << query->lastError().text();

  bool b = true;
  if (!v1 || !v2 || !v3 || !v4 || !v5) {
    mDebug() << QString("ERROR DELETE VISIT: %1:%2:%3 : ")
                    .arg(ID)
                    .arg(julianDate)
                    .arg(mVisitTime)
             << query->lastError().text();
  ...
  foreach (QString p, pathList) {
    qCountPathCMD =
        QString("SELECT COUNT(*)  FROM investigations WHERE PATH=\"%1\"")
            .arg(p);
...
bool sqlBase::deleteSugicalNote(int ID, QString surgeryID) {
  bool b = queryExec(
      QString("DELETE FROM surgicalNotes WHERE ID=%1 AND surgeryID=%2")
          .arg(ID)
          .arg(surgeryID));
  if (!b) {
...
QString sqlBase::createPatientUpdatePreparePhrase(int ID) {
  QString patientUpdatePreparePhrase = QString(
                                           "UPDATE patients SET "
                                           "ID=:ID,"
                                           "Name=:Name,"
                                           "dateTime=:dateTime,"
...
QString sqlBase::createVisitUpdatePreparePhrase(int ID, QString visitDateTime) {
  QString visitUpdatePreparePhrase =
      QString(
          "UPDATE visits SET "
          "visitAge=:visitAge,"
          ...
          "WHERE ID=%1 AND visitDateTime='%2'")
          .arg(ID)
          .arg(visitDateTime);
  return visitUpdatePreparePhrase;
}

QString sqlBase::createVisitVitalsUpdatePreparePhrase(int ID,
  ...
  QString visitUpdatePreparePhrase =
      QString(
          ...
          "sPo2=:sPo2,"
          "RBS=:RBS "
          "WHERE ID=%1 AND visitDate='%2' AND visitTime='%3'")
          .arg(ID)
          .arg(mVisitDate)
          .arg(mVisitTime);
  return visitUpdatePreparePhrase;
...
QString sqlBase::createSurgicalNotesPrepareUpdatePhrase(int ID,
                                                        QString surgeryID) {
  return QString(
             "UPDATE surgicalNotes "
             "SET "
             "operation=:operation,"
             "report=:report,"
             "dateTime=:dateTime "
...
QString sqlBase::createPatientInsertPreparePhrase() {
  QString patientInsertPreparePhrase = QString(
...
QString sqlBase::createVisitInsertPreparePhrase() {
  QString visitInsertPreparePhrase = QString(
      "INSERT INTO visits"
      "("
      "ID,"
      "visitAge,"
...
bool sqlBase::bindAndExecVisitSqlQuery(QString queryPhrase, Visit _visit) {
  ...
  QLocale locale(QLocale::English, QLocale::UnitedStates);
  QDateTime dt =
      locale.toDateTime(_visit.visitDateTime, "dd/MM/yyyy hh:mm AP ddd");
...
bool sqlBase::bindAndExecVisitVitalsSqlQuery(const QString &queryPhrase,
                                             const Visit &_visit,
                                             const int &ID) {
  QLocale locale(QLocale::English, QLocale::UnitedStates);
  QDateTime dt =
...
bool sqlBase::bindAndExecSurgicalNoteQuery(QString queryPhrase, int ID,
                                           QString surgeryID, int julianDate,
                                           QString opName, QString opReport) {
  query->clear();
  bool p = query->prepare(queryPhrase);
...
sqlBase::~sqlBase() { query->finish(); }

bool sqlBase::isEncryptionEnabled() {
  return bool(
      sqlExec("SELECT version from metadata WHERE data='encryption';").toInt());
}

void sqlBase::toggleEncryption(bool enabled) {
  if (sqlExec("SELECT EXISTS(SELECT 1 FROM metadata WHERE data='encryption' "
              "LIMIT 1);")
          .toInt())
    sqlExec(QString("UPDATE metadata SET version='%1' WHERE data='encryption'")
                .arg(int(enabled)));
  else
    sqlExec(
        QString(
            "INSERT INTO metadata (data,version) VALUES ('encryption','%1');")
            .arg(int(enabled)));
...
int sqlBase::getDrugsCout(int id, int date) {
  ...
  bool q = query->exec(
      QString("Select count(*) from drugs where id=%1 and visitdate=%2 ")
          .arg(id)
          .arg(date));
...
bool sqlBase::createInvstTable() {
  ...
  bool b = query->exec(
      ...
      "VISITTIME INTEGER)");
  if (!b) {
    mDebug() << "ERROR CREATING INVESTIGATIONS TABLE"
             << query->lastError().text();
  }
...
bool sqlBase::createSurgeryTable() {
  ...
  if (!b) {
    mDebug() << "ERROR CREATING surgicalNotes TABLE"
             << query->lastError().text();
  }
...
bool sqlBase::createVisitsVitalsTable() {
  ...
  if (!b) {
    mDebug() << "ERROR CREATING VISIT Vitals TABLE"
             << query->lastError().text();
  }
...
bool sqlBase::createConditionsTable() {
  ...
  if (!b) {
    mDebug() << "ERROR CREATING COMMON CONDITIONS TABLE"
             << query->lastError().text();
  }
...
bool sqlBase::createConditionsPatientTable() {
  ...
  QString querycmd =
      "CREATE TABLE IF NOT EXISTS conditions_patients(ID int UNIQUE";
  ...
  if (!b) {
    mDebug() << "ERROR CREATING PATIENTS CONDITIONS TABLE"
             << query->lastError().text();
  }
...
bool sqlBase::createPerinatalTable() {
  bool b = queryExec(
      "create table if not exists perinatal (ID integer primary key,pregLine "
      "varchar,laborLine varchar,NICULine varchar,dietLine varchar,GALine "
      "varchar,GLine varchar,PLine varchar,LLine varchar);");
...
bool sqlBase::createObGynTable() {
  ...
  if (!b) mDebug() << "ERROR CREATING OBGYN TABLE" << query->lastError().text();
  return b;
}
...
bool sqlBase::createDevelopmentTable() {
  ...
  if (!b) {
    mDebug() << "Failed to create development table:"
             << query->lastError().text();
  }
  query->finish();
  return b;
...
bool sqlBase::isUserExists(QString user) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM credentials where userName=\"%1\")")
          .arg(user);
  return bool(sqlExec(sqlCmd).toInt());
}
...
bool sqlBase::isConditionRowExists(int ID) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM conditions_patients where ID=%1)")
          .arg(ID);
  return bool(sqlExec(sqlCmd).toInt());
}
...
bool sqlBase::isDevelopmentRowExists(int ID) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM development where ID=%1)").arg(ID);
  return bool(sqlExec(sqlCmd).toInt());
}

bool sqlBase::isObGynRowExists(int ID) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM obgyn where ID=%1)").arg(ID);
  return bool(sqlExec(sqlCmd).toInt());
}

bool sqlBase::isPerinatalRowExists(int ID) {
  QString sqlCmd =
      QString("SELECT EXISTS(SELECT * FROM perinatal where ID=%1)").arg(ID);
  return bool(sqlExec(sqlCmd).toInt());
}

bool sqlBase::isPasswordSet() {
  QString sqlCmd = QString(
      "SELECT EXISTS(SELECT * FROM credentials where length(passwordHash)>1)");
  return bool(sqlExec(sqlCmd).toInt());
}

bool sqlBase::updateLMP(int ID, int julianDate) {
  QString sqlCMD =
      QString("UPDATE obgyn SET LMP=%1 WHERE ID=%2").arg(julianDate).arg(ID);
...
//    emit hideProgress();
//    QMessageBox::information(0,"Message","You have successfuly cleared
//    patients' database from unused data that might cause slow downs, good for
//    you!",QMessageBox::Ok);
...
/*
    ...
    for ( int p = 1 ; p < iLength ; p++)
    {
        ...
        query->exec();
        query->finish();
        //emit showMessage("Upgrading Database",QString("encrypting patient name
   %1 of %2").arg(p).arg(iLength));
    ...
    queryExec("CREATE TEMPORARY TABLE
   t_DRUGS(TRADENAME,GENERICNAME,DOSE,STARTDATE,STOPDATE,STATE,FORM,ID,VISITDATE)");
    queryExec("INSERT INTO t_DRUGS SELECT * FROM drugs");
    queryExec("DROP TABLE drugs");
    queryExec("CREATE  TABLE
   drugs(TRADENAME,GENERICNAME,DOSE,STARTDATE,STOPDATE,STATE,FORM,PRICE,ID,VISITDATE)");
    queryExec("INSERT INTO drugs
   (TRADENAME,GENERICNAME,DOSE,STARTDATE,STOPDATE,STATE,FORM,ID,VISITDATE)
   SELECT TRADENAME,GENERICNAME,DOSE,STARTDATE,STOPDATE,STATE,FORM,ID,VISITDATE
   FROM t_DRUGS"); queryExec("DROP TABLE t_DRUGS");
    ...
    {
        queryExec(QString("UPDATE drugs SET PRICE=\"%1\" WHERE
   TRADENAME=\"%2\"")
...
int sqlBase::getFollowUpsCountForThisDate(QDate dt, int ID) {
  ...
  query->clear();
  bool x = query->exec(
      QString("SELECT * FROM agendaView "
  ...
  while (query->next()) {
    ...
    if (!cList.contains(_id) && ID != _id) c += 1;
    cList << _id;
  }
  query->finish();
  return c;
...
bool sqlBase::exportPatientListToXLS(QString fileName) {
  ...
  if (!query->exec("SELECT ID,Name,mobile FROM patients")) {
    ...
    if (!excelexport.createXLS(fileName)) return false;
...
void sqlBase::toggleDeceased(int ID, int state, int row) {
  ...
  bool s1 = bool(
      sqlExec(
          QString("SELECT EXISTS(SELECT * FROM deceased where ID=%1)").arg(ID))
          .toInt());
  if (s1)
    cmd =
        QString("UPDATE deceased SET state=%1 WHERE ID=%2;").arg(state).arg(ID);
  else
    cmd = QString("INSERT INTO deceased (ID,state) VALUES (%1,%2);")
              .arg(ID)
              .arg(state);

  if (!query->exec(cmd)) mDebug() << cmd << query->lastError().text();
...
bool sqlBase::createAgendaView() {
  query->clear();
  bool x = query->exec(
      "CREATE VIEW IF NOT EXISTS agendaView AS SELECT "
      ...
      "patients.Residence,patients.maritalStatus,patients.occupation,visits."
      "visitType "
...
bool sqlBase::createMyRegisterView() {
  query->clear();
  bool x = query->exec(
      "CREATE VIEW IF NOT EXISTS registerView AS SELECT DISTINCT "
  ...
  if (!x) {
    mDebug() << "Failed to create myRegister view ? "
             << query->lastError().text();
  }
...
bool sqlBase::createEddView() {
  query->clear();
  bool x = query->exec(
      "CREATE VIEW IF NOT EXISTS EddView AS SELECT "
      "patients.ID, patients.Name, patients.Age, patients.Gender, "
      "patients.Residence, "
      "patients.maritalStatus, patients.occupation, patients.mobile, "
      "visits.LMPDATE , visits.Antenatal , max(visits.visitJulianDate) , "
      "obgyn.GPA "
      "FROM patients "
...
void sqlBase::createIndexes() {
  ...
  queryExec(
      "CREATE INDEX if not exists index_investigations ON investigations (ID)");
  queryExec("CREATE INDEX if not exists index_patients ON patients (ID,Name)");
  queryExec(
      "CREATE INDEX if not exists index_visits ON visits "
      "(ID,visitJulianDate,visitTime,followDate,checkButtonCaseClose,"
      "visitDateTime)");
  queryExec("CREATE INDEX if not exists index_visitPrices ON visitPrices (ID)");
  queryExec(
      "CREATE INDEX if not exists index_conditions_patients ON "
      "conditions_patients (ID)");
  queryExec("CREATE INDEX if not exists index_siblings ON siblings (ID)");
  queryExec(
      "CREATE INDEX if not exists index_surgicalNotes ON surgicalNotes (ID)");
  ...
  queryExec(
      "CREATE INDEX if not exists index_visitVitals ON visitVitals "
      "(ID,visitDate,visitTime)");
}

void sqlBase::upgradeDatabase() {
  if (dataHelper::doubleEqual(getDatabaseVersion(), double(2.80))) {
    if (migrateSmoking()) setDatabaseVersion(2.81);
  }
  if (dataHelper::doubleEqual(getDatabaseVersion(), double(2.81))) {
    if (createPerinatalTable() && migratePerinatal()) setDatabaseVersion(2.82);
  }
  ...
  if (dataHelper::doubleEqual(getDatabaseVersion(), double(2.83))) {
    if (removePerinatalDevelopmentfromPatientsTable()) setDatabaseVersion(2.84);
  }
  if (dataHelper::doubleEqual(getDatabaseVersion(), double(2.84))) {
    if (migrateObGyn()) setDatabaseVersion(2.85);
  }
  if (dataHelper::doubleEqual(getDatabaseVersion(), double(2.85))) {
    if (removeObgynfromPatientsTable()) setDatabaseVersion(2.86);
  }
...
bool sqlBase::setDatabaseVersion(double version) {
  ...
  if (!dataHelper::doubleEqual(currentVersion, version))
    b = queryExec(QString("UPDATE metadata SET version=%1 WHERE data='version'")
                      .arg(version));
  //    if(b)
...
bool sqlBase::setFollowDate(int ID, QString visitDateTime, int followDate) {
  QString query =
      QString(
          "UPDATE visits SET followDate=%1 WHERE ID=%2 AND visitDateTime='%3'")
          .arg(followDate)
          .arg(ID)
          .arg(visitDateTime);
  return queryExec(query);
...
sqlBase::development sqlBase::getDevelopment(int id) {
  ...
  query->clear();
  bool q =
      query->exec(QString("SELECT * FROM development WHERE ID=%1").arg(id));
  if (!q) {
...
bool sqlBase::saveDevelopment(development develop) {
  ...
  if (!isDevelopmentRowExists(develop.ID)) {
    if (!queryExec(QString("INSERT INTO development (ID) VALUES (%1)")
                       .arg(develop.ID)))
      return false;
  }
  ...
  bool b = query->exec();
  if (!b) mDebug() << "ERROR SAVING DEVELOPMENT" << query->lastError().text();
...
bool sqlBase::savePerinatal(sqlBase::perinatal pnatal) {
  ...
  if (!isPerinatalRowExists(pnatal.ID)) {
    if (!queryExec(
            QString("INSERT INTO perinatal (ID) VALUES (%1)").arg(pnatal.ID)))
      return false;
  }
  ...
  bool b = query->exec();
  if (!b) mDebug() << "ERROR SAVING PERINATAL" << query->lastError().text();
...
bool sqlBase::saveObGyn(sqlBase::obGyn og) {
  ...
  bool b = query->exec();
  if (!b) mDebug() << "ERROR SAVING OBGYN" << query->lastError().text();
...
double sqlBase::getDatabaseVersion() {
  ...
  if (dataHelper::doubleEqual(version, 0)) {
    bool versionExisits =
        bool(sqlExec(QString("SELECT EXISTS(SELECT version FROM metadata WHERE "
                             "data='version')"))
                 .toInt());

    if (versionExisits)
      queryExec(QString("UPDATE metadata SET version='%1' WHERE data='version'")
                    .arg(dbVersion));
    else
      queryExec(
          QString("INSERT INTO metadata (data,version) VALUES ('version',%1)")
              .arg(dbVersion));
...
QList<sqlBase::temp> sqlBase::getMigrateList() {
  ...
  query->clear();
  bool b =
      query->exec("SELECT ID,visitJulianDate,visitTime,visitType FROM visits");
  if (!b) mDebug() << "error migarting 1" << query->lastError().text();
  while (query->next()) {
...
//        b = query->exec(QString("INSERT INTO visitPrices
//        (ID,visitDate,visitTime,price) VALUES (%1,%2,%3,%4)")
//                        .arg(_t.ID)
//                        .arg(_t.date)
//                        .arg(_t.time)
//                        .arg(p));
...
//    {
//        query->exec(QString("update investigations set PRICE=%1 WHERE
//        NAME=\"%2\"")
...
bool sqlBase::migrateConditions() {
  ...
  for (int i = 1; i < indexLength; i++) {
    ...
    condition.chronicKidenyInjury =
        int(dataHelper::str2bool(p.chronicKidenyInjury));
    condition.chronicLiverDisease =
        int(dataHelper::str2bool(p.chronicLiverDisease));
    condition.chronicObstructivePulmonaryDisease =
        int(dataHelper::str2bool(p.chronicObstructivePulmonaryDisease));
    condition.congestiveHeartFailure =
        int(dataHelper::str2bool(p.congestiveHeartFailure));
    condition.coronaryArteryDisease =
        int(dataHelper::str2bool(p.coronaryArteryDisease));
    ...
    condition.PERIPHERALNEUROPATHY =
        int(dataHelper::str2bool(p.PERIPHERALNEUROPATHY));
    condition.peripheralVascularDisease =
        int(dataHelper::str2bool(p.peripheralVascularDisease));
    ...
    QString mQuery = QString(
                         "INSERT INTO conditions_patients "
                         "(ID,"
  ...
  QString createNeoPatientTableQuery = QString(
      "CREATE TABLE IF NOT EXISTS neo_patients"
      "("
      "ID  INTEGER PRIMARY KEY,"
      "Name    varchar,"
      "dateTime    varchar,"
  ...
  if (queryExec(createNeoPatientTableQuery)) {
    ...
    QString copyQuery = QString(
        "INSERT INTO neo_patients SELECT "
...
bool sqlBase::migratePerinatal() {
  ...
  return queryExec(
      "INSERT INTO perinatal SELECT "
      "ID,pregLine,laborLine,NICULine,dietLine,GALine,GLine,PLine,LLine from "
      "patients");
}
...
bool sqlBase::migrateDevelopment() {
  ...
  return queryExec(
      "INSERT INTO development SELECT "
      "ID,devHead,devStand,devSit,devWalk,devCrawl,devTeeth,devMother,devlight,"
      "devSmile,devSpeech,devSound from patients");
}

bool sqlBase::migrateObGyn() {
  mDebug() << "Database updated to V 2.85";
  return queryExec(
      "INSERT INTO obgyn SELECT "
      "ID,husband,GPA,menarche,menoPause,lastFor,every,cycle,LMP,FPAL,"
      "cyclePain from patients");
}
...
void sqlBase::updateVisitsTable288() {
  ...
  for (int _id = 1; _id <= maxID; _id++) {
    QStringList mVisitsDatesForID = sqlExecList(
        QString("SELECT visitDateTime FROM visits_old WHERE ID='%1'").arg(_id));
...
void sqlBase::updateVisitsTable289() {
  ...
  sqlExec(
      "UPDATE investigations SET VISITTIME=visits.visitTime FROM visits WHERE "
      "visits.ID=investigations.ID AND "
      "visits.visitJulianDate=investigations.VISITDATE;");
  sqlExec(
      "UPDATE drugs SET VISITTIME=visits.visitTime FROM visits WHERE "
      "visits.ID=drugs.ID AND visits.visitJulianDate=drugs.VISITDATE;");
...
void sqlBase::encryptDecryptNames(bool enable) {
  ...
  for (int id = 1; id < getPatientIndexLength(); id++) {
    ...
    queryExec(QString("UPDATE patients SET Name='%1' WHERE ID=%2;")
                  .arg(toName)
                  .arg(id));
    qApp->processEvents();
  }
...
void sqlBase::getPatientTooltip(QString &tooltip,
                                const sqlBase::patientInfo &info) {
  tooltip =
      QString(
          "<table>"
          "<tr>"
          ...
          "</tr>"
          "<tr>"
          "<td>Marital Status</td><td>:</td><td><div "
          "align=\"left\">%5</div><\td>"
          "</tr>"
          "<tr>"
          ...
          "</tr>"
          "</table>")
          .arg(info.Name, info.Age, info.Gender, info.Residence,
               info.MaritalStatus, info.Occupation, info.Mobile);
  // return tooltip;
}
...
void sqlBase::getVisitType(int ID, int visitJulianDate, Attended &attended) {
  QString sqlCmd =
      QString("SELECT visitType FROM agendaView where ID=%1 AND followDate=%2;")
          .arg(ID)
          .arg(visitJulianDate);
...
QString sqlBase::getAge(int julianDays, bool printable) {
  ...
  if (speciality == dataHelper::Speciality::Paediatrics ||
    ...
    Age = dataHelper::julianToAge(julianDays,
                                  printable ? dataHelper::AgeStyle::compact
                                            : dataHelper::AgeStyle::expanded);
  else
    Age = dataHelper::julianToAge(julianDays);
  return Age;
}

void sqlBase::agendaAttendedLoader(int julianDate,
  ...
  for (int r = 0; r < rows; r++) {
    ...
    if (attended.state) {
      ...
      agendaModel->item(r, 2)->setText(
          en_US.toString(attended.time, "hh:mm AP"));
      _attended += 1;
      for (int c = 0; c < agendaModel->columnCount(); c++) {
        agendaModel->item(r, c)->setBackground(
            visitTypes.getVisitTypesByAlgoIndex(attended.visitType).color);
      }
      agendaModel->item(r, 2)->setToolTip(
          visitTypes.getVisitTypesByAlgoIndex(attended.visitType).name);
    } else {
      ...
      for (int c = 0; c < agendaModel->columnCount(); c++) {
        if (attended.visitType == VisitTypes::n_visitsType::NewVisit &&
          ...
          agendaModel->item(r, c)->setBackground(
              QBrush(QColor::fromRgb(255, 204, 255)));
        } else {
          agendaModel->item(r, c)->setBackground(
              visitTypes.getVisitTypesByAlgoIndex(attended.followVisitType)
                  .color);
        }
        agendaModel->item(r, 2)->setToolTip(
            visitTypes.getVisitTypesByAlgoIndex(attended.followVisitType).name);
      }
    }
  }

  double _total = static_cast<double>(agendaModel->rowCount());
  if (_total > 0) percent = static_cast<double>(_attended / _total) * 100;
}

QStandardItemModel *sqlBase::getMyRegisterCalcModel(
    QStandardItemModel *myRegisterModel, QStandardItemModel *calcModel,
    sqlExtra *sqlextra, const QModelIndexList &selection, double &TOTAL) {
  calcModel->clear();
  ...
  if (selection.size() == 0)
    for (int i = 0; i < myRegisterModel->rowCount(); i++) selectedRows << i;
  else
  ...
  foreach (int currentRow, selectedRows) {
    ...
    if (visitType ==
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::NewVisit)
            .name) {
      newVisitCOUNT += 1;
      subTotalNewVisits +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
    } else if (visitType ==
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Requests)
                   .name) {
      ...
      subTotalRequests +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
               ...
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow1)
                   .name) {
      follow1VisitCOUNT += 1;
      subTotalFollow1Visits +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
    } else if (visitType ==
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow2)
                   .name) {
      follow2VisitCOUNT += 1;
      subTotalFollow2Visits +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
    } else if (visitType ==
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow3)
                   .name) {
      follow3VisitCOUNT += 1;
      subTotalFollow3Visits +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
    } else if (visitType ==
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow4)
                   .name) {
      follow4VisitCOUNT += 1;
      subTotalFollow4Visits +=
          myRegisterModel->index(currentRow, 5).data().toDouble();
    } else if (visitType ==
               visitTypes
                   .getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Free)
                   .name) {
      freeVisitCOUNT += 1;
      subTotalFree = 0;
    }
  ...
  if (newVisitCOUNT > 0) {
    ...
    newCOUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::NewVisit)
            .color);
    newCOUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *newSUBTOTAL =
        new QStandardItem(QString::number(subTotalNewVisits, 'f', 2));
    newSUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    newSUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::NewVisit)
            .color);
    calcModel->appendRow(QList<QStandardItem *>()
                         << newItem << newCOUNT << newSUBTOTAL);
  }

  if (requestsVisitCOUNT > 0) {
    QStandardItem *requestsItem = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Requests)
            .name);
    requestsItem->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Requests)
            .color);
    QStandardItem *requestsCOUNT =
        new QStandardItem(QString::number(requestsVisitCOUNT));
    requestsCOUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Requests)
            .color);
    requestsCOUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *requestSUBTOTAL =
        new QStandardItem(QString::number(subTotalRequests, 'f', 2));
    requestSUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    requestSUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Requests)
            .color);
    calcModel->appendRow(QList<QStandardItem *>()
                         << requestsItem << requestsCOUNT << requestSUBTOTAL);
  }

  if (follow1VisitCOUNT > 0) {
    QStandardItem *followItem1 = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow1)
            .name);
    followItem1->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow1)
            .color);
    QStandardItem *follow1COUNT =
        new QStandardItem(QString::number(follow1VisitCOUNT));
    follow1COUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow1)
            .color);
    follow1COUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *follow1SUBTOTAL =
        new QStandardItem(QString::number(subTotalFollow1Visits, 'f', 2));
    follow1SUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow1)
            .color);
    follow1SUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    calcModel->appendRow(QList<QStandardItem *>()
                         << followItem1 << follow1COUNT << follow1SUBTOTAL);
  }

  if (maxFollowsPerProblem > 1 && follow2VisitCOUNT > 0) {
    QStandardItem *followItem2 = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow2)
            .name);
    followItem2->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow2)
            .color);
    QStandardItem *follow2COUNT =
        new QStandardItem(QString::number(follow2VisitCOUNT));
    follow2COUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow2)
            .color);
    follow2COUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *follow2SUBTOTAL =
        new QStandardItem(QString::number(subTotalFollow2Visits, 'f', 2));
    follow2SUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow2)
            .color);
    follow2SUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    calcModel->appendRow(QList<QStandardItem *>()
                         << followItem2 << follow2COUNT << follow2SUBTOTAL);
  }

  if (maxFollowsPerProblem > 2 && follow3VisitCOUNT > 0) {
    QStandardItem *followItem3 = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow3)
            .name);
    followItem3->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow3)
            .color);
    QStandardItem *follow3COUNT =
        new QStandardItem(QString::number(follow3VisitCOUNT));
    follow3COUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow3)
            .color);
    follow3COUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *follow3SUBTOTAL =
        new QStandardItem(QString::number(subTotalFollow3Visits, 'f', 2));
    follow3SUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    follow3SUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow3)
            .color);
    calcModel->appendRow(QList<QStandardItem *>()
                         << followItem3 << follow3COUNT << follow3SUBTOTAL);
  }

  if (maxFollowsPerProblem > 3 && follow4VisitCOUNT > 0) {
    QStandardItem *followItem4 = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow4)
            .name);
    followItem4->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow4)
            .color);
    QStandardItem *follow4COUNT =
        new QStandardItem(QString::number(follow4VisitCOUNT));
    follow4COUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    follow4COUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow4)
            .color);
    QStandardItem *follow4SUBTOTAL =
        new QStandardItem(QString::number(subTotalFollow4Visits, 'f', 2));
    follow4SUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    follow4SUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Follow4)
            .color);
    calcModel->appendRow(QList<QStandardItem *>()
                         << followItem4 << follow4COUNT << follow4SUBTOTAL);
  }

  if (freeVisitCOUNT > 0) {
    QStandardItem *freeItem = new QStandardItem(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Free)
            .name);
    freeItem->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Free)
            .color);
    QStandardItem *freeCOUNT =
        new QStandardItem(QString::number(freeVisitCOUNT));
    freeCOUNT->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Free)
            .color);
    freeCOUNT->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    QStandardItem *freeSUBTOTAL =
        new QStandardItem(QString::number(subTotalFree, 'f', 2));
    freeSUBTOTAL->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(VisitTypes::n_visitsType::Free)
            .color);
    freeSUBTOTAL->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
    calcModel->appendRow(QList<QStandardItem *>()
                         << freeItem << freeCOUNT << freeSUBTOTAL);
  }

  TOTAL = static_cast<double>(subTotalNewVisits + subTotalFollow1Visits +
                              subTotalFollow2Visits + subTotalFollow3Visits +
                              subTotalFollow4Visits + subTotalFree);
  for (int c = startCol; c < endCol; c++) {
    ...
    foreach (int r, selectedRows) {
      count +=
          myRegisterModel->index(r, c).data(Qt::CheckStateRole).toInt() / 2;
    ...
    if (count > 0) {
      ...
      QStandardItem *itemPrice =
          new QStandardItem(QString::number(subTOTAL, 'f', 2));
      itemPrice->setBackground(QBrush(QColor::fromRgb(255, 204, 255)));
      itemPrice->setData(Qt::AlignCenter, Qt::TextAlignmentRole);
      calcModel->appendRow(QList<QStandardItem *>()
                           << itemService << itemCount << itemPrice);
    }
    TOTAL += subTOTAL;
    qApp->processEvents();
  }
  calcModel->setHorizontalHeaderLabels(QStringList() << "Service"
                                                     << "Count"
                                                     << "SUBTOTAL");
  return calcModel;
}

void sqlBase::registerServiceLoader(QStandardItemModel *myRegisterModel,
                                    sqlExtra *sqlextra) {
  QStringList serviceList = sqlextra->getPaidServicesList();
  ...
  for (int x = 6; x < columns; x++) {
    ...
    for (int r = 0; r < rows; r++) {
      ...
      item->setCheckState(
          Qt::CheckState((serviceState.state) ? Qt::Checked : Qt::Unchecked));
    }
  }
}

void sqlBase::createNewVisit(int ID, QString previous,
                             const QDateTime &datetime,
                             const QDate &followUpDate, int visitType,
                             double visitPrice, DrugsItemModel *drugsModel,
                             InvestModel *investModel, sqlExtra *sqlextra) {
  QLocale locale = QLocale(QLocale::English, QLocale::UnitedStates);
  QString englishDateTime =
      locale.toString(datetime, "dd/MM/yyyy hh:mm AP ddd");

  qint64 nextDateJulian =
      locale.toDateTime(englishDateTime, "dd/MM/yyyy hh:mm AP ddd")
          .date()
          .toJulianDay();
  qint64 nextVisitTime =
      locale.toDateTime(englishDateTime, "dd/MM/yyyy hh:mm AP ddd")
          .time()
          .msecsSinceStartOfDay() /
      1000;

  qint64 previousDateJulian =
      locale.toDateTime(previous, "dd/MM/yyyy hh:mm AP ddd")
          .date()
          .toJulianDay();
  qint64 previousTime = locale.toDateTime(previous, "dd/MM/yyyy hh:mm AP ddd")
                            .time()
                            .msecsSinceStartOfDay() /
                        1000;
...
bool sqlBase::addCondition(QString name, int _id) {
  QString query1 =
      QString("INSERT INTO conditions (_id,state,name) VALUES (%1,%2,\"%3\");")
          .arg(_id)
          .arg(1)
          .arg(name);
...
bool sqlBase::insertDefaultConditions() {
  if (getConditionsStatus() == static_cast<int>(conditionsVersion)) return true;

  QStringList f1 = QStringList() << "0"
                                 << "1"
                                 << "2"
                                 << "3"
                                 << "4"
  ...
  for (int i = 0; i < f1.count(); i++) {
    query =
        QString("INSERT INTO conditions (_id,state,name) VALUES(%1,1,\"%2\")")
            .arg(f1.at(i), f2.at(i));
    queryExec(query);
  }

  // set conditions metadata
  queryExec(QString("INSERT INTO metadata (data,version) VALUES(\"%1\",1)")
                .arg("conditions"));
  return true;
}
...
bool sqlBase::savePatientConditions(int ID, QList<QPair<QString, int> > list) {
  ...
  if (!isConditionRowExists(ID)) {
    if (!queryExec(QString("INSERT INTO conditions_patients (ID) VALUES (%1)")
                       .arg(ID)))
      return false;
  }

  foreach (p, list) _s.append(QString("%1=%2, ").arg(p.first).arg(p.second));

  query = QString("UPDATE conditions_patients SET %1 WHERE ID=%2")
              .arg(_s.left(_s.lastIndexOf(",")))
              .arg(ID);
  return queryExec(query);
}
...
QList<QPair<QString, int> > sqlBase::readPatientConditions(int ID) {
  ...
  query->clear();
  bool q = query->exec(
      QString("SELECT * FROM conditions_patients WHERE ID=%1").arg(ID));
  if (!q) {
    mDebug() << "Failed to load patient conditions"
             << query->lastError().text();
  }
...
bool sqlBase::createPatientItemModel() {
  if (patientTableModel == nullptr) delete patientTableModel;
...
QStringList sqlBase::sqlExecList(QString sqlCmd, bool crypt) {
  ...
  while (query->next()) {
    item = crypt ? getHumanizedName(query->value(0).toString())
...
QString sqlBase::genericQuery(int ID, QString col, QString table) {
  QString sqlPhrase = QString("SELECT %1 FROM %2 WHERE ID=%3")
                          .arg(col, table, QString::number(ID));
...
QList<QPair<QString, QString> > sqlBase::getListVisitsType(int ID) {
  QString sqlPhrase =
      QString("SELECT visitDateTime,visitType FROM visits WHERE ID=%1").arg(ID);
  return sqlExecPair(sqlPhrase);
}

InvestModel *sqlBase::getInvestigationsModel(InvestModel *investModel, int ID,
  ...
  if (visitJulianDate == 0 && visitTime == 0)
    selectInvestigationQuery = QString(
                                   "SELECT * FROM investigations WHERE ID=%1 "
                                   "AND invState !=0 AND length(PATH) > 8")
                                   .arg(ID);
  else
    selectInvestigationQuery = QString(
                                   "SELECT * FROM investigations WHERE ID=%1 "
                                   "AND VISITDATE=%2 AND VISITTIME=%3")
                                   .arg(ID)
                                   .arg(visitJulianDate)
                                   .arg(visitTime);
  ...
  if (!i) {
    mDebug() << "FAILED TO GET INVESTIGATIONS MODEL FOR ID:VISITDATE" << ID
             << ":" << visitJulianDate << query->lastError().text();
  }
  QStringList paths;
  int row = 0;
  QLocale locale(QLocale::English, QLocale::UnitedStates);
  ...
  while (query->next()) {
    ...
    auto visitDateItem =
        new QStandardItem(QString::number(query->value(2).toInt()));
    auto pathItem = new QStandardItem(path);
    auto invDateItem = new QStandardItem();
    auto invTimeItem = new QStandardItem();
    auto invStateItem =
        new QStandardItem(QString::number(query->value(6).toInt()));
    ...
    {
      invDateItem->setText(locale.toString(
          QDate::fromJulianDay(query->value(4).toInt()), "dd/MM/yyyy"));
    ...
    if (visitJulianDate == 0)
      invTimeItem->setText(locale.toString(
          QTime::fromMSecsSinceStartOfDay(query->value(5).toInt() * 1000),
          "hh:mm AP"));
    else
...
bool sqlBase::saveInvestigationsModel(int ID, qint64 visitDate,
                                      qint64 visitTime, InvestModel *model) {
  ...
  d = query->exec(QString("DELETE FROM investigations WHERE ID=%1 AND "
                          "VISITDATE=%2 AND VISITTIME=%3")
                      .arg(ID)
                      .arg(visitDate)
                      .arg(visitTime));
  ...
  for (int row = 0; row < rows; row++) {
    ...
    // invTime = QTime::fromString(model->item(row,5)->text(),"hh:mm
    // AP").msecsSinceStartOfDay();
    ...
    e = addInvestigation(ID, visitDate, visitTime, invName, invPath, invDate,
                         invTime, invState, price, invResults);
    if (!e) x = e;
  }
  return x;
}
...
int sqlBase::getInvestigationsCount(int ID) {
  query->clear();
  int size = 0;
  bool z = query->exec(QString("SELECT  invState from investigations WHERE "
                               "ID=%1 AND length(PATH) > 8;")
                           .arg(ID));
  if (!z) {
...
bool sqlBase::deleteInvestigation(int ID, qint64 visitDate, qint64 visitTime,
...
bool sqlBase::removeMediaUrlNormalizeRequest(int ID, int visitDate,
                                             QString path, QString name) {
  query->clear();
  bool b =
      query->exec(QString("UPDATE investigations SET INVSTATE=0 , PATH=\"\""
                          " WHERE ID=%1 AND"
                          " VISITDATE=%2 AND"
                          " NAME=\"%4\"")
                      .arg(ID)
...
QStringList sqlBase::investigationsPathListForVisit(int ID, int visitDate) {
  ...
  bool b = query->exec(QString("SELECT PATH FROM investigations"
                           ...
                           .arg(visitDate));
  if (!b) {
    mDebug() << "ERROR DELETE VISIT investigation: ? : "
             << query->lastError().text();
  }
  while (query->next()) {
...
QStandardItemModel *sqlBase::getAgendaModel(int julianDate,
                                            QStandardItemModel *agendaModel) {
  query->clear();
  bool x = query->exec(QString("SELECT * FROM agendaView WHERE followDate=%1 "
                           ...
                           .arg(julianDate));
  //    bool x = query->exec(QString("SELECT * FROM agendaView WHERE
  //    followDate=%1 AND checkButtonCaseClose=\"false\"")
  //                         .arg(julianDate));
  if (!x) {
  ...
  while (query->next()) {
    ...
    idItem->setData(QVariant(QString("%1").arg(_id, 5, 10, QLatin1Char('0'))),
                    Qt::DisplayRole);
    info.Name = getHumanizedName(query->value(1).toString());
  ...
  agendaModel->setHorizontalHeaderLabels(QStringList() << "ID"
                                                       << "Name"
                                                       << "Attended");
...
QStandardItemModel *sqlBase::getMyRegisterModel(
    RegisterRange timeframe, QStandardItemModel *myRegisterModel,
    sqlExtra *sqlextra) {
  mRegisterMutex.lock();
  ...
  QStringList labels = QStringList()
                       ...
                       << "julianDate"
                       << "VisitType"
                       << "price" << sqlextra->getPaidServicesList();
  ...
  while (query->next()) {
    ...
    idItem->setData(QVariant(QString("%1").arg(_id, 5, 10, QLatin1Char('0'))),
                    Qt::DisplayRole);
    idItem->setBackground(visitTypes.getVisitTypesByAlgoIndex(visitType).color);

    QStandardItem *nameItem = new QStandardItem(info.Name);
    nameItem->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(visitType).color);

    QStandardItem *visitTypeItem =
        new QStandardItem(visitTypes.getVisitTypesByAlgoIndex(visitType).name);
    visitTypeItem->setBackground(
        visitTypes.getVisitTypesByAlgoIndex(visitType).color);

    QStandardItem *dateTimeItem =
        new QStandardItem(QString("%1%2").arg(julianDate).arg(timeMS));
    ...
    QString date =
        en_US.toString(QDate::fromJulianDay(julianDate), "dd/MM/yyyy");
    QString time = en_US.toString(
        QTime::fromMSecsSinceStartOfDay(timeMS * 1000), "hh:mm AP");

    myRegisterModel->item(row, 4)->setToolTip(
        QString("%1 - %2").arg(date, time));
...
QStandardItemModel *sqlBase::getEddModel(qint64 start, qint64 end,
  ...
  while (query->next()) {
    ...
    info.Age = getAge(static_cast<int>(todayJulian) -
                      query->value(QString("Age")).toInt());
    ...
    QStandardItem *gpaItem =
        new QStandardItem(query->value(QString("GPA")).toString());

    QString eddString = QDate::fromJulianDay(
                            int(query->value(QString("LMPDATE")).toInt() + 280))
  ...
  eddModel->setHorizontalHeaderLabels(QStringList() << "ID"
                                                    << "Name"
                                                    << "Age"
                                                    << "Mobile"
                                                    << "GPA"
...
bool sqlBase::createEmptyPatientProfiles(int maxID) {
  ...
  for (int id = minID; id < maxID + 1; id++) {
    ...
    sqlCMD =
        QString(
            "INSERT INTO patients(ID,Name,dateTime)VALUES(%1,\"%2\",\"%3\")")
            .arg(QString::number(id), name, dateTime);
    emit emptyProfileProgressbar(id, maxID);
    query->clear();
    b = query->exec(sqlCMD);
    if (!b) {
      mDebug() << "Failed to create empty profiles ?"
               << query->lastError().text();
      db.rollback();
      query->finish();
      return b;
...
bool sqlBase::setPassWD(QString pw) {
  ...
  if (isUserExists("doktor")) {
    b = queryExec(
        QString("UPDATE credentials SET passwordHash=\"%1\"").arg(password));
  } else {
    b = queryExec(QString("INSERT INTO credentials (userName,passwordHash) "
                          "VALUES (\"doktor\",\"%1\")")
                      .arg(password));
  }
  return b;
...
bool sqlBase::clearPW() {
  ...
  if (isUserExists("doktor")) {
    ...
    b = queryExec(
        "INSERT INTO credentials (userName,passwordHash) VALUES "
        "(\"doktor\",NULL)");
  }
  return b;
}
...
QString sqlBase::getUserPasswordHash(QString user) {
  return sqlExec(
      QString("SELECT passwordHash FROM credentials WHERE userName=\"%1\"")
          .arg(user));
}
