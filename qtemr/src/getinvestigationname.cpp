...
getInvestigationName::getInvestigationName(QWidget *parent)
    : mDialog(parent), ui(new Ui::getInvestigationName) {
  ui->setupUi(this);
  setFixedSize(size());
  setWindowFlags(windowFlags() | Qt::Window | Qt::FramelessWindowHint);
  ui->OK->setEnabled(false);

  QAction *selectResourceAction = ui->Path->addAction(
      QIcon(":/ionicons/more"), QLineEdit::ActionPosition::TrailingPosition);
  connect(selectResourceAction, &QAction::triggered, this,
          &getInvestigationName::setPath);
}
...
getInvestigationName::~getInvestigationName() { delete ui; }
...
void getInvestigationName::on_closeButton_clicked() { close(); }

void getInvestigationName::setPath() {
  QFileDialog fileDialog;
  QString desktop =
      QStandardPaths::writableLocation(QStandardPaths::DesktopLocation);
  QString selectedImagePath = fileDialog.getOpenFileName(
      this, "Select Photo Copy", desktop, "*.jpg *.jpeg *.png *.pdf");
...
void getInvestigationName::on_Close_clicked() { close(); }
