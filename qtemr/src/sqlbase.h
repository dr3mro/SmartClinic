 ...
 public:
  ...
  struct Visit {
    ...
    bool operator!=(const Visit &visit) const {
      return std::tie(
                 ID, visitAge, presentation, presentationAnalysis, diagnosis,
                 // investigations,
                 invResults,
                 // drugList,
                 // pulse,
                 ...
                 // weight,
                 // length,
                 headCir, exGeneral, exChestHeart, exAbdback, exLL, visitNotes,
                 visitDateTime, followDate, visitType, checkButtonCaseClose,
                 LMPDATE, ObstUS, Antenatal, pelvicExam, vitals) !=
             std::tie(visit.ID, visit.visitAge, visit.presentation,
                      visit.presentationAnalysis, visit.diagnosis,
                      ...
                      // visit.weight,
                      // visit.length,
                      visit.headCir, visit.exGeneral, visit.exChestHeart,
                      visit.exAbdback, visit.exLL, visit.visitNotes,
                      visit.visitDateTime, visit.followDate, visit.visitType,
                      visit.checkButtonCaseClose, visit.LMPDATE, visit.ObstUS,
                      visit.Antenatal, visit.pelvicExam, visit.vitals);
    }
  };
  ...
  struct development {
    ...
    bool operator!=(const development &devel) const {
      return std::tie(devHead, devStand, devSit, devWalk, devCrawl, devTeeth,
                      devMother, devlight, devSmile, devSpeech, devSound) !=
             std::tie(devel.devHead, devel.devStand, devel.devSit,
                      devel.devWalk, devel.devCrawl, devel.devTeeth,
                      devel.devMother, devel.devlight, devel.devSmile,
                      devel.devSpeech, devel.devSound);
    }
  };
  ...
  struct perinatal {
    ...
    bool operator!=(const perinatal &pnatal) const {
      return std::tie(pregLine, laborLine, NICULine, dietLine, GALine, GLine,
                      PLine, LLine) !=
             std::tie(pnatal.pregLine, pnatal.laborLine, pnatal.NICULine,
                      pnatal.dietLine, pnatal.GALine, pnatal.GLine,
                      pnatal.PLine, pnatal.LLine);
    }
  };
  ...
  struct obGyn {
    ...
    bool operator!=(const obGyn &og) const {
      return std::tie(GPA, FPAL, menarche, cycle, lastFor, every, LMP,
                      menoPause, cyclePain) !=
             std::tie(og.GPA, og.FPAL, og.menarche, og.cycle, og.lastFor,
                      og.every, og.LMP, og.menoPause, og.cyclePain);
    }
  };
  ...
  explicit sqlBase(QObject *parent = nullptr,
                   QString connectionName = "qt_sql_base_connection",
  ...
  mSettings::Vitals getPatientVisitVitals(const int &ID,
                                          const QString &dateTimeString);
  mSettings::Vitals getPatientVisitVitalsforPlaceholderText(
      const int &ID, const int &mVisitDate);
  bool addPatient2myDataBase(Patient patient);
  bool addVisit2myDataBase(Visit visit);
  bool addSurgicalNote(int ID, QString surgeryID, int julianDate,
                       QString opName, QString opReport);
  bool addInvestigation(int ID, qint64 visitDate, qint64 visitTime,
  ...
  inline void isServiceExistsInThisVisit(const int &ID, const int &visitDate,
  ...
  bool saveVisit(int ID, QString visitDateTime, const Visit &visit,
                 double Price);
  bool savePatientData(Patient &patient, DrugsItemModel *drugsModel,
  ...
  DrugsItemModel *readDrugs(int ID, qint64 visitDate, qint64 visitTime,
                            DrugsItemModel *drugsModel);
  ...
  bool saveDrugs(int ID, qint64 visitDate, qint64 visitTime,
                 DrugsItemModel *model);
  bool saveSiblings(int ID, QStandardItemModel *model);
  bool updateSurgicalNote(int ID, QString surgeryID, int julianDate,
                          QString opName, QString opReport);
  ...
  bool bindAndExecVisitVitalsSqlQuery(const QString &queryPhrase,
                                      const Visit &_visit, const int &ID);
  bool bindAndExecSurgicalNoteQuery(QString queryPhrase, int ID,
  ...
  InvestModel *getInvestigationsModel(InvestModel *investModel, int ID,
                                      qint64 visitJulianDate, qint64 visitTime);
  bool saveInvestigationsModel(int ID, qint64 visitDate, qint64 visitTime,
                               InvestModel *model);
  int getInvestigationsCount(int ID);
  bool deleteInvestigation(int ID, qint64 visitDate, qint64 visitTime,
                           QString path, QString name);
  bool removeMediaUrlNormalizeRequest(int ID, int visitDate, QString path,
                                      QString name);
  QStringList investigationsPathListForVisit(int ID, int visitDate);
  void closeDataBase();
  QStandardItemModel *getAgendaModel(int julianDate,
                                     QStandardItemModel *agendaModel);
  QStandardItemModel *getEddModel(qint64 start, qint64 end,
  ...
  void agendaAttendedLoader(int julianDate, QStandardItemModel *agendaModel,
                            double &percent);
  QStandardItemModel *getMyRegisterCalcModel(
  ...
  QStandardItemModel *getMyRegisterModel(RegisterRange timeFrame,
                                         QStandardItemModel *myRegisterModel,
                                         sqlExtra *sqlextra);
  void registerServiceLoader(QStandardItemModel *myRegisterModel,
                             sqlExtra *sqlextra);
  void createNewVisit(int ID, QString previous, const QDateTime &datetime,
