...

{
  ...
  connect(completar, SIGNAL(activated(QString)), this,
          SLOT(insertCompletion(QString)), Qt::UniqueConnection);
  connect(this, SIGNAL(selectionChanged()), this, SLOT(false2autoSelected()));

  if (WindowName == "visitsBox")
    connect(parent->window(), SIGNAL(onVisitBoxClose()), completar->popup(),
  ...
  if (WindowName != "printDrugs")
    setToolTip(
        QString("<html>"
                ...
                "<tr><td>- [CTRL + .] to insert • </td><td>- [CTRL + +] to "
                "increase font size.</td></tr>"
                "<tr><td>- [CTRL + ,] to insert ◾ </td><td>- [CTRL + -] to "
                "decrease font size.</td></tr>"
                "<tr><td>- [CTRL + →] to insert → </td><td>- [CTRL + ↑] to "
                "insert ↑</td></tr>"
                "<tr><td>- [CTRL + ←] to insert ← </td><td>- [CTRL + ↓] to "
                "insert ↓</td></tr>"
                "<tr><td>- [ALT + /] to set Green flag.</td><td>- [ALT + |] to "
                "set Blue flag.</td></tr>"
                "<tr><td>- [ALT + \\] to set Red flag.</td><td>- [CTRL + "
                "R][SHIFT+2SPACES] to reset formating.</td></tr>"
                "</table></html>"));

  connect(&watcher, SIGNAL(finished()), this, SLOT(completerLoader()));
  if (WindowName != "printDrugsHeaderTab" &&
      WindowName != "printDrugsBannerTab" &&
      WindowName != "printDrugsFooterTab")
    connect(parent->window(), SIGNAL(loadCompleters()), this,
            SLOT(setCompleter()));

  connect(this, SIGNAL(selectionChanged()), this, SLOT(isSelectionChanged()));

  if (WindowName == "MainWindow" || WindowName == "visitsBox") {
    connect(this, SIGNAL(removeItemFromCompleter(QString, int)),
            parent->window(), SLOT(removeItemFromDictionary(QString, int)));
    connect(parent->window(), SIGNAL(removeFromCompleter(int)), this,
            SLOT(removeFromCompleter(int)));
  ...
  if (WindowName == "printDrugs" || WindowName == "surgicalNoteEditor" ||
      WindowName == "DietEditor" || WindowName == "printDrugsHeaderTab" ||
      WindowName == "printDrugsBannerTab" ||
      WindowName == "printDrugsFooterTab")
    return;

  connect(parent->window(), SIGNAL(reloadCompleter()), this,
...
void TextEdit::updateCompleter() { setCompleter(); }

QCompleter *TextEdit::completer() const { return completar; }

void TextEdit::cleanCompleterMemory() {
  model->removeRows(0, model->rowCount());
}

void TextEdit::setInkFont(QString font) { inkFont = font; }
...
void TextEdit::insertChar(QString str) {
  ...
  if (r.match(str, QRegularExpression::NormalMatch).hasMatch() ||
      isSelectionStyled)
    tc.clearSelection();
  else
    tc.removeSelectedText();
...
void TextEdit::makeEditable(bool b) { setReadOnly(!b); }

QString TextEdit::getSelectedText() { return textCursor().selectedText(); }
...
void TextEdit::createRichTextToolBox() {
  ...
  connect(rt, SIGNAL(setFGcolor(const QColor &)), this,
          SLOT(setForegroundColor(const QColor &)));
  connect(rt, SIGNAL(setBGcolor(const QColor &)), this,
...
void TextEdit::setDefaultFont() {
  ...
  setStyleSheet(QString("QTextEdit{font:%1 %2pt \"%3\";}")
                    .arg(defaultBold ? "bold" : "")
                    .arg(DefaultPointSize)
...
void TextEdit::removeFromCompleter(int row) { model->removeRow(row); }

void TextEdit::isSelectionChanged() { isSelectionStyled = false; }
...
void TextEdit::false2autoSelected() { isAutoSelection = false; }
...
void TextEdit::insertCompletion(const QString &completion) {
  if (completar->widget() != this) return;
  QTextCursor tc = textCursor();
...
void TextEdit::showRichTextToolBox() {
                            ...
                            // !QGuiApplication::keyboardModifiers().testFlag(Qt::ShiftModifier))
  {
    ...
    bool bold =
        (cursor.charFormat().fontWeight() == QFont::Bold) ? true : false;
    ...
    bool highlight =
        (cursor.charFormat().background() == Qt::yellow) ? true : false;
    bool alignLeft =
        (cursor.blockFormat().alignment() == Qt::AlignLeft) ? true : false;
    bool alignCenter =
        (cursor.blockFormat().alignment() == Qt::AlignCenter) ? true : false;
    bool alignRight =
        (cursor.blockFormat().alignment() == Qt::AlignRight) ? true : false;
    bool upper =
        (cursor.charFormat().fontCapitalization() == QFont::AllUppercase)
            ? true
            : false;
    rt->syncFontPoint(font, point, bold, italic, underLine, strike, highlight,
                      alignLeft, alignCenter, alignRight, upper);
    QPoint pos = viewport()->mapToGlobal(
        QPoint(cursorRect().x(), cursorRect().y() + 15));
    rt->move(pos);
    rt->show();
    isSelectionStyled = false;
  }
...
void TextEdit::setUpper(bool normal) {
  QTextCharFormat fmt;
  fmt.setFontCapitalization(normal ? QFont::Capitalization::AllUppercase
                                   : QFont::Capitalization::AllLowercase);
  mergeCurrentCharFormat(fmt);
  isSelectionStyled = true;
}
...
void TextEdit::focusInEvent(QFocusEvent *e) {
  if (completar) completar->setWidget(this);
  QTextEdit::focusInEvent(e);
}

void TextEdit::mouseReleaseEvent(QMouseEvent *e) {
  int selectedTextLength = textCursor().selectedText().length();
  if (selectedTextLength > 0) emit showRichTextToolBox();
...
void TextEdit::keyPressEvent(QKeyEvent *e) {
  ...
  if (completar && completar->popup()->isVisible()) {
    if (e->modifiers() == Qt::SHIFT && e->key() == Qt::Key_Delete) {
      QAbstractProxyModel *t_proxyModel =
          static_cast<QAbstractProxyModel *>(completar->popup()->model());
      QString currentText =
          completar->popup()->currentIndex().data(Qt::DisplayRole).toString();
      int ChildRow = completar->popup()->selectionModel()->currentIndex().row();
      int ParentRow =
          t_proxyModel->mapToSource(t_proxyModel->index(ChildRow, 0)).row();
      emit removeItemFromCompleter(currentText, ParentRow);
      ChildRow = (ChildRow == 0) ? 0 : ChildRow - 1;
      QItemSelectionModel *sm = static_cast<QItemSelectionModel *>(
          completar->popup()->selectionModel());
      sm->select(completar->completionModel()->index(ChildRow, 0),
  ...
  if (isALT && !isReadOnly()) {
    ...
    if (crsr.selectedText().length() > 0 && completar != nullptr &&
        !completar->popup()->isVisible())
      showRichTextToolBox();
    isSelectionStyled = false;
    return;
  }
  ...
  if (completar == nullptr ||
      !isShortcut)  // dont process the shortcut when we have a completer
    QTextEdit::keyPressEvent(e);

  const bool ctrlOrShift =
      e->modifiers() & (Qt::ControlModifier | Qt::ShiftModifier);
  if (completar == nullptr || (ctrlOrShift && e->text().isEmpty())) return;
  ...
  if (!isShortcut &&
      (isReadOnly() || hasModifier || e->text().isEmpty() ||
  ...
  if (completionPrefix != completar->completionPrefix()) {
    ...
    completar->popup()->setCurrentIndex(
        completar->completionModel()->index(0, 0));
  }
  QRect cr = cursorRect();
...
void TextEdit::keyReleaseEvent(QKeyEvent *e) {
  if (!completar->popup()->isVisible() && !rt->isVisible() &&
      textCursor().selectedText().length() > 0) {
    switch (e->key()) {
      case Qt::Key_Shift:
      case Qt::Key_Control:
