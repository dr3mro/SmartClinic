...
patientTable::patientTable(QWidget *parent)
    : QTableView(parent), proxy_model(new QSortFilterProxyModel(this)) {
  ...
  connect(&watcher, &QFutureWatcher<QStandardItemModel *>::finished, this,
          &patientTable::updatePatientsCompleted, Qt::QueuedConnection);
  connect(&initWatcher, &QFutureWatcher<QStandardItemModel *>::finished, this,
          &patientTable::setMyModel, Qt::QueuedConnection);
  connect(this, &patientTable::loadSelectedPatient, this,
          &patientTable::activated, Qt::QueuedConnection);
...
void patientTable::setFilterByName() {
  if (loadingIsFinished) proxy_model->setFilterKeyColumn(1);
  filterColumn = 1;
}

void patientTable::setFilterByID() {
  if (loadingIsFinished) proxy_model->setFilterKeyColumn(0);
  filterColumn = 0;
}

void patientTable::setFiletrByMobile() {
  if (loadingIsFinished) proxy_model->setFilterKeyColumn(2);
  filterColumn = 2;
}

void patientTable::updatePatients() {
  if (modelFuture.isRunning()) return;
...
bool patientTable::eventFilter(QObject *o, QEvent *e) {
  ...
  if ((o == this->viewport()) && (e->type() == QEvent::MouseButtonDblClick) &&
    ...
    if (!cell.isValid() || _ID == 0) return QObject::eventFilter(o, e);
...
void patientTable::loadPatient() {
  ...
  if (proxy_model->rowCount() > 0) emit loadSelectedPatient(currentIndex());
}

void patientTable::setPatientIcon(bool b, int row) {
  QModelIndex cell = selectionModel()->currentIndex().sibling(row, 1);
  if (b)
    proxy_model->setData(cell, QIcon(":/Graphics/ionicons/android-star.png"),
                         Qt::DecorationRole);
  else
    proxy_model->setData(selectionModel()->currentIndex().sibling(row, 1),
                         QIcon(), Qt::DecorationRole);
}

void patientTable::setID(int id) { ID = id; }

int patientTable::getID() { return ID; }
...
void patientTable::setConnection(QString conname) {
  ...
  connect(this, &patientTable::modelLoadingFinished, this,
          &patientTable::tweaksAfterModelLoadingIsFinished,
          Qt::QueuedConnection);
#if QT_VERSION >= 0x060000
