...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      ...
      sqlbase(
          new sqlBase(this, QString("qt_sql_base_MainWin_connection"), true)),
      sqlextra(
          new sqlExtra(this, QString("qt_sql_extra_MainWin_connection"), true)),
      ...
      assistant(
          new mAssistant(std::ref(labelMsg), this, std::ref(pEditingMode))),
      ...
      QuitShortcut(new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_Q), this,
                                 SLOT(close()))),
      settingsShortcut(new QShortcut(QKeySequence(Qt::Key_F9), this,
                                     SLOT(show_settings_win()))),
      menuShortcut(new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_M), this)),
      remoteAssistShortcut(new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_F1),
                                         this, SLOT(showRemoteAssistWin()))),
...
void MainWindow::boot() {
  ...
  QAction *WAaction =
      ui->patientMobile->addAction(QIcon(":/ionicons/ionicons/whatsapp.png"),
  ...
  connect(this, SIGNAL(clearDrugLineSignal()), ui->drugLine, SLOT(clear()),
          Qt::QueuedConnection);
  connect(ui->buttonAddDrug, SIGNAL(clicked()), this, SLOT(addDrug()),
          Qt::QueuedConnection);
  connect(ui->drugLine, SIGNAL(returnPressed()), this, SLOT(addDrug()),
          Qt::QueuedConnection);
  connect(ui->buttonAddAllergy, SIGNAL(clicked()), this, SLOT(addAllergy()),
          Qt::QueuedConnection);
  connect(ui->allergyLine, SIGNAL(returnPressed()), this, SLOT(addAllergy()),
          Qt::QueuedConnection);
  connect(ui->buttonAddOP, SIGNAL(clicked()), this, SLOT(AddOperation()),
          Qt::QueuedConnection);
  connect(ui->opAdLine, SIGNAL(returnPressed()), this, SLOT(AddOperation()),
          Qt::QueuedConnection);
  connect(ui->allergyList, SIGNAL(enableItemRemoveButton(bool)),
          ui->buttonRemoveAllergy, SLOT(setEnabled(bool)));
  connect(ui->opAdmissionsList, SIGNAL(enableItemRemoveButton(bool)),
          ui->buttonRemoveOP, SLOT(setEnabled(bool)));
  connect(this, SIGNAL(clearPatientName()), ui->patientName, SLOT(clear()),
          Qt::QueuedConnection);
  connect(ui->pDrugsTable, SIGNAL(clicked(QModelIndex)), this,
          SLOT(enableRemoveDrugButton()));
  connect(ui->pDrugsTable, SIGNAL(popUpMessage(QString, QString)), this,
          SLOT(popUpMessage(QString, QString)));
  connect(sqlbase, SIGNAL(showMessage(QString, QString)), this,
          SLOT(popUpMessage(QString, QString)));
  connect(sqlbase, SIGNAL(reloadPatientList()), ui->searchWidgetx,
          SIGNAL(updatePatients()));
  connect(ui->searchWidgetx, SIGNAL(loadPatient(int)), this,
          SLOT(on_PatientSelected(int)));
  connect(this, SIGNAL(toggleTrayMenuActions(bool)), trayIcon,
          SLOT(toggleActions(bool)));
  connect(ui->patientnumber, SIGNAL(enableGoHigher(bool)), ui->goHigherID,
          SLOT(setEnabled(bool)));
  connect(ui->patientnumber, SIGNAL(enableGoLower(bool)), ui->goLowerID,
          SLOT(setEnabled(bool)));
  connect(ui->pDrugsTable, SIGNAL(reloadDrugsLineAutoComplete()), ui->drugLine,
          SLOT(mUpdateCompleter()));
  connect(calWidget, SIGNAL(clicked(const QDate &)), this,
          SLOT(on_birthDateClicked(const QDate &)));
  connect(gSettings, SIGNAL(reloadMainWindow()), this,
          SLOT(reloadMainWindow()));
  connect(ui->pDrugsTable, SIGNAL(saveDrugs()), this, SLOT(saveDrugs()));
  connect(ui->pDrugsTable, SIGNAL(drugsHasBeenAltered(bool)), this,
          SLOT(toggleDrugsAlteredStatus(bool)));
  connect(gSettings, SIGNAL(reloadTheme()), this, SLOT(reloadTheme()));
  connect(&themeFutureWatcher, SIGNAL(finished()), this,
          SLOT(loadThemeWatcher()));
  connect(add2CompleterWorker, SIGNAL(finished()), this,
          SIGNAL(reloadCompleter()));
  connect(assistant, SIGNAL(loadThisPatient(int)), this,
          SLOT(loadThisPatient(int)));
  connect(assistant, SIGNAL(toggleEditMode(bool)), this,
          SLOT(toggleEditMODE(bool)));
  connect(&clickTimer, &QTimer::timeout, &clickTimer, &QTimer::stop);
  connect(&netserver, &NetServer::loadVisitors, rAssistant,
          &remoteAssist::loadVisitors);
  connect(rAssistant, &remoteAssist::search, ui->searchWidgetx,
          &searchWidget::setSearchString);
  connect(rAssistant, &remoteAssist::createVisitor, this,
          &MainWindow::newPatientWithData);
  connect(drugindex, &drugIndex::resetDrugsAutoComplete, sqlextra,
          &sqlExtra::resetDrugsTable);
  if (!settings.isDeviceActivated()) mship->run();

  QTimer::singleShot(0, this, &MainWindow::createVisitBox);

  connect(gSettings, SIGNAL(newMessage(QString, QString)), this,
          SLOT(popUpMessage(QString, QString)));
  connect(gSettings, SIGNAL(refreshCompleter()), this,
          SIGNAL(reloadCompleter()));
  ...
  if (settings.isUpdateNotify()) {
    ...
    connect(microupdater, SIGNAL(deleteMe()), microupdater,
            SLOT(deleteLater()));
  }
...
void MainWindow::setCompleters() { emit loadCompleters(); }
...
void MainWindow::tweakui() {
  ...
  ui->buttonSuricalNotes->setVisible(
      (user_speciality > dataHelper::Speciality::Paediatrics &&
...
void MainWindow::fillPatient(const sqlBase::Patient &_patient) {
  ...
  ui->DateTimeEdit->setDateTime(
      datetime.fromString(_patient.dateTime, "yyyy-MM-dd hh:mm:ss"));
  ...
  if (pEditingMode || _patient.name.contains(QString("#%1").arg(ID)) ||
      _patient.name.length() < 6) {
  ...
  if (settings.isInLinePatientList()) ui->searchWidgetx->selectRow(ID - 1);
...
void MainWindow::grabPatient() {
  ...
  patient.dateTime =
      locale.toString(ui->DateTimeEdit->dateTime(), "yyyy-MM-dd hh:mm:ss");
  ...
  patient.cigarretsPerDay = (ui->spinBoxCigarettesPerDay->isVisible())
                                ? ui->spinBoxCigarettesPerDay->text()
                                : "0";
...
void MainWindow::on_ButtonSaveEdit_clicked() {
  ...
  if (pEditingMode) {
    ...
    bool reloadInlinePatientList =
        (settings.isInLinePatientList() && personalHistoryEdited);
    pEditingMode = !save();

    if (!pEditingMode) {
      if (personalHistoryEdited || historyExtraEdited) addToMyCompleters();
      if (reloadInlinePatientList) ui->searchWidgetx->updatePatientList();
      drugsAltered = false;
    ...
    if (sqlbase->isPatientExists(patient.ID))
      selected_date = QDate::fromJulianDay(
          sqlbase->genericQuery(patient.ID, "Age", "patients").toInt());
  }
  toggleEditMODE(pEditingMode);
}
...
bool MainWindow::save() {
  ...
  if (appLocked() && ID > regMyApp.eval_max()) return false;
  ...
  if (pEditingMode && alwaysSave)
    msg =
        "<b><p1>Unable to Save that patient because patient's name is too "
        "short to be valid.</p1><p2>Save manually or leave EDIT MODE <AUTOSAVE "
        "ENABLED></p2></b>";
  else
  ...
  if (sqlbase->haveSameNameWithDifferentID(patient.ID, patient.name)) {
    if (pEditingMode && alwaysSave)
      msg =
          "<b><p>Unable to save that patient because another patient with the "
          "same exact name already Exist.<p>change name manually or leave EDIT "
          "MODE <AUTOSAVE ENABLED></p></b>";
    else
      msg =
          "<b>Another record with the same Name already Exist, Can't save this "
          "patients Name!";
  ...
  if (patient.name.contains(QString("#%1").arg(ID))) {
    ...
    msgbox->information(this, "Message",
                        "Sorry, you are not allowed to add patient "
                        "<b>number</b> to patient's <b>Name</b>");
    removeBlurEffect();
    return false;
  }
  ...
  if ((patient.name != "") && (patient.name.length() >= 6)) {
    bool s_ =
        sqlbase->savePatientData(patient, ui->pDrugsTable->getDrugsModel(),
...
void MainWindow::on_buttonNewCancel_clicked() {
  ...
  if (pEditingMode) {
    ...
    if (historyEdited)
      warningMessage =
          "This Will <b>Cancel</b> any changes you made to the current patient "
          "record, Are you Sure?"
          "<center> <i>This Can't be undone!</i></center>";
    else if (personalEmpty)
      warningMessage =
          "This Patient is not saved yet <b>Are you sure that you want to "
          "cancel without saving</b>\n"
          "<center> <i>This Can't be undone!</i></center>";
    if (showWarning) {
      applyBlurEffect();
      reply = msgbox->question(this, "Message", warningMessage,
  ...
  if (indexLength == 1) ui->patientnumber->display(1);
}
void MainWindow::clear() {
  ...
  ui->patientGender->setCurrentIndex(
      (user_speciality == dataHelper::Speciality::ObGyn) ? 1 : 0);
  ...
  //    ui->patientMobile->setStyleSheet("QLineEdit{background-color: white;
  //    color:black;}"); ui->patientMobile->setToolTip("");

  setAgeCalButtonToolTip();
}

void MainWindow::on_ButtonDelete_clicked() {
  if (clickTimer.isActive() || appLocked()) return;
  ...
  if (settings.isInLinePatientList()) ui->searchWidgetx->updatePatientList();
...
void MainWindow::on_ButtonVisit_clicked() {
  ...
  patientBasicDetails.sex = patient.gender == "Male" ? mSettings::mSex::male
                                                     : mSettings::mSex::female;
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  if (!appLocked() && pEditingMode &&
      ((editing &&
        (personalHistoryEditChecker() || isHistoryEditExtraChecker())) ||
       (!editing && (personalHistoryEmptyChecker() &&
                     ui->patientName->text().length() >= 6)))) {
    applyBlurEffect();
    reply = msgbox->question(
        this, "Message", "Do you want to save any unsaved data?",
...
void MainWindow::on_birthDateClicked(const QDate &birthDate) {
  ...
  setAgeCalButtonToolTip();
  ui->spinBoxAge->setToolTip(
      dataHelper::daysToAge(static_cast<int>(birthDate.toJulianDay())));
}
...
void MainWindow::on_patientName_returnPressed() {
  ...
  if (name.length() < 6) {
    applyBlurEffect();
    msgbox->information(this, "Message",
                        "<b>Patient's Name is too short, Please Enter a Valid "
                        "Name for your Patinet</b>");
    ui->patientName->setFocus(Qt::OtherFocusReason);
    removeBlurEffect();
    return;
  }
  ...
  if (sqlbase->isPatientNameUsed(name)) {
    ...
    reply = msgbox->question(this, "Message",
                             "The Patient Name you have Entered Already "
                             "Exists,do you want to Load This Patient?",
                             QMessageBox::Yes, QMessageBox::No);
    removeBlurEffect();
    if (reply == QMessageBox::Yes) {
...
void MainWindow::show_settings_win() {
  ...
  if (gSettings->isSettingsModified) updateSettings();
...
void MainWindow::show_gen_win() {
  ...
  connect(&generator, SIGNAL(updatePList()), ui->searchWidgetx,
          SLOT(updatePatientList()));
  connect(&generator, SIGNAL(loadPatient(int)), this,
          SLOT(loadThisPatient(int)));
  connect(&generator, SIGNAL(toggleEditMode(bool)), this,
          SLOT(toggleEditMODE(bool)));
  applyBlurEffect();
  generator.exec();
  removeBlurEffect();
...
void MainWindow::on_buttonRemoveAllergy_clicked() {
  ...
  if (ui->allergyList->count() == 0) ui->buttonRemoveAllergy->setEnabled(false);
  allergiesAltered = true;
}
...
void MainWindow::showRemoteAssistWin() { rAssistant->exec(); }

void MainWindow::on_buttonRemoveOP_clicked() {
  delete ui->opAdmissionsList->currentItem();
  ui->opAdLine->clear();
  if (ui->opAdmissionsList->count() == 0) ui->buttonRemoveOP->setEnabled(false);
  opAdAltered = true;
}

void MainWindow::keyPressEvent(QKeyEvent *e) {
  if (ID == indexLength) ID = indexLength - 1;
  ...
  else if (e->key() == Qt::Key_PageUp && e->modifiers() == Qt::SHIFT &&
           !pEditingMode) {
    if (ID != indexLength - 1) {
      ID = indexLength - 1;
      loadThisPatient(ID);
    }
  } else if (e->key() == Qt::Key_PageDown && e->modifiers() == Qt::SHIFT &&
             !pEditingMode) {
    if (ID != 1) {
      ID = 1;
      loadThisPatient(ID);
...
void MainWindow::on_buttonPatientList_clicked() {
  if (settings.isInLinePatientList()) {
    ...
    // connect ( patientlist,SIGNAL ( toggleEditMode(bool)
    // ),this,SLOT(toggleEditMODE(bool)) );
    applyBlurEffect();
    patientlist->exec();
    removeBlurEffect();
    delete patientlist;
...
void MainWindow::loadThisPatient(int _ID) {
  ...
  if (_xID_ != _ID)
    popUpMessage("Message",
                 QString("Patient ID (%1) is Loaded Successfully.")
                     .arg(QString::number(_ID).rightJustified(5, '0')));
}
...
void MainWindow::reloadMainWindow() {
  if (!pEditingMode) loadThisPatient(patient.ID);
  ui->searchWidgetx->updatePatientList();
}

void MainWindow::on_ButtonRefresh_clicked() {
  int reply = msgbox->question(
      this, "Confirm patient record reloading",
...
//     if(true){
//         popUpMessage("Information","Busy sending feedback please try again
//         later");
//     }else{
//         deleteLockFile();
//         trayIcon->hide();
...
void MainWindow::registeration() {
  ...
  if (reg_status) {
    ...
    if (eval_copy) {
      ...
      popUpMessage("Message",
                   QString("This Application is in Evaluation Period! The App "
                           "will limit you to %1 patient files only!")
                       .arg(regMyApp.eval_max()));

    } else {
      popUpMessage("Message",
                   "This Application is NOT registered , please contact Dr.Amr "
                   "Nasr Osman <dr3mro@gmail.com>, Thank you!");
      ui->ButtonSaveEdit->setEnabled(eval_copy);
    }
  }
}
...
bool MainWindow::appLocked() {
  ...
  if (eval_copy &&
      (indexLength > regMyApp.eval_max() ||
       QDate::fromString(QString(BUILDDATE), "MMM d yyyy").addDays(14) <
           QDate::currentDate())) {
...
void MainWindow::toggleEditMODE(bool pEditMODE) {
  ...
  ui->ButtonRefresh->setEnabled((sqlbase->isPatientExists(ID)) ? pEditMODE
                                                               : false);
  ui->ButtonAgeCal->setEnabled(pEditMODE);

  ui->buttonSuricalNotes->setEnabled(
      !pEditMODE && !(patient.name.contains(QString("#%1").arg(ID)) ||
  ...
  ui->buttonAddAllergy->setEnabled(
      (ui->allergyLine->text().length() > 0) ? pEditMODE : false);
  ui->buttonRemoveAllergy->setEnabled(
      (ui->allergyList->selectedItems().count() == 0) ? false : pEditMODE);
  ui->buttonAddDrug->setEnabled(
      (ui->drugLine->text().length() == 0) ? false : pEditMODE);
  ui->buttonRemoveDrug->setEnabled(
      (ui->pDrugsTable->selectionModel()->selection().indexes().count() == 0)
          ? false
          : pEditMODE);
  ui->buttonAddOP->setEnabled((ui->opAdLine->text().length() == 0) ? false
                                                                   : pEditMODE);
  ui->buttonRemoveOP->setEnabled(
      (ui->opAdmissionsList->selectedItems().count() == 0) ? false : pEditMODE);
  // ui->buttonPatientList->setEnabled((
  // sqlbase->isPatientExists(ui->patientnumber->value()))?!pEditMODE:pEditMODE);
  ui->drugLine->clear();
  ui->allergyLine->clear();
  ui->opAdLine->clear();
...
int MainWindow::getCenterPos() {
  QRect desktop = qApp->primaryScreen()->geometry();
...
void MainWindow::on_PatientSelected(int _id) {
  if (pEditingMode) {
    ...
    if (ID == _xID_) {
      popUpMessage("Message",
                   QString("Patient ID (%1) is already loaded.")
                       .arg(QString::number(ID).rightJustified(5, '0')));
      return;
    } else {
      ui->searchWidgetx->clearFilter();
      emit loadThisPatient(ID);
...
void MainWindow::toggleWindow(QSystemTrayIcon::ActivationReason reason) {
  if (trayIcon->childWindowsActive()) {
    ...
    this->hide();
    popUpMessage("Message", QString("%1 %2 is now Minimized to tray!")
                                .arg(APPNAME)
                                .arg(APPVERSION));
  }
}
...
void MainWindow::showHelloPirate() {
  QMessageBox::critical(nullptr, "Hello Pirate!",
                        "This device has been blocked for Piracy!");
  exit(0);
}
...
void MainWindow::show_expand_win() {
  ...
  connect(&dexpand, SIGNAL(reloadDrugsCompleter()), ui->drugLine,
          SIGNAL(updateDrugsCompleter()));
  connect(&dexpand, SIGNAL(reloadDrugsCompleter()), visitsbox,
          SIGNAL(updateDrugsCompleter()));
  connect(&dexpand, SIGNAL(newMessage(QString, QString)), this,
          SLOT(popUpMessage(QString, QString)));
  applyBlurEffect();
  dexpand.exec();
...
void MainWindow::exportPatientList() {
  ...
  QString savePath = fileDialog.getSaveFileName(
      this, "Select Export File", desktop,
      "Microsoft Excel (*.xls);;XML files (*.xml)", nullptr);
  QFileInfo info(savePath);

  if (info.baseName().isEmpty()) return;
  ...
  if (!file.open(QFile::WriteOnly)) {
    QMessageBox::information(nullptr, "Message",
                             "Export Failed,I don't have permission to create "
...
void MainWindow::addSurgicalNotes(int _ID, QString surgeryID, int julianDate,
...
void MainWindow::show_listEdit_win() {
  ...
  connect(&listeditor, SIGNAL(reloadCompleter()), this,
          SIGNAL(reloadCompleter()));
  applyBlurEffect();
  listeditor.exec();
  removeBlurEffect();
...
void MainWindow::show_favouritesDrugs_win() {
  ...
  connect(favouritesEditor, SIGNAL(reloadCompleter()), this,
          SIGNAL(reloadCompleter()));
  applyBlurEffect();
...
void MainWindow::passwordPopup() {
  if (sqlbase->isPasswordSet()) {
    ...
    connect(upass, SIGNAL(newMessage(QString, QString)), this,
            SLOT(popUpMessage(QString, QString)));
    upass->exec();
    delete upass;
  }
...
void MainWindow::checkDatabaseCompatibility() {
  ...
  if (currentDbVersion < static_cast<double>(minimumDatabaseVersion)) {
    QMessageBox::information(nullptr, "Message",
                             QString("<b>%1</b> Detected that your current "
                                     "database version [%2] is not supported.\n"
...
void MainWindow::toggleBlur(bool b) { blureffect->setEnabled(!b); }
...
void MainWindow::showMergedlg() {
  ...
  emit toggleTrayMenuActions(false);
  MergeDlg mDlg;
  connect(&mDlg, SIGNAL(reloadPatientsList()), ui->searchWidgetx,
          SLOT(updatePatientList()));
  connect(&mDlg, SIGNAL(loadThisPatient(int)), this,
          SLOT(loadThisPatient(int)));
  connect(&mDlg, SIGNAL(toggleEditMode(bool)), this,
          SLOT(toggleEditMODE(bool)));
...
void MainWindow::on_settingButton_clicked() { show_settings_win(); }
...
void MainWindow::appendCheckBoxTextToPastHx(QString text) {
  ...
  if (checkBox->isChecked())
    ui->PastHx->append(
        QString("<font color=red> <b>%1:</b></font> </br>").arg(text));
  else
...
void MainWindow::on_buttonSuricalNotes_clicked() {
  ...
  connect(&surgNotes, SIGNAL(addsNoteSignal(QString, int, QString, QString)),
          this, SLOT(addsNoteSlot(QString, int, QString, QString)));
  connect(&surgNotes, SIGNAL(savesNoteSignal(QString, int, QString, QString)),
          this, SLOT(savesNoteSlot(QString, int, QString, QString)));
  connect(&surgNotes, SIGNAL(deleteNote(QString)), this,
          SLOT(deleteNoteSlot(QString)));
  connect(this, SIGNAL(reloadSurgeryNotes(int)), &surgNotes,
          SLOT(reloadNotes(int)));
  applyBlurEffect();
  surgNotes.exec();
  removeBlurEffect();
}

void MainWindow::addsNoteSlot(QString surgeryID, int julianDate, QString opName,
                              QString opReport) {
...
void MainWindow::savesNoteSlot(QString surgeryID, int julianDate,
                               QString opName, QString opReport) {
  if (!sqlbase->updateSurgicalNote(ID, surgeryID, julianDate, opName,
                                   opReport)) {
...
void MainWindow::createVisitBox() {
  visitsbox = new visitsBox(this);
  connect(visitsbox, SIGNAL(newMessage(QString, QString)), this,
          SLOT(popUpMessage(QString, QString)));
  connect(visitsbox, SIGNAL(reloadMWCompleter()), this,
          SIGNAL(reloadCompleter()));
  connect(visitsbox, SIGNAL(syncDrugs(QList<QStringList>, bool)),
          ui->pDrugsTable, SLOT(DrugSyncWithVisit(QList<QStringList>, bool)));
  connect(visitsbox, SIGNAL(toggleBlur(bool)), this, SLOT(toggleBlur(bool)),
          Qt::QueuedConnection);
  connect(visitsbox, SIGNAL(syncLMPDateWithPatients(QDate)), ui->ObstWidget,
          SLOT(syncLMPDateWithVisit(QDate)));
  connect(visitsbox, SIGNAL(insertDrug2Patient(QString, QString)), this,
          SIGNAL(insertUniqueItem(QString, QString)));
  connect(this, SIGNAL(insertDrug2Visit(QString, QString)), visitsbox,
          SIGNAL(insertUniqueItem(QString, QString)));
  connect(visitsbox, SIGNAL(getDrugsModel()), ui->pDrugsTable,
          SLOT(getDrugsModel()));
}
...
void MainWindow::on_crashTool_clicked() { errViewer.showMaximized(); }

void MainWindow::showNotifywidget() {
  if (settings.isUseToast()) notifyWidget.showMaximized();
}
...
bool MainWindow::personalHistoryEditChecker() {
  ...
  //    mDebug() <<    "age" << (patient.age !=
  //    QString::number(selected_date.toJulianDay()) ) << patient.age <<
  //    QString::number(selected_date.toJulianDay()) <<
  //                   "name"<< (patient.name!=name) << patient.name << name <<
  //                   "res" << (patient.residence!=residence) <<
  //                   patient.residence << residence << "ocu" <<
  //                   (patient.occupation!=occupation) << patient.occupation <<
  //                   occupation << "mob" << (patient.mobile!=mobile) <<
  //                   patient.occupation << occupation << "gend"<<
  //                   (patient.gender!=gender) << patient.gender << gender <<
  //                   "ms"  << (patient.maritalStatus!=marital) <<
  //                   patient.maritalStatus << marital;

  return ((patient.age != QString::number(selected_date.toJulianDay())) ||
          (patient.name != name) || (patient.residence != residence) ||
          (patient.occupation != occupation) || (patient.mobile != mobile) ||
          ((user_speciality == dataHelper::Speciality::ObGyn)
               ? false
...
bool MainWindow::isHistoryEditExtraChecker() {
  ...
  bool ogModified = (speciality == dataHelper::Speciality::ObGyn)
                        ? ui->ObstWidget->isObGynModified()
                        : false;
  bool pnatalModified = (speciality == dataHelper::Speciality::Paediatrics)
                            ? ui->perinatal->isPnatalModified()
                            : false;
  bool devModified = (speciality == dataHelper::Speciality::Paediatrics)
                         ? ui->dev->isDevelopmentModified()
                         : false;

  QString smokingyears = (ui->spinBoxSmokingYears->value() != 0)
                             ? QString::number(QDate::currentDate().year() -
                                               ui->spinBoxSmokingYears->value())
                             : QString::number(0);
  ...
  return ((placeofbirth != patient.birthPlace) || (smoker != patient.smoker) ||
          (alcohol != patient.alcohol) ||
          (cigaretts != patient.cigarretsPerDay) ||
          (smokingyears != patient.smokingYears) || (abo != patient.ABO) ||
          pastHxModified || familyHxModified || notesModified || drugsAltered ||
          allergiesAltered || opAdAltered || conditionsModified || ogModified ||
          pnatalModified || devModified);
}
...
void MainWindow::on_patientMobile_textChanged(const QString &arg1) {
  ...
  if (ui->patientMobile->validator()->validate(string, pos) ==
      QValidator::Acceptable) {
    ui->patientMobile->setStyleSheet(style.normalStylesheet);
    ui->patientMobile->setToolTip("");
  } else if (ui->patientMobile->validator()->validate(string, pos) ==
             QValidator::Intermediate) {
    ui->patientMobile->setStyleSheet(style.warningStylesheet);
    ui->patientMobile->setToolTip(
        "One or more of phone numbers are slightly invalid.");
  } else if (ui->patientMobile->validator()->validate(string, pos) ==
             QValidator::Invalid) {
...
void MainWindow::toggleDrugsAlteredStatus(bool b) { drugsAltered = b; }
...
void MainWindow::loadThemeWatcher() {
  ...
  on_patientMobile_textChanged(
      ui->patientMobile->text());  // this to fix bug that not update color
}
...
void MainWindow::WAaction_clicked() {
  for (auto b : WA_widget->findChildren<m_ClickableLabel *>()) delete b;
    ...
    if (number.contains(QRegularExpression("[0][1][0-9]{9}"))) {
      ...
      connect(l, &m_ClickableLabel::leftMouseButtonClicked, [=]() {
        QDesktopServices::openUrl(
            QUrl(QString("https://api.whatsapp.com/send/"
                         "?phone=2%1&text&app_absent=0&lang=en")
                     .arg(l->getPhoneNumber())));
      });
