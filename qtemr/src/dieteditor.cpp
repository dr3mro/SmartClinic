...
DietEditor::DietEditor(QWidget *parent)
  ...
  ui->dietList->horizontalHeader()->setSectionResizeMode(0,
                                                         QHeaderView::Stretch);
...
void DietEditor::on_dietList_clicked(const QModelIndex &index) {
  ui->dietEdit->setHtml(
      dietModel->data(index.sibling(index.row(), 1)).toString());
...
void DietEditor::on_lineEdit_editingFinished() { ui->lineEdit->hide(); }

void DietEditor::on_buttonEdit_toggled(bool checked) { setEditable(checked); }
...
void DietEditor::on_buttonDelete_clicked() {
  int reply = QMessageBox::question(
      0, "Confirm", "Are you sure you want to delete the selected diet?",
...
void DietEditor::on_closeButton_clicked() { close(); }
