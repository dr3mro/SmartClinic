...
MergeDlg::MergeDlg(QWidget *parent) : mDialog(parent), ui(new Ui::MergeDlg) {
  ui->setupUi(this);
  sqlbase = new sqlBase(this, "qt_sql_base_merge", false);
  this->setStyleSheet(settings.themeMaker());
  setWindowFlags(windowFlags() | Qt::FramelessWindowHint);

  connect(ui->cbAccept, SIGNAL(toggled(bool)), ui->btnApply,
...
void MergeDlg::on_closeButton_clicked() { this->close(); }
...
void MergeDlg::on_btnApply_clicked() {
  ...
  if (state)
    copyPath(QString("data/media/%1").arg(fromID),
             QString("data/media/%1").arg(toID));
...
void MergeDlg::loadVisits() {
  int fromID = ui->fromID->value();
  QList<QPair<QString, QString> > visitList =
      sqlbase->getListVisitsType(fromID);
...
void MergeDlg::on_fromID_valueChanged(int) { loadVisits(); }
