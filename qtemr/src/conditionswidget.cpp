...
conditionsWidget::conditionsWidget(QWidget *parent) : QWidget(parent),
    sqlbase (new sqlBase(this,"conditionsWidget",false))
{
    setLayout(&layout);
...
void conditionsWidget::fillConditions(const int &_ID)
{
    ...
    foreach (pair, list)
    {
        ...
        if (cb)
            cb->setChecked(bool(pair.second));
    }
}
...
bool conditionsWidget::isConditionsModified()
{
    ...
    {
        if ((saved.at(i).second !=visible.at(i).second) )
            return true;
    }
    return false;
}

void conditionsWidget::setID(int id)
{
    ID = id;
}
