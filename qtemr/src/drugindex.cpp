...
drugIndex::drugIndex(QWidget *parent)
  ...
  ui->updated->setText(locale.toString(
      QDate::fromString(QString::number(sqlcore->getDrugsDatabaseVersion()),
                        "yyMMdd")
          .addYears(100),
      "dd-MMMM-yyyy"));
  ...
  connect(sqlcore, &sqlCore::drugsDatabaseUpdateFinished, this,
          &drugIndex::onDrugsDatabaseChange);
...
void drugIndex::load() {
  ...
  connect(ui->manufacturer, SIGNAL(currentTextChanged(QString)), this,
          SLOT(setManufacturer(QString)));
  connect(ui->form, SIGNAL(currentTextChanged(QString)), this,
          SLOT(setForm(QString)));
  connect(proxy_model, SIGNAL(rowsRemoved(QModelIndex, int, int)), this,
          SLOT(setResultsCount(QModelIndex, int, int)));
  connect(proxy_model, SIGNAL(rowsInserted(QModelIndex, int, int)), this,
          SLOT(setResultsCount(QModelIndex, int, int)));
  toggleResetButton();
  setFilters();
...
void drugIndex::on_updateButton_clicked() {
  ...
  if (!connected) {
    QMessageBox::information(
        nullptr, "Error",
        QString("%1 cannot access the internet, Please check your connection.")
            .arg(APPNAME));
    sock.abort();
    return;
  }
  ...
  int reply = QMessageBox::question(
      nullptr, "Upadating drugs database",
      "Are you sure that you want to update drugs index database?",
...
void drugIndex::onDrugsDatabaseChange(bool success) {
  if (success) {
    ...
    QMessageBox::information(
        nullptr, "Done",
        QString("%1 will now quit to reload the new changes of drugs index.")
            .arg(APPNAME));
    qApp->quit();
  } else {
    QMessageBox::information(
        nullptr, "Done",
        "Updating drugs index Database failed! Try Again later.");
  }
  message.hide();
}
