...
wm_investTooltipLoader::wm_investTooltipLoader(QObject *parent)
    : QObject(parent) {}
...
void wm_investTooltipLoader::setScale(const int &s) { scale = s; }
...
InvestModel *wm_investTooltipLoader::Work() {
  ...
  for (int r = 0; r < rows; r++) {
    ...
    if (killloopFlag) {
      ...
      if (model->hasIndex(r, 1)) model->item(r, 1)->setToolTip(html);
    }
  }
...
void wm_investTooltipLoader::killloop() { killloopFlag = true; }
