...
printDrugs::printDrugs(QWidget *parent)
  ...
  ui->printerProfile->setCurrentIndex(
      reg.value("selectedPrintingProfile").toInt());
  ...
  connect(ui->printerProfile, QOverload<int>::of(&QComboBox::activated), this,
          &printDrugs::printerProfile_activated, Qt::QueuedConnection);
  connect(ui->showInvs, &Switch::clicked, this, &printDrugs::showInvs_clicked,
          Qt::QueuedConnection);
  connect(ui->drugsMode, QOverload<int>::of(&QComboBox::activated), this,
          &printDrugs::drugsMode_activated, Qt::QueuedConnection);
  // connect(ui->drugsMode,QOverload<int>::of(&QComboBox::highlighted),this,&printDrugs::drugsMode_activated,Qt::QueuedConnection);
  connect(ui->bannerFontName, &QFontComboBox::textActivated, this,
          &printDrugs::bannerFontName_activated, Qt::QueuedConnection);
  connect(ui->bannerFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::bannerFontName_activated, Qt::QueuedConnection);
  connect(ui->bannerFontSize, &QComboBox::textActivated, this,
          &printDrugs::bannerFontSize_activated, Qt::QueuedConnection);
  connect(ui->bannerFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::bannerFontSize_activated, Qt::QueuedConnection);
  connect(ui->bannerFontBold, &QToolButton::clicked, this,
          &printDrugs::bannerFontBold_clicked, Qt::QueuedConnection);

  connect(ui->altBannerFontName, &QFontComboBox::textActivated, this,
          &printDrugs::altBannerFontName_activated, Qt::QueuedConnection);
  connect(ui->altBannerFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::altBannerFontName_activated, Qt::QueuedConnection);
  connect(ui->altBannerFontSize, &QComboBox::textActivated, this,
          &printDrugs::altBannerFontSize_activated, Qt::QueuedConnection);
  connect(ui->altBannerFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::altBannerFontSize_activated, Qt::QueuedConnection);
  connect(ui->altBannerFontBold, &QToolButton::clicked, this,
          &printDrugs::altBannerFontBold_clicked, Qt::QueuedConnection);

  connect(ui->roshettaFontName, &QFontComboBox::textActivated, this,
          &printDrugs::roshettaFontName_activated, Qt::QueuedConnection);
  connect(ui->roshettaFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::roshettaFontName_activated, Qt::QueuedConnection);
  connect(ui->roshettaFontSize, &QComboBox::textActivated, this,
          &printDrugs::roshettaFontSize_activated, Qt::QueuedConnection);
  connect(ui->roshettaFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::roshettaFontSize_activated, Qt::QueuedConnection);
  connect(ui->roshettaFontBold, &QToolButton::clicked, this,
          &printDrugs::roshettaFontBold_clicked, Qt::QueuedConnection);

  connect(ui->doseFontName, &QFontComboBox::textActivated, this,
          &printDrugs::doseFontName_activated, Qt::QueuedConnection);
  connect(ui->doseFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::doseFontName_activated, Qt::QueuedConnection);
  connect(ui->doseFontSize, &QComboBox::textActivated, this,
          &printDrugs::doseFontSize_activated, Qt::QueuedConnection);
  connect(ui->doseFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::doseFontSize_activated, Qt::QueuedConnection);
  connect(ui->doseFontBold, &QToolButton::clicked, this,
          &printDrugs::doseFontBold_clicked, Qt::QueuedConnection);

  connect(ui->requestsFontName, &QFontComboBox::textActivated, this,
          &printDrugs::requestsFontName_activated, Qt::QueuedConnection);
  connect(ui->requestsFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::requestsFontName_activated, Qt::QueuedConnection);
  connect(ui->requestsFontSize, &QComboBox::textActivated, this,
          &printDrugs::requestsFontSize_activated, Qt::QueuedConnection);
  connect(ui->requestsFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::requestsFontSize_activated, Qt::QueuedConnection);
  connect(ui->requestsFontBold, &QToolButton::clicked, this,
          &printDrugs::requestsFontBold_clicked, Qt::QueuedConnection);

  connect(ui->measurementsFontName, &QFontComboBox::textActivated, this,
          &printDrugs::measurementsFontName_activated, Qt::QueuedConnection);
  connect(ui->measurementsFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::measurementsFontName_activated, Qt::QueuedConnection);
  connect(ui->measurementsFontSize, &QComboBox::textActivated, this,
          &printDrugs::measurementsFontSize_activated, Qt::QueuedConnection);
  connect(ui->measurementsFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::measurementsFontSize_activated, Qt::QueuedConnection);
  connect(ui->measurementsFontBold, &QToolButton::clicked, this,
          &printDrugs::measurementsFontBold_clicked, Qt::QueuedConnection);

  connect(ui->signatureFontName, &QFontComboBox::textActivated, this,
          &printDrugs::signatureFontName_activated, Qt::QueuedConnection);
  connect(ui->signatureFontName, &QFontComboBox::textHighlighted, this,
          &printDrugs::signatureFontName_activated, Qt::QueuedConnection);
  connect(ui->signatureFontSize, &QComboBox::textActivated, this,
          &printDrugs::signatureFontSize_activated, Qt::QueuedConnection);
  connect(ui->signatureFontSize, &QComboBox::textHighlighted, this,
          &printDrugs::signatureFontSize_activated, Qt::QueuedConnection);
  connect(ui->signatureFontBold, &QToolButton::clicked, this,
          &printDrugs::signatureFontBold_clicked, Qt::QueuedConnection);

  connect(ui->headerHeightPercent, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &printDrugs::headerHeightPercent_valueChanged,
          Qt::QueuedConnection);
  connect(ui->bannerHeightPercent, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &printDrugs::bannerHeightPercent_valueChanged,
          Qt::QueuedConnection);
  connect(ui->footerHeightPercent, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &printDrugs::footerHeightPercent_valueChanged,
          Qt::QueuedConnection);

  connect(ui->Header, &PagesTextEdit::textChanged, this,
          &printDrugs::header_textChanged, Qt::QueuedConnection);
  connect(ui->Footer, &PagesTextEdit::textChanged, this,
          &printDrugs::footer_textChanged, Qt::QueuedConnection);
  connect(ui->bannerTemplate, &PagesTextEdit::textChanged, this,
          &printDrugs::bannerTemplate_textChanged, Qt::QueuedConnection);

  connect(ui->paperSizeId, &QComboBox::textActivated, this,
          &printDrugs::paperSizeId_activated, Qt::QueuedConnection);
  connect(ui->paperSizeId, &QComboBox::textHighlighted, this,
          &printDrugs::paperSizeId_activated, Qt::QueuedConnection);

  connect(ui->SignaturePrintedOn, &Switch::clicked, this,
          &printDrugs::SignaturePrintedOn_clicked, Qt::QueuedConnection);
  connect(ui->drugsInitDate, &Switch::clicked, this,
          &printDrugs::drugsInitDate_clicked, Qt::QueuedConnection);

  connect(ui->showHeader, &Switch::clicked, this,
          &printDrugs::showHeader_clicked, Qt::QueuedConnection);
  connect(ui->showFooter, &Switch::clicked, this,
          &printDrugs::showFooter_clicked, Qt::QueuedConnection);
  connect(ui->showLogo, &Switch::clicked, this, &printDrugs::showLogo_clicked,
          Qt::QueuedConnection);

  connect(ui->showDrugs, &Switch::clicked, this, &printDrugs::showDrugs_clicked,
          Qt::QueuedConnection);
  connect(ui->showMesurements, &Switch::clicked, this,
          &printDrugs::showMesurements_clicked, Qt::QueuedConnection);
  connect(ui->showDrugsTableOutline, &Switch::clicked, this,
          &printDrugs::showDrugsTableOutline_clicked, Qt::QueuedConnection);
  connect(ui->showDrugsTitle, &Switch::clicked, this,
          &printDrugs::showDrugsTitle_clicked, Qt::QueuedConnection);
  connect(ui->showBanner, &Switch::clicked, this,
          &printDrugs::showBanner_clicked, Qt::QueuedConnection);
  connect(ui->showOnlyNewlyModifiedAddedDrugs, &Switch::clicked, this,
          &printDrugs::showOnlyNewlyModifiedAddedDrugs_clicked,
          Qt::QueuedConnection);
  // connect(ui->showTradeNamesBold,&Switch::clicked,this,&printDrugs::showTradeNamesBold_clicked,Qt::QueuedConnection);
  connect(ui->showDoseNewLine, &Switch::clicked, this,
          &printDrugs::showDoseNewLine_clicked, Qt::QueuedConnection);
  connect(ui->preferArabic, &Switch::clicked, this,
          &printDrugs::preferArabic_clicked, Qt::QueuedConnection);
  ...
  connect(ui->enableFullPage, &Switch::clicked, this,
          &printDrugs::enableFullPage_clicked, Qt::QueuedConnection);

  connect(ui->pageMargin, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &printDrugs::pageMargin_valueChanged, Qt::QueuedConnection);
  connect(ui->logoSize, &QComboBox::textActivated, this,
          &printDrugs::logoSize_activated, Qt::QueuedConnection);
  connect(ui->logoSize, &QComboBox::textHighlighted, this,
          &printDrugs::logoSize_activated, Qt::QueuedConnection);

  connect(ui->ButtonRefresh, &QToolButton::clicked, this,
          &printDrugs::ButtonRefresh_clicked, Qt::QueuedConnection);
  connect(ui->lockUnlockButton, &QToolButton::toggled, this,
          &printDrugs::lockUnlockButton_toggled, Qt::QueuedConnection);
  ...
  connect(m_roshetta, &QTextDocument::modificationChanged, this,
          &printDrugs::roshettaEdited, Qt::QueuedConnection);

  connect(ui->BannerStyle, QOverload<int>::of(&QComboBox::activated), this,
          &printDrugs::bannerStyle_activated, Qt::QueuedConnection);
  connect(ui->resetBannerTemplate, &QPushButton::clicked, this,
          &printDrugs::resetBannerTemplateClicked);

  connect(ui->bannerTemplate, &TextEdit::fontFamilyChanged, this,
          &printDrugs::altBannerFontName_activated);
  connect(ui->bannerTemplate, &TextEdit::fontPointChanged, this,
          &printDrugs::altBannerFontSize_activated);
  connect(ui->bannerTemplate, &TextEdit::fontBoldChanged, this,
          &printDrugs::altBannerFontBold_clicked);

  connect(ui->enableBodyHeaderSeparator, &Switch::clicked, this,
          &printDrugs::enableBodyHeaderSeparator_clicked);
  connect(ui->compactMode, &Switch::clicked, this,
          &printDrugs::compactMode_clicked);
  connect(ui->clearDuplicateDrugs, &Switch::clicked, this,
          &printDrugs::clearDuplicateDrugs_clicked);
  connect(ui->preferRTF, &Switch::clicked, this,
          &printDrugs::preferRTF_clicked);

  connect(ui->printers, &QComboBox::currentIndexChanged, this,
          &printDrugs::printers_currentIndexChanged);
...
void printDrugs::showPrintDialog() {
  if (dlg->exec() != QDialog::Rejected) printDoc(printer, m_roshetta);
}
...
void printDrugs::showPrintPreviewDialog() {
  ...
  previewDialog.setWindowState(previewDialog.windowState() |
                               Qt::WindowMaximized);
  connect(&previewDialog, SIGNAL(paintRequested(QPrinter *)), this,
          SLOT(makePrintPreview(QPrinter *)));
  previewDialog.exec();
}
...
void printDrugs::savePrintSettings() {
  if (!(lSettings == pSettings))  // check if settings have been changed or not
    settings.savePrintSettings(
        grabPrintSettings(),
        selectedPrintingProfile);  // so dont cause useless writes to disk.
}

mSettings::prescriptionPrintSettings printDrugs::loadPrintSettings() {
  mSettings::prescriptionPrintSettings printSettings =
      settings.getPrintSettings(selectedPrintingProfile);
  ...
  ui->bannerFontSize->setCurrentText(
      QString::number(printSettings.bannerFont.fontSize));
  ui->bannerFontBold->setChecked(printSettings.bannerFont.fontBold);

  ui->altBannerFontName->setCurrentFont(
      QFont(printSettings.altBannerFont.fontName));
  ui->altBannerFontSize->setCurrentText(
      QString::number(printSettings.altBannerFont.fontSize));
  ui->altBannerFontBold->setChecked(printSettings.altBannerFont.fontBold);

  ui->roshettaFontName->setCurrentFont(
      QFont(printSettings.roshettaFont.fontName));
  ui->roshettaFontSize->setCurrentText(
      QString::number(printSettings.roshettaFont.fontSize));
  ...
  ui->doseFontSize->setCurrentText(
      QString::number(printSettings.doseFont.fontSize));
  ui->doseFontBold->setChecked(printSettings.doseFont.fontBold);

  ui->requestsFontName->setCurrentFont(
      QFont(printSettings.requestsFont.fontName));
  ui->requestsFontSize->setCurrentText(
      QString::number(printSettings.requestsFont.fontSize));
  ui->requestsFontBold->setChecked(printSettings.requestsFont.fontBold);

  ui->measurementsFontName->setCurrentFont(
      QFont(printSettings.measurementsFont.fontName));
  ui->measurementsFontSize->setCurrentText(
      QString::number(printSettings.measurementsFont.fontSize));
  ui->measurementsFontBold->setChecked(printSettings.measurementsFont.fontBold);

  ui->signatureFontName->setCurrentFont(
      QFont(printSettings.signatureFont.fontName));
  ui->signatureFontSize->setCurrentText(
      QString::number(printSettings.signatureFont.fontSize));
  ...
  ui->showOnlyNewlyModifiedAddedDrugs->setChecked(
      printSettings.showOnlyNewlyModifiedAddedDrugs);
  ...
  ui->enableBodyHeaderSeparator->setChecked(
      printSettings.enableBodyHeaderSeparator);
  ...
  ui->resetButtonAndLabel->setVisible(
      (bool)printSettings.prescriptionBannerStyle);

  ui->bannerFontGroup->setVisible(printSettings.prescriptionBannerStyle ==
                                  mSettings::bannerStyle::belowHeader);
  ui->altBannerFontGroup->setVisible(printSettings.prescriptionBannerStyle ==
                                         mSettings::bannerStyle::replaceLogo &&
                                     !printSettings.preferRTFBanner);
  ui->preferRTFGroup->setVisible(printSettings.prescriptionBannerStyle ==
                                 mSettings::bannerStyle::replaceLogo);

  ui->enableBodyHeaderSeparator->setEnabled(
      (bool)printSettings.prescriptionBannerStyle);
...
mSettings::prescriptionPrintSettings printDrugs::grabPrintSettings() {
  ...
  printSettings.drugsPrintMode =
      (mSettings::drugsPrintMode)ui->drugsMode->currentIndex();
  ...
  printSettings.prescriptionBannerStyle =
      static_cast<mSettings::bannerStyle>(ui->BannerStyle->currentIndex());
  ...
  printSettings.altBannerFont.fontSize =
      ui->altBannerFontSize->currentText().toInt();
  ...
  ;
  printSettings.roshettaFont.fontSize =
      ui->roshettaFontSize->currentText().toInt();
  ...
  ;
  printSettings.requestsFont.fontSize =
      ui->requestsFontSize->currentText().toInt();
  printSettings.requestsFont.fontBold = ui->requestsFontBold->isChecked();

  printSettings.measurementsFont.fontName =
      ui->measurementsFontName->currentText();
  ;
  printSettings.measurementsFont.fontSize =
      ui->measurementsFontSize->currentText().toInt();
  printSettings.measurementsFont.fontBold =
      ui->measurementsFontBold->isChecked();

  printSettings.signatureFont.fontName = ui->signatureFontName->currentText();
  ;
  printSettings.signatureFont.fontSize =
      ui->signatureFontSize->currentText().toInt();
  ...
  printSettings.showOnlyNewlyModifiedAddedDrugs =
      ui->showOnlyNewlyModifiedAddedDrugs->isChecked();
  ...
  // printSettings.showHorizontalLineBelowHeader =
  // ui->showHorizontalLineBelowHeader->isChecked();
  printSettings.enableFullPage = ui->enableFullPage->isChecked();
  printSettings.enableBodyHeaderSeparator =
      ui->enableBodyHeaderSeparator->isChecked();
...
void printDrugs::saveRoshettaAutoComplete() {
  ...
  auto f =
      QtConcurrent::run(&wm_add2Completer::save2Dict, wm_add2completer, dict);
#else
  auto f =
...
void printDrugs::setupPrinter(QPrinter *p) {
  ...
  m_layout.setPageSize(pageSize,
                       QMarginsF(pSettings.pageMargin, pSettings.pageMargin,
                                 pSettings.pageMargin, pSettings.pageMargin));
  ...
  p->setDocName(QString("%1_%2_%3")
                    .arg(roshettaData.ID)
                    .arg(roshettaData.name)
                    .arg(roshettaData.visitDate.toString("yyyy-MM-dd")));
}
...
void printDrugs::printDoc(QPrinter *p, QTextDocument *_doc, bool isPreview) {
  ...
  if (roshettaMaker.getIsDrugsOutOfRange()) {
    int reply = QMessageBox::question(
        NULL, "warning",
        "Some drugs seems to be out of printing area, please double check your "
        "prescribed drugs before printing.",
  ...
  if (_doc->pageCount() > 1) {
    ...
    if (roshettaData.diet.printRequired) {
      ...
      emit message("Message",
                   QString("Print job is being sent to Printer (%1).")
                       .arg(p->printerName()));
      reply = QMessageBox::question(
          nullptr, "Attention",
          "Please press Ok when ready to print the selected Diet.",
      ...
      reply = QMessageBox::question(
          nullptr, "Warning",
          "Your print might be splitted over more than one page."
    ...
    _doc->print(p);
    emit message("Message", QString("Print job is being sent to Printer (%1).")
                                .arg(p->printerName()));
  }
  if (settings.alwaysClosePrintDlgAfterClick() && isVisible()) this->close();
}

void printDrugs::applyPageSizeParamaters() {
  ui->Roshetta->setPageFormat(
      PageMetrics::pageSizeIdFromString(pSettings.paperSizeId));
  ui->Roshetta->setPageMargins(
      QMarginsF(pSettings.pageMargin, pSettings.pageMargin,
                pSettings.pageMargin, pSettings.pageMargin));
}
...
void printDrugs::reload() {
  ...
  ui->warning->setText(roshettaMaker.getIsDrugsOutOfRange()
                           ? "Warning: There might be some drugs not visible"
                           : "");
  RoshettaEdited = false;
...
bool printDrugs::modificationsOK() {
  if (RoshettaEdited) {
    int reply = QMessageBox::question(
        nullptr, "Caution",
        "You have altered the current Roshetta, Press Ok to continue and lose "
        "any modifications you have done or press cancel.",
        QMessageBox::Ok, QMessageBox::Cancel);
...
void printDrugs::clearDuplicateDrugs_clicked(bool checked) {
  pSettings.clearDuplicateDrugs = checked;
  refreshView();
}

void printDrugs::bannerStyle_activated(int index) {
  pSettings.prescriptionBannerStyle =
      static_cast<mSettings::bannerStyle>(index);
  ...
  ui->bannerFontGroup->setVisible(pSettings.prescriptionBannerStyle ==
                                  mSettings::bannerStyle::belowHeader);
  ui->altBannerFontGroup->setVisible(pSettings.prescriptionBannerStyle ==
                                         mSettings::bannerStyle::replaceLogo &&
                                     !pSettings.preferRTFBanner);
  ui->preferRTFGroup->setVisible(pSettings.prescriptionBannerStyle ==
                                 mSettings::bannerStyle::replaceLogo);
...
void printDrugs::pageMargin_valueChanged(int arg1) {
  if (!ui->pageMargin->hasFocus()) return;
...
void printDrugs::lockUnlockButton_toggled(bool checked) {
  ...
  ui->lockUnlockButton->setIcon(
      QIcon(checked ? ":/Graphics/lock" : ":/Graphics/unlock"));
}
...
void printDrugs::setLogo() {
  QString newlogo = QFileDialog::getOpenFileName(
      this, tr("Open Image"),
...
void printDrugs::roshettaEdited(bool b) { RoshettaEdited = b; }
