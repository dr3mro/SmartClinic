...
w_obGyn::w_obGyn(QWidget *parent)
  ...
  connect(ui->checkBoxMenoPause, SIGNAL(toggled(bool)), this,
          SLOT(toggleLMPdateBox(bool)));
}
...
QString w_obGyn::getFPAL() { return ui->fpal->text(); }

void w_obGyn::toggleEditMode(bool pEditMODE) {
  ui->addSiblings->setEnabled(pEditMODE);
  ui->editSiblingButton->setEnabled(
      (ui->siblings->selectionModel()->currentIndex().row() >= 0) ? pEditMODE
                                                                  : false);
  ui->removeSiblings->setEnabled(
      (ui->siblings->selectionModel()->currentIndex().row() >= 0) ? pEditMODE
                                                                  : false);
...
void w_obGyn::setID(int id) { ID = id; }

QString w_obGyn::getHusbandHtml() { return ui->husband->toHtml(); }
...
void w_obGyn::toggleLMPdateBox(bool checked) { ui->LMP->setEnabled(!checked); }

void w_obGyn::on_addSiblings_clicked() {
  addSiblings addsiblings(ui->siblings->getSelectedRow(),
                          ui->siblings->getSelectedRowContent(), this, false);
  connect(&addsiblings, &addSiblings::addSibling, ui->siblings,
          &mSiblings::appendRow);
  addsiblings.exec();
}
...
void w_obGyn::on_editSiblingButton_clicked() {
  addSiblings addsiblings(ui->siblings->getSelectedRow(),
                          ui->siblings->getSelectedRowContent(), this, true);
  connect(&addsiblings, &addSiblings::editSibling, ui->siblings,
          &mSiblings::editRow);
  addsiblings.exec();
}
