...
mRequestViewer::mRequestViewer(QWidget *parent)
    : mDialog(parent), ui(new Ui::mRequestViewer) {
  ui->setupUi(this);
  ...
  connect(ui->graphicsView, SIGNAL(closeViewer()), this, SLOT(close()),
          Qt::QueuedConnection);
  connect(ui->graphicsView, SIGNAL(loadPreviousRequest()), this,
          SLOT(loadPreviousRequest()), Qt::QueuedConnection);
  connect(ui->graphicsView, SIGNAL(loadNextRequest()), this,
          SLOT(loadNextRequest()), Qt::QueuedConnection);
  connect(ui->graphicsView, SIGNAL(deleteRequest()), this,
          SLOT(deleteRequestMedia()));
  connect(ui->graphicsView, SIGNAL(saveRotation(const qreal &)), this,
          SLOT(saveRotation(const qreal &)));
  connect(ui->graphicsView, SIGNAL(openInWinPhotoViewer()), this,
          SLOT(openInWinPhotoViewer()));
}
...
void mRequestViewer::setRequests(const QVector<QVector<QString> > &vector,
                                 const int &i) {
  requests = vector;
  index = i;
...
void mRequestViewer::loadNextRequest() {
  ...
  if (index >= requestLength) index = requestLength - 1;
...
void mRequestViewer::deleteRequestMedia() {
  ...
  int reply = QMessageBox::question(
      nullptr, tr("Warning"),
      tr("Are you sure that you want to delete this request media file? "
  ...
  if (reply == QMessageBox::Yes) {
    ...
    close();
    if (b)
      QMessageBox::information(nullptr, tr("infromation"),
                               tr("Media file deleted successfuly."));
    else
      QMessageBox::information(nullptr, tr("infromation"),
                               tr("Media file deletion failed."));
  }
}
...
void mRequestViewer::saveRotation(const qreal &r) {
  ...
  int reply = QMessageBox::question(
      nullptr, tr("Warning"),
      tr("Are you sure that you want to save rotation to media file? "
         "This can not be undone."),
      QMessageBox::Yes, QMessageBox::No);
  if (reply == QMessageBox::Yes) {
    QTransform transform;
    transform.rotate(r);
    b = writeImage(request, readImage(request).transformed(
                                transform, Qt::SmoothTransformation));
    if (b)
      QMessageBox::information(nullptr, tr("infromation"),
                               tr("Media file saved successfuly."));
    else
      QMessageBox::information(nullptr, tr("infromation"),
                               tr("Media file save failed."));
  }
}
...
mRequestViewer::~mRequestViewer() { delete ui; }
