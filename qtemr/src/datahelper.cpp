...
QString dataHelper::julianToAge(int totalDays, dataHelper::AgeStyle style) {
  ...
  QString dateString = QString(_string).arg(
      QString::number(years), QString::number(months), QString::number(days));
  return dateString;
}
...
QString dataHelper::daysToAge(int totalDays) {
  ...
  QString dateString = QString("%1 Years %2 Months %3 Days")
                           .arg(QString::number(years), QString::number(months),
                                QString::number(days));
  return dateString;
}
...
//     visitTypes << "New Visit" << "Follow Up1" << "Follow Up2" << "Follow Up3"
//     << "Follow Up4" << "Free"; return visitTypes.at(x);
// }
...
void dataHelper::cleanExpanderHTML(QString &expander) {
  expander.remove(QRegularExpression(
      "style=\\\"([^\"]*)\\\"", QRegularExpression::CaseInsensitiveOption));
  expander.remove(
      QRegularExpression("\\n", QRegularExpression::CaseInsensitiveOption));
  expander.remove(QRegularExpression(
      "<p ><span >", QRegularExpression::CaseInsensitiveOption));
  expander.replace(QRegularExpression("</span></p>"), "<br>");
  expander.remove(QRegularExpression(
      "^.*<body >", QRegularExpression::CaseInsensitiveOption));
  expander.remove(QRegularExpression(
      "<br><body>", QRegularExpression::CaseInsensitiveOption));
  expander.remove(QRegularExpression(
      "<br></body></html>", QRegularExpression::CaseInsensitiveOption));
}
