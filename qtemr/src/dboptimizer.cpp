...
dbOptimizer::dbOptimizer(QWidget *parent)
  ...
  connect(&opt, SIGNAL(status(QString, bool)), this,
          SLOT(status(QString, bool)));
  connect(&opt, SIGNAL(toggleProgress(bool)), this,
          SLOT(toggleAnimation(bool)));
  connect(ui->sw_optimize, SIGNAL(toggled(bool)), ui->sw_shrinkMemory,
...
void dbOptimizer::doOptimize() {
...
#endif
  while (future1.isRunning()) qApp->processEvents();
  ...
  if (clean) {
    ...
    while (future2.isRunning()) qApp->processEvents();
...
bool dbOptimizer::saveDbOptions(const mSettings::dbOptions &options) {
  ...
  if (tweaks_changed) settings.saveDbOptions(options);
  ...
  if (!working) {
    ...
    if (s)
      QMessageBox::information(nullptr, "Alert!",
                               "Database options has been changed, please "
                               "restart application to avoid crashes.",
                               QMessageBox::Ok);
    e->accept();
  }
  while (working) {
...
void dbOptimizer::status(QString operation, bool state) {
  ...
  ui->status->append(
      QString("<b>%1</b> : %2")
          .arg(operation, state ? "<b style='color:green;'>OK</b>"
...
void dbOptimizer::show() { wm.centerOnScreen(this); }
