...
fileGrabber::fileGrabber(bool _save, QUrl _url, QObject *parent)
    : QObject(parent), url(_url), save(_save) {
...
QString fileGrabber::saveFileName(const QUrl &_url) {
  ...
  // mDebug() << basename;
  if (basename.isEmpty()) basename = "update/download.zip";
  return basename;
}
...
void fileGrabber::run() { QTimer::singleShot(0, this, SLOT(execute())); }
...
void fileGrabber::setURL(const QUrl &_url) { url = _url; }
...
void fileGrabber::execute() {
  if (save) openFile(saveFileName(url));
