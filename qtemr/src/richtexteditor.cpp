...
richTextEditor::richTextEditor(QWidget *parent)
  ...
  connect(ui->textColor, SIGNAL(leftButtonClicked()), cp1, SLOT(show()),
          Qt::QueuedConnection);
  connect(ui->textColor, SIGNAL(midButtonClicked(const QColor &)), this,
          SLOT(setForegroundColor(const QColor &)), Qt::QueuedConnection);
  connect(ui->backgroundColor, SIGNAL(leftButtonClicked()), cp2, SLOT(show()),
          Qt::QueuedConnection);
  connect(ui->backgroundColor, SIGNAL(midButtonClicked(const QColor &)), this,
          SLOT(setBackgroundColor(const QColor &)), Qt::QueuedConnection);

  connect(cp1, SIGNAL(newColor(const QColor &)), this,
          SLOT(setForegroundColor(const QColor &)), Qt::QueuedConnection);
  connect(cp1, SIGNAL(newColor(const QColor &)), ui->textColor,
          SLOT(setColor(const QColor &)), Qt::QueuedConnection);
  connect(cp2, SIGNAL(newColor(const QColor &)), this,
          SLOT(setBackgroundColor(const QColor &)), Qt::QueuedConnection);
  connect(cp2, SIGNAL(newColor(const QColor &)), ui->backgroundColor,
          SLOT(setColor(const QColor &)), Qt::QueuedConnection);

  connect(ui->fontComboBox, SIGNAL(currentFontChanged(QFont)), parent,
          SLOT(setFont(QFont)), Qt::QueuedConnection);
  connect(ui->fontComboBox, SIGNAL(textHighlighted(const QString &)), parent,
          SLOT(setFont(const QString &)), Qt::QueuedConnection);
  connect(ui->fontComboBox, SIGNAL(activated(int)), this, SLOT(reGainFocus()),
          Qt::QueuedConnection);
  connect(ui->point, SIGNAL(currentTextChanged(QString)), parent,
          SLOT(setPoint(QString)), Qt::QueuedConnection);
  connect(ui->point, SIGNAL(textHighlighted(QString)), parent,
          SLOT(setPoint(QString)), Qt::QueuedConnection);
  ...
  connect(timeout, SIGNAL(timeout()), timeout, SLOT(stop()),
          Qt::QueuedConnection);
  connect(ui->fontComboBox, SIGNAL(highlighted(int)), this, SLOT(noFadeOut()),
          Qt::QueuedConnection);
  connect(ui->point, SIGNAL(highlighted(int)), this, SLOT(noFadeOut()),
          Qt::QueuedConnection);
  ...
  connect(this, SIGNAL(redFlagHighLight()), parent, SLOT(redFlagHighLight()),
          Qt::QueuedConnection);
  connect(this, SIGNAL(greenFlagHighLight()), parent,
          SLOT(greenFlagHighLight()), Qt::QueuedConnection);
...
void richTextEditor::syncFontPoint(QString font, int point, bool bold,
                                   bool italic, bool underLine, bool strike,
                                   bool highlight, bool alignLeft,
                                   bool alignCenter, bool alignRight,
                                   bool upper) {
  if (font != "") {
...
void richTextEditor::reGainFocus() { this->setFocus(Qt::OtherFocusReason); }
...
bool richTextEditor::eventFilter(QObject *o, QEvent *e) {
  if (cp1->isVisible() || cp2->isVisible()) return QWidget::eventFilter(o, e);
