...
surgicalNoteEditor::surgicalNoteEditor(bool editMode, QString surgeryID,
                                       QString date, QString opName,
                                       QString opReport, QWidget *parent)
    : QDialog(parent), ui(new Ui::surgicalNoteEditor) {
  ui->setupUi(this);
  mEditMode = editMode;
  if (mEditMode) {
...
surgicalNoteEditor::~surgicalNoteEditor() { delete ui; }
...
void surgicalNoteEditor::on_Templates_activated(int index) {
  ...
  switch (index) {
    ...
    case 1:
      CS =
          "Caesarian section. Under spinal anesthesia, abdomen was opened "
          "by pfennesteil incision, sheath, muscles and parietal proton were "
          "opened. Visceral proton was opened and bladder was dissected "
          "downward."
...
void surgicalNoteEditor::on_buttonClose_clicked() { this->close(); }
