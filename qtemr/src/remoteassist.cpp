...
remoteAssist::remoteAssist(QWidget *parent)
    : QDialog(parent),
      ...
      sqlbase(new sqlBase(
          this, QString("qt_sql_base_remoteAssistant_connection"), false)),
      ui(new Ui::remoteAssist) {
  ui->setupUi(this);
  ui->visitorsTableView->setModel(model);
  connect(ui->searchByID, &QToolButton::clicked, this,
          &remoteAssist::searchByID);
  connect(ui->searchByName, &QToolButton::clicked, this,
          &remoteAssist::searchByName);
  connect(ui->searchByTel, &QToolButton::clicked, this,
          &remoteAssist::searchByTel);
  connect(ui->reload, &QToolButton::clicked, this, &remoteAssist::loadVisitors);
  connect(ui->visitorsTableView->selectionModel(),
          &QItemSelectionModel::currentRowChanged, this,
          &remoteAssist::showVisitor);
...
void remoteAssist::loadVisitors() {
  ...
  foreach (const QVariant &v, list) {
    ...
    QStandardItem *placeofbirth =
        new QStandardItem(map["placeofbirth"].toString());
...
void remoteAssist::showVisitor(const QModelIndex &current,
  ...
  bool nameIdMatch =
      sqlbase->haveSameNameWithSameID(visitor.ID.toInt(), visitor.name);
...
void remoteAssist::tweakTable() {
  ...
  ui->visitorsTableView->horizontalHeader()->setSectionResizeMode(
...
void remoteAssist::searchByID() { search(ui->_id->text()); }

void remoteAssist::searchByName() { search(ui->_name->text()); }

void remoteAssist::searchByTel() { search(ui->_tel->text()); }

void remoteAssist::on_visitorsTableView_doubleClicked(
