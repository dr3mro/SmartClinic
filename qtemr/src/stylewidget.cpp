...
StyleWidget::StyleWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::StyleWidget) {
  ui->setupUi(this);

  path = QString("%1\\themes").arg(dataIOhelper::getCurrentFolder());
  themeWatcher.setRootPath(path);

  connect(&themeWatcher, SIGNAL(rowsInserted(QModelIndex, int, int)), this,
          SLOT(themesListMaker(QModelIndex, int, int)));
  connect(&themeWatcher, SIGNAL(rowsRemoved(QModelIndex, int, int)), this,
          SLOT(themesListMaker(QModelIndex, int, int)));
...
void StyleWidget::putStyle(const mStyler::Style &style) {
  ...
  ui->scrollArea->setDisabled(
      styler.getPackagedThemes().contains(selected_theme));
}
...
QString StyleWidget::getSelectedTheme() { return selected_theme; }

StyleWidget::~StyleWidget() { delete ui; }

bool StyleWidget::save() {
  selected_theme = ui->Styles->currentText();

  if (styler.getPackagedThemes().contains(selected_theme) ||
      selected_theme.isEmpty())
    return false;
...
void StyleWidget::on_Styles_activated(int index) {
  if (index < 0 || index > ui->Styles->count() - 1) return;
...
void StyleWidget::on_btn_save_as_clicked() {
  bool Ok;
  QString Name;
  QStringList themes = QStringList() << styler.getNonPackagedThemes()
                                     << styler.getPackagedThemes();
...
void StyleWidget::on_btn_Delete_clicked() {
  int reply = QMessageBox::question(
      0, "Alert!",
      QString("Are you sure that you want to delete theme %1?")
          .arg(selected_theme),
