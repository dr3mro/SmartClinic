...
bool excelExport::createXLS(QString excelFilePath) {
  ...
  QString dsn =
      QString(
          "DRIVER={Microsoft Excel Driver (*.xls)};DSN=''; FIRSTROWHASNAMES=1; "
          "READONLY=FALSE;CREATE_DB=\"%1\";DBQ=%2")
          .arg(excelFilePath)
          .arg(excelFilePath);
  ...
  if (!db.open()) {
    ...
    mDebug() << db.lastError().text()
             << "Get-OdbcDriver on powershell as admin";
    return false;
  }
...
bool excelExport::addRow(QString ID, QString Name, QString Mobile) {
  ...
  QString prepare = QString(
      "INSERT INTO PATIENTS (ID,NAME,MOBILE)VALUES(:ID,:NAME,:MOBILE);");
