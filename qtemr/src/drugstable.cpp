...
drugsTable::drugsTable(QWidget *parent) : zTableView(parent) {
  ...
  QAction *okDose = le_setDose->addAction(
      QIcon(":/ionicons/check"), QLineEdit::ActionPosition::TrailingPosition);
  ...
  setToolTip(
      ...
      "<li>Middle mouse button double click or (Shift + Left) or Space bar key "
      "to toggle item.</li>"
      "<li>Left mouse button click or Enter key to set selected drug dose.</li>"
      "</ul>");

  connect(le_setDose, SIGNAL(returnPressed()), this, SLOT(setDose()),
          Qt::QueuedConnection);
  connect(okDose, SIGNAL(triggered(bool)), le_setDose, SIGNAL(returnPressed()),
          Qt::QueuedConnection);

  connect(&selectedDrugLabel, SIGNAL(leftMouseButtonClicked()), &w_setDose,
          SLOT(close()));
  connect(&selectedDrugLabel, SIGNAL(rightMouseButtonClicked()), &w_setDose,
          SLOT(close()), Qt::QueuedConnection);
  connect(&selectedDrugLabel, SIGNAL(rightMouseButtonClicked()), this,
          SLOT(showContextMenu()), Qt::QueuedConnection);
  ...
  connect(le_setDose, SIGNAL(removeItemFromCompleter(QString, QString)), this,
          SLOT(removeItemFromCompleter(QString, QString)));
  connect(this, SIGNAL(setFindDrugsModel(QStandardItemModel *)), altDrugs,
          SLOT(setModel(QStandardItemModel *)));
  connect(this, SIGNAL(setFindDrugsLabel(QString)), altDrugs,
          SLOT(setLabel(QString)));
  connect(altDrugs, SIGNAL(replaceDrug(QString)), this,
          SLOT(replaceDrug(QString)));
...
bool drugsTable::addDrug2DrugList(QString newDrug) {
  if (newDrug.simplified().isEmpty()) return false;

  bool isDruginAutoComplete = isTradeNameinAutocomplete(newDrug);

  if (!isDruginAutoComplete && !(newDrug.right(1) == QString("*"))) {
    if (insertUnknownDrugName != newDrug) insertUnknownDrugCounter = 0;

    if (insertUnknownDrugCounter == 0) {
      emit popUpMessage("Warning",
                        "The item you are trying to add is unknown! press "
  ...
  for (int i = 0; i < druglength; i++) {
    QString drug =
        drugsModel->item(i, 0)->text();  //  ui->drugList->item(i)->text();

    if (drug.toUpper().append("*") == newDrug.toUpper()) {
      drugsModel->item(i, 5)->setText("0");
      drugsModel->item(i, 4)->setText(
          QString::number(QDate::currentDate().toJulianDay()));
      emit drugTableItemChanged();
      emit drugsHasBeenAltered(true);
      return true;
    } else if ((drug.toUpper() == newDrug.toUpper()) &&
               (drugsModel->item(i, 5)->text() == "0")) {
      drugsModel->item(i, 5)->setText("1");
      drugsModel->item(i, 3)->setText(
          QString::number(QDate::currentDate().toJulianDay()));
      emit drugTableItemChanged();
      emit drugsHasBeenAltered(true);
      return true;
    } else if (drug.toUpper() == newDrug.toUpper() &&
               (drugsModel->item(i, 5)->text() == "1")) {
      emit popUpMessage(
          "Warning",
          "The item you are trying to add to this Drug List already exists!");
      return true;
    }
  }
...
void drugsTable::DrugSyncWithVisit(QList<QStringList> array, bool sync) {
  ...
  for (int v = 0; v < vDrugs_rows; v++) {
    ...
    for (int m = mDrugs_rows - 1; m >= 0; --m) {
      tradeName_m =
          drugsModel->item(m, 0)->text().simplified().split(" ").at(0);
...
void drugsTable::clearDrugs() {
  int reply = showMessageBox(
      "Are you sure that you want to remove all drugs from list?");
  if (reply == QMessageBox::Yes) clear();
}
...
void drugsTable::clearInactiveDrugs() {
  int reply = showMessageBox(
      "Are you sure that you want to remove stopped drugs from list?");
...
void drugsTable::replaceDrug(QString newDrug) {
  ...
  for (int i = 0; i < druglength; i++) {
    if (drug.toUpper() == newDrug.toUpper() &&
        (drugsModel->item(i, 5)->text() == "1")) {
      emit popUpMessage(
          "Warning",
          "The item you are trying to add to this Drug List already exists!");
...
DrugsItemModel *drugsTable::getDrugsModel() { return drugsModel; }

bool drugsTable::isWorking() { return working; }

void drugsTable::loadPatientDrugsModel(int ID, qint64 julianDate,
                                       qint64 visitTime, bool syncLoader) {
  working = true;
...
void drugsTable::createDrugsTableConMenu() {
  ...
  a_copy2pWin =
      new QAction("&Copy selected item into Patient's Drugs", m_drugs);
  a_copy2pWin->setIcon(QIcon(":/Graphics/copy"));

  a_updateAll2PWin =
      new QAction("&Update Patient's Drugs from this list.", m_drugs);
  ...
  a_cp4rmLastVisit =
      new QAction("&Fill this list with last visit's drugs.", m_drugs);
  a_cp4rmLastVisit->setIcon(QIcon(":/Graphics/copy"));

  a_overWrite2pWin =
      new QAction("&Overwrite all in Patient's Drugs with this list.", m_drugs);
  ...
  a_ShowAltDrugsSameGeneric =
      new QAction("&With same generic name", altDrugsMenu);
  a_ShowAltDrugsSameGeneric->setIcon(QIcon(":/Graphics/beaker"));

  a_ShowAltDrugsSameIndication =
      new QAction("&With same indication", altDrugsMenu);
  a_ShowAltDrugsSameIndication->setIcon(QIcon(":/Graphics/beaker"));

  // a_ShowAltDrugsSameGroup = new QAction("&With same Drug
  // Group",altDrugsMenu);
  ...
  actionList1 << a_resume << a_stop << a_setDose << a_clearDefaultDose
              << a_EditExpander << a_ResetExpander << a_checkPrice
              << a_genericInfo;
  actionList2 << a_aAutoComp << a_rAutoComp << a_delete << a_stopAll
              << a_resumeAll << a_clear << a_clearInactive;
  actionList3 << a_ShowAltDrugsSameTrade << a_ShowAltDrugsSameGeneric
              << a_ShowAltDrugsSameIndication;  //<< a_ShowAltDrugsSameGroup;
  actionList4 << a_copy2pWin << a_updateAll2PWin << a_cp4rmPWin
              << a_cp4rmLastVisit << a_overWrite2pWin;
  ...
  connect(a_clearDefaultDose, SIGNAL(triggered()), this,
          SLOT(clearDefaultDose()));
  connect(a_checkPrice, SIGNAL(triggered()), this, SLOT(checkPrice()));
  connect(a_genericInfo, SIGNAL(triggered()), this,
          SLOT(searchGenricNameOnline()));
  connect(a_aAutoComp, SIGNAL(triggered()), this, SLOT(addToAutoComplete()));
  connect(a_rAutoComp, SIGNAL(triggered()), this,
          SLOT(removeFromAutoComplete()));
  connect(a_delete, SIGNAL(triggered()), this,
          SLOT(removeCurrentSelectedDrugFromList()));
  connect(a_clear, SIGNAL(triggered()), this, SLOT(clearDrugs()));
  connect(a_clearInactive, SIGNAL(triggered()), this,
          SLOT(clearInactiveDrugs()));
  ...
  connect(a_EditExpander, &QAction::triggered, this,
          &drugsTable::showExpanderEasyEdit);
  connect(a_ResetExpander, &QAction::triggered, this,
          &drugsTable::resetExpander);

  connect(a_ShowAltDrugsSameTrade, SIGNAL(triggered()), this,
          SLOT(findSameTradename()));
  connect(a_ShowAltDrugsSameGeneric, SIGNAL(triggered()), this,
          SLOT(findSameGeneric()));
  connect(a_ShowAltDrugsSameIndication, SIGNAL(triggered()), this,
          SLOT(findSameIndication()));
  // connect
  // (a_ShowAltDrugsSameGroup,SIGNAL(triggered()),this,SLOT(findSameGroup()));
}
...
void drugsTable::genDrugTableToolTip() {
  ...
  for (int t = 0; t < drugsModel->rowCount(); t++) {
    ...
    isExpand = (form == "EXPAND" ||
                (form.isEmpty() && sqlextra->isExpandExists(tradeName)));

    if (isExpand) {
      if (form != "EXPAND") genericName = sqlextra->getExpand(tradeName);

      genericLabel = "Exapnded to";
    }

    QLocale locale(QLocale::English, QLocale::UnitedStates);
    date = (current) ? locale.toString(QDate::fromJulianDay(startDate.toInt()),
                                       "dd/MM/yyyy")
                     : locale.toString(QDate::fromJulianDay(stopDate.toInt()),
                                       "dd/MM/yyyy");
    ...
    if (price.isEmpty()) price = "UNKNOWN";

    if (genericName.isEmpty() && !isExpand) genericName = "UNKNOWN";

    QString tooltip_template;
    if (!isExpand) {
      tooltip_template = QString(
          "<table>"
          "<tr>"
          "<td><b> %6 </td><td>:</td> </b><td> %1 </td>"
          "</tr>"
          "<tr>"
          "<td><b> Dose         </td><td>:</td> </b><td dir=RTL "
          "align=\"left\"> %2 </td>"
          "</tr>"
          "<tr>"
          ...
          "</tr>"
          "<tr>"
          "<td><b> Category     </td><td>:</td> </b><td><p align=\"left\"> "
          "%5.</p></td>"
          "</tr>"
          "</table>");
      tTip = QString(tooltip_template)
                 .arg(genericName, dose, date, price, category, genericLabel,
                      dateLabel, currentTag);
      ...
      tooltip_template = QString(
          ...
          "</table>");
      tTip = QString(tooltip_template)
                 .arg(genericName, dose, date, genericLabel, dateLabel,
                      currentTag);
    }
...
int drugsTable::showMessageBox(QString message) {
  myMessageBox msgbox;
  return msgbox.question(this, "Alert!", message, QMessageBox::Yes,
                         QMessageBox::No);
}
...
bool drugsTable::eventFilter(QObject *o, QEvent *e) {
  ...
  if (!cell.isValid() || isReadOnly) return zTableView::eventFilter(o, e);
...
void drugsTable::keyPressEvent(QKeyEvent *keyEvent) {
  if (!isReadOnly && keyEvent->key() == Qt::Key_Return)
    ...
    QModelIndex cell = selectionModel()->currentIndex();
    if (cell.isValid() &&
        drugsModel->item(cell.row(), 5)->text().toInt() == 1) {
      stop_drug();
    } else if (cell.isValid() &&
               drugsModel->item(cell.row(), 5)->text().toInt() == 0) {
      resume_drug();
    }
  }
...
void drugsTable::showContextMenu(const QPoint &pos) {
  ...
  if (!cell.isValid()) {
    ...
    canFetchDrugInfo =
        !(isExpand || genericName.isEmpty() || genericName == "Unknown");
    null_selected = false;
  }
  ...
  a_clearInactive->setVisible(isThereInactiveDrugs() && !isReadOnly &&
                              !no_drugs);
  a_aAutoComp->setVisible(!isTradeNameinAutocomplete(tradeName) &&
                          !isReadOnly && !null_selected);
  a_rAutoComp->setVisible(isTradeNameinAutocomplete(tradeName) && !isReadOnly &&
                          !null_selected);
  a_setDose->setVisible(!isReadOnly && !null_selected);
  a_clearDefaultDose->setVisible(!isReadOnly &&
                                 sqlextra->isDefaultDoseExists(tradeName) &&
                                 !null_selected);
  a_clear->setVisible(!isReadOnly && !no_drugs);
  a_delete->setVisible(!isReadOnly && !null_selected);

  a_EditExpander->setVisible(!isReadOnly && !null_selected && isExpand);
  a_ResetExpander->setVisible(!isReadOnly && !null_selected && isExpand);
  ...
  a_updateAll2PWin->setVisible((objectName() == "vDrugsTable") && !no_drugs &&
                               doesHavePatientDrugs);
  a_cp4rmPWin->setVisible((objectName() == "vDrugsTable") && no_drugs &&
                          doesHavePatientDrugs && !isReadOnly);
  a_cp4rmLastVisit->setVisible((objectName() == "vDrugsTable") && no_drugs &&
                               doesLastVisitHasDrugs && !isReadOnly);
  ...
  if (!null_selected && coreDrugs.contains(tradeName)) {
    a_checkPrice->setVisible(!isReadOnly &&
                             !isPriceEqual(cell.row(), tradeName));
...
void drugsTable::removeFromAutoComplete() {
  ...
  int reply = showMessageBox(
      QString(
          "This will remove <%1> from your autocomplete list, Are you sure?")
          .arg(tradeName));
  ...
  if (sqlextra->removeFromAutoComplete("drugs", tradeName)) {
    popUpMessage("Message",
                 QString("<%1> item was removed from drugs auto complete list.")
                     .arg(tradeName));
    sqlextra->deleteDefaultDrugDose(tradeName);
...
void drugsTable::addToAutoComplete() {
  ...
  if (sqlextra->addToAutoComplete("drugs", tradeName, true)) {
    popUpMessage("Message",
                 QString("<%1> item was added to drugs auto complete list.")
                     .arg(tradeName));
    emit reloadDrugsLineAutoComplete();
  }
}
...
void drugsTable::setDose() {
  ...
  if (newDose != drugsModel->item(cell.row(), 2)->text()) {
    ...
    drugsModel->item(cell.row(), 3)
        ->setText(QString::number(QDate::currentDate().toJulianDay()));
    genDrugTableToolTip();
...
void drugsTable::show_setDose() {
  ...
  QPoint pos = this->mapToGlobal(
      QPoint(this->columnViewportPosition(this->currentIndex().column()) + 2,
...
void drugsTable::drugsTableSort() {
  ...
  for (int row = 0; row < drugsModel->rowCount(); row++) {
    if (drugsModel->item(row, 5)->text().toInt() == 0) {
      drugsModel->setData(drugsModel->index(row, 0),
                          QVariant(QBrush(Qt::lightGray)), Qt::ForegroundRole);
      drugsModel->item(row, 0)->setFont(iFont);

    } else {
      drugsModel->setData(drugsModel->index(row, 0),
                          QVariant(QBrush(Qt::black)), Qt::ForegroundRole);
...
void drugsTable::findSameTradename() {
  ...
  QString tradeName =
      QString(
          rawTradeName.split(QRegularExpression("(\\d+(\\.\\d+)?)")).first())
          .simplified();
  ...
  setFindDrugsLabel(
      QString("Viewing drugs Similar to trade's name : <b>[%1]</b>")
          .arg(tradeName));
...
void drugsTable::findSameGeneric() {
  ...
  setFindDrugsLabel(
      QString(
          "Viewing drugs Similar to <b>[%1]</b> generic's name : <b>[%2]</b>")
          .arg(tradeName)
          .arg(genericNames.join(", ")));
  altDrugs->setSelectedDrugName(tradeName);
}
...
void drugsTable::findSameIndication() {
  ...
  setFindDrugsLabel(
      QString("Viewing drugs Similar to <b>[%1]</b> indication : <b>[%2]</b>")
          .arg(tradeName)
          .arg(indications.join(", ")));
  altDrugs->setSelectedDrugName(tradeName);
}
...
//     setFindDrugsLabel(QString("Viewing drugs Similar to <b>[%1]</b> drug's
//     group : <b>(%2)</b>").arg(tradeName).arg(indications.join(", ")));
//     altDrugs->setSelectedDrugName(tradeName);
// }
...
void drugsTable::toggleAllDrugs() {
  ...
  int reply = showMessageBox(
      QString("Are you sure that you want to %1 all drugs from list?")
          .arg((state == 0) ? "stop" : "resume"));

  if (reply == QMessageBox::No) return;

  for (int row = 0; row < rows; row++) {
    drugsModel->item(row, 4)->setText(
        QString::number(QDate::currentDate().toJulianDay()));
...
void drugsTable::calculateRxCosts() {
  ...
  for (int row = 0; row < rows; row++) {
    ...
    if (state == 1) {
      cost += itemPrice;
      if (itemPrice == (double)0) percision = false;
    }
  }
...
void drugsTable::showExpanderEasyEdit() {
  ...
  if (save) {
    ...
    drugsModel->item(cell.row(), 3)
        ->setText(QString::number(QDate::currentDate().toJulianDay()));
    if (setDefault) sqlextra->saveExpand(expanderShortcut, expanderContents);
    emit drugTableItemChanged();
  }
}
