...
invesList::invesList(QWidget *parent, int _id)
    : mDialog(parent), model(new InvestModel(this)), ui(new Ui::invesList) {
  ui->setupUi(this);
  ...
  connect(getNameDlg, SIGNAL(investigationAccepted(QString, QString)), this,
          SLOT(addInvestigation(QString, QString)));
  connect(getNameDlg, SIGNAL(popUpMessage(QString, QString)), parent,
          SLOT(popUpMessage(QString, QString)));
  connect(this, SIGNAL(popUpMessge(QString, QString)), parent,
          SLOT(popUpMessage(QString, QString)));
...
void invesList::mHide() { setWindowOpacity(0.00); }

void invesList::mShow() { setWindowOpacity(1.00); }
...
void invesList::on_tableView_activated(const QModelIndex &_index) {
  ...
  } else if ((mediaFileurl.endsWith(".jpg", Qt::CaseInsensitive) ||
              mediaFileurl.endsWith(".jpeg", Qt::CaseInsensitive) ||
              mediaFileurl.endsWith(".png", Qt::CaseInsensitive)) &&
...
void invesList::uptodate() {
  ...
  for (int i = 0; i < model->rowCount(); i++) {
    ...
    if (!file.exists()) {
      model->removeRow(i);
      sqlbase->removeMediaUrlNormalizeRequest(ID, visitDate, mediaFileurl,
                                              name);
    }
  }
  setModal(model);
...
void invesList::loadInvestigations() {
  ...
  QStringList label = QStringList() << "ID"
                                    << "Investigation"
                                    << "visitDate"
                                    << "PATH"
                                    << "invDate"
...
void invesList::addInvestigation(QString invName, QString selectedImagePath) {
  ...
  QString invTime =
      QString::number(dateTime.time().msecsSinceStartOfDay() / 1000);
  int invState = 1;
  int invPrice = 0;
  QString invResults = "";
  ...
  QString localImagePath =
      QString("%1%2_%3.%4")
          .arg(localCopyPath,
               inv2Name.replace(QRegularExpression("[^A-Za-z\\d\\s]"), "_"), dt,
               fileExtension);
  ...
  if (!fileExtension.compare("pdf", Qt::CaseInsensitive)) {
    ...
    addInvBool = sqlbase->addInvestigation(
        ID, visitJulianDate, visitTime, invName, localImagePath, invDate,
        invTime, invState, invPrice, invResults);
    load();
  ...
  if (addInvBool) {
    emit popUpMessge("Message",
                     "New Investigation has been added Successfully.");
  } else {
