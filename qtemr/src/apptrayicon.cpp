...
appTrayIcon::appTrayIcon(QObject *parent) : QSystemTrayIcon(parent) {
  ...
  connect(a_favourites, SIGNAL(triggered()), parent,
          SLOT(show_favouritesDrugs_win()));
  ...
  connect(a_exportPatientList, SIGNAL(triggered()), parent,
          SLOT(exportPatientList()));
  ...
  connect(a_conEdit, SIGNAL(triggered()), parent,
          SLOT(show_commonConditionsEdit()));
  ...
  dbList << a_CompactDataBase << a_genRec << a_backup << a_exportPatientList
         << a_merge;
  easyList << a_listEdit << a_favourites << a_expander << a_dietsEdit
           << a_conEdit;
...
bool appTrayIcon::childWindowsActive() { return _mChildWindowActive; }
