...
void wm_add2Completer::setHusband(QString h)
{
    husband = h;
}

void wm_add2Completer::setMode(bool b)
{
    autocompletebyword=b;
}

void wm_add2Completer::save2Dict(QStringList dict)
{
    saveDicts(dict);
}
...
void wm_add2Completer::vWork()
{
    if ( autocompletebyword )
    {
        ...
        {
            if ( d.length() > 2 )
                sqlextra->addToAutoComplete("diagnoses",d);
        }
    }
    else
    {
    ...
    QStringList dicts = QStringList() << visit.presentationAnalysis <<
                                         visit.invResults <<
                                         visit.exGeneral <<
                                         visit.exChestHeart <<
                                         visit.exAbdback <<
...
{
    QStringList dicts = QStringList() << patient.pastHistory
