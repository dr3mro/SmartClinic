...
void visitsList::makeReadWrite(bool b) { isReadOnly = !b; }

void visitsList::stopLoading() { _stopLoadingNow = true; }

void visitsList::setMaxFollows(int mxf) { maxFollows = mxf; }
...
void visitsList::setVisitIcon(int type, QIcon icon) {
  ...
  if (count > 0 && index < count && index != -1) list[index].visitType = type;
...
void visitsList::addDetails() {
  ...
  foreach (sqlBase::visitItem item, list) {
    if (!_stopLoadingNow) {
      ...
      setItemData(i, visitTypes.getVisitTypesByAlgoIndex(item.visitType).color,
                  Qt::BackgroundRole);
      model()->setData(model()->index(i, 0), QSize(width(), 30),
                       Qt::SizeHintRole);
...
visitsList::~visitsList() { delete worker; }
...
bool visitsList::eventFilter(QObject *o, QEvent *e) {
  ...
  if ((mouseEvent->button() == Qt::LeftButton) && !onlyVisit && !isReadOnly &&
      (o == this)) {
    emit saveVisit();
    return false;
  }
...
void visitsList::wheelEvent(QWheelEvent *e) { e->ignore(); }
