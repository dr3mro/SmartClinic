...
void mSettings::saveSettings(const mSettings::pSettings &psets) {
  ...
  settings.setValue(_remmberLastFollowupDate,
  ...
  settings.setValue(_autoSetnewAfterMaxPerProblemIsReached,
...
mSettings::pSettings mSettings::readSettings() {
  ...
  psettings.autoCompleteByWord =
      settings.value(_autoCompleteByWord, QVariant(true)).toBool();
  psettings.showChronicConditions =
      settings.value(_showChronicConditions, QVariant(true)).toBool();
  psettings.showNavArrows =
      settings.value(_showNavArrows, QVariant(true)).toBool();
  psettings.smallScreenMode =
      settings.value(_smallScreenMode, QVariant(false)).toBool();
  psettings.simpleExamination =
      settings.value(_simpleExamination, QVariant(false)).toBool();
  psettings.alwaysSave = settings.value(_alwaysSave, QVariant(true)).toBool();
  psettings.inLinePatientList =
      settings.value(_inLinePatientList, QVariant(true)).toBool();
  psettings.autosaveinterval =
      settings.value(_autosaveinterval, QVariant(60)).toInt();
  psettings.autoSave = settings.value(_autoSave, QVariant(true)).toBool();
  psettings.minimizeToTray =
      settings.value(_minimizeToTray, QVariant(true)).toBool();
  psettings.autoClosePrintDlg =
      settings.value(_autoClosePrintDlg, QVariant(true)).toBool();
  psettings.enableVisualEffects =
      settings.value(_enableVisualEffects, QVariant(false)).toBool();
  psettings.updateNotify =
      settings.value(_updateNotify, QVariant(true)).toBool();
  psettings.useToast = settings.value(_useToast, QVariant(true)).toBool();
  psettings.usePhotoViewer =
      settings.value(_usePhotoViewer, QVariant(true)).toBool();
  psettings.remmberLastFollowupDate =
      settings.value(_remmberLastFollowupDate, QVariant(false)).toBool();
  // psettings.lastSelectedFollowUpDate = getLastSelectedFollowUpDate(); // not
  // needed and caused unneccessary setting save
  ...
  psettings.maxFollowUpsPerProblem =
      settings.value(_maxFollowUpsPerProblem, QVariant(4)).toInt();
  psettings.autoSetnewAfterMaxPerProblemIsReached =
      settings.value(_autoSetnewAfterMaxPerProblemIsReached, QVariant(false))
          .toBool();
  psettings.workingdays =
      settings.value(_workingDays, QVariant(222)).toInt();  // friday
  ...
  psettings.defaultFont =
      settings.value(_defaultFont, QVariant("Tahoma")).toString();
  psettings.defaultFontSize =
      settings.value(_defaultFontSize, QVariant(8)).toDouble();
  psettings.defaultFontBold =
      settings.value(_defaultFontBold, QVariant(false)).toBool();
  psettings.textboxFont =
      settings.value(_textboxFont, QVariant("Tahoma")).toString();
  psettings.textboxFontSize =
      settings.value(_textboxFontSize, QVariant(8)).toDouble();
  psettings.textboxFontBold =
      settings.value(_textboxFontBold, QVariant(false)).toBool();
  settings.endGroup();
...
mSettings::prescriptionPrintSettings mSettings::getPrintSettings(
    QString printProfile) {
  prescriptionPrintSettings mPageSettings;
  QSettings settings("./settings.ini", QSettings::IniFormat);
  settings.beginGroup(printProfile);
  mPageSettings.paperSizeId =
      settings.value(_paperSizeId, QVariant("A5")).toString();
  mPageSettings.printerIndex =
      settings.value(_printerIndex, QVariant(0)).toInt();
  mPageSettings.pageMargin =
      settings.value(_pageMargin, QVariant(5)).toDouble();
  mPageSettings.showBanner =
      settings.value(_showBanner, QVariant(true)).toBool();
  mPageSettings.showDrugs = settings.value(_showDrugs, QVariant(true)).toBool();
  mPageSettings.showInvestigations =
      settings.value(_showInvsestigations, QVariant(true)).toBool();
  mPageSettings.showMeasurments =
      settings.value(_showMeasurments, QVariant(true)).toBool();
  mPageSettings.showDrugsTableOutline =
      settings.value(_showDrugsTableOutline, QVariant(false)).toBool();
  mPageSettings.drugsPrintMode =
      (mSettings::drugsPrintMode)settings.value(_drugsPrintMode, QVariant(0))
          .toInt();
  mPageSettings.showDrugsTitle =
      settings.value(_showDrugsTitle, QVariant(false)).toBool();

  mPageSettings.showPrescriptionHeader =
      settings.value(_showHeader, QVariant(false)).toBool();
  mPageSettings.showPrescriptionFooter =
      settings.value(_showFooter, QVariant(false)).toBool();
  mPageSettings.showPrescriptionLogo =
      settings.value(_showLogo, QVariant(false)).toBool();
  mPageSettings.prescriptionBannerStyle = static_cast<mSettings::bannerStyle>(
      settings.value(_bannerStyle, QVariant(0)).toInt());

  mPageSettings.logoSize = settings.value(_logoSize, QVariant(64)).toInt();

  mPageSettings.headerHeightPercent =
      settings.value(_headerHeightPercent, QVariant(10)).toInt();
  mPageSettings.footerHeightPercent =
      settings.value(_footerHeightPercent, QVariant(5)).toInt();
  mPageSettings.bannerHeightPercent =
      settings.value(_bannerHeightPercent, QVariant(5)).toInt();

  mPageSettings.bannerFont.fontName =
      settings.value(_bannerFont, QVariant("Tahoma")).toString();
  mPageSettings.bannerFont.fontSize =
      settings.value(_bannerFontSize, QVariant(8)).toInt();
  mPageSettings.bannerFont.fontBold =
      settings.value(_bannerFontBold, QVariant(false)).toBool();

  mPageSettings.altBannerFont.fontName =
      settings.value(_altBannerFont, QVariant("Tahoma")).toString();
  mPageSettings.altBannerFont.fontSize =
      settings.value(_altBannerFontSize, QVariant(8)).toInt();
  mPageSettings.altBannerFont.fontBold =
      settings.value(_altBannerFontBold, QVariant(false)).toBool();

  mPageSettings.roshettaFont.fontName =
      settings.value(_roshettaFont, QVariant("Tahoma")).toString();
  mPageSettings.roshettaFont.fontSize =
      settings.value(_roshettaFontSize, QVariant(8)).toInt();
  mPageSettings.roshettaFont.fontBold =
      settings.value(_roshettaFontBold, QVariant(false)).toBool();

  mPageSettings.doseFont.fontName =
      settings.value(_doseFont, QVariant("Tahoma")).toString();
  mPageSettings.doseFont.fontSize =
      settings.value(_doseFontSize, QVariant(8)).toInt();
  mPageSettings.doseFont.fontBold =
      settings.value(_doseFontBold, QVariant(false)).toBool();

  mPageSettings.requestsFont.fontName =
      settings.value(_requestsFont, QVariant("Tahoma")).toString();
  mPageSettings.requestsFont.fontSize =
      settings.value(_requestsFontSize, QVariant(8)).toInt();
  mPageSettings.requestsFont.fontBold =
      settings.value(_requestsFontBold, QVariant(false)).toBool();

  mPageSettings.measurementsFont.fontName =
      settings.value(_measurementsFont, QVariant("Tahoma")).toString();
  mPageSettings.measurementsFont.fontSize =
      settings.value(_measurementsFontSize, QVariant(8)).toInt();
  mPageSettings.measurementsFont.fontBold =
      settings.value(_measurementsFontBold, QVariant(false)).toBool();

  mPageSettings.signatureFont.fontName =
      settings.value(_signatureFont, QVariant("Tahoma")).toString();
  mPageSettings.signatureFont.fontSize =
      settings.value(_signatureFontSize, QVariant(8)).toInt();
  mPageSettings.signatureFont.fontBold =
      settings.value(_signatureFontBold, QVariant(false)).toBool();

  mPageSettings.showDrugsInitDate =
      settings.value(_showDrugsInitDate, QVariant(true)).toBool();
  mPageSettings.showSignaturePrintedOn =
      settings.value(_showSignaturePrintedOn, QVariant(true)).toBool();

  mPageSettings.showOnlyNewlyModifiedAddedDrugs =
      settings.value(_showOnlyNewlyModifiedAddedDrugs, QVariant(false))
          .toBool();
  // mPageSettings.showTradeNamesBold =
  // settings.value(_showTradeNamesBold,QVariant(false)).toBool();
  mPageSettings.showDoseNewLine =
      settings.value(_showDoseNewLine, QVariant(false)).toBool();

  mPageSettings.preferArabic =
      settings.value(_preferArabic, QVariant(true)).toBool();
  // mPageSettings.showStartDate =
  // settings.value(_showStartDate,QVariant(true)).toBool();
  // mPageSettings.showHorizontalLineBelowHeader =
  // settings.value(_showHorizontalLineBelowHeader,QVariant(false)).toBool();
  mPageSettings.enableFullPage =
      settings.value(_enableFullPage, QVariant(false)).toBool();
  mPageSettings.enableBodyHeaderSeparator =
      settings.value(_enableBodyHeaderSeparator, QVariant(false)).toBool();
  mPageSettings.compactMode =
      settings.value(_compactMode, QVariant(true)).toBool();
  mPageSettings.clearDuplicateDrugs =
      settings.value(_clearDuplicateDrugs, QVariant(false)).toBool();
  mPageSettings.preferRTFBanner =
      settings.value(_preferRTFBanner, QVariant(true)).toBool();
  settings.endGroup();
  return mPageSettings;
}

void mSettings::savePrintSettings(
    mSettings::prescriptionPrintSettings mPageSettings, QString printProfile) {
  QSettings settings("./settings.ini", QSettings::IniFormat);
  settings.beginGroup(printProfile);
  ...
  settings.setValue(_showDrugsTableOutline,
                    mPageSettings.showDrugsTableOutline);
  ...
  settings.setValue(_measurementsFontSize,
                    mPageSettings.measurementsFont.fontSize);
  settings.setValue(_measurementsFontBold,
                    mPageSettings.measurementsFont.fontBold);
  ...
  settings.setValue(_showSignaturePrintedOn,
                    mPageSettings.showSignaturePrintedOn);

  settings.setValue(_showOnlyNewlyModifiedAddedDrugs,
                    mPageSettings.showOnlyNewlyModifiedAddedDrugs);
  ...
  settings.setValue(_enableBodyHeaderSeparator,
                    mPageSettings.enableBodyHeaderSeparator);
...
bool mSettings::isAutoCompleteByWord() { return psettings.autoCompleteByWord; }

int mSettings::userSpeciality() { return psettings.speciality; }

bool mSettings::showChronicBox() { return psettings.showChronicConditions; }

bool mSettings::showNavigation() { return psettings.showNavArrows; }

bool mSettings::isMinimizedToTray() { return psettings.minimizeToTray; }

bool mSettings::getScreenMode() { return psettings.smallScreenMode; }

bool mSettings::isVisualEffectsEnabled() {
  return psettings.enableVisualEffects;
}

bool mSettings::isAlwaysSave() { return psettings.alwaysSave; }

bool mSettings::isAutoSave() { return psettings.autoSave; }

bool mSettings::alwaysClosePrintDlgAfterClick() {
  return psettings.autoClosePrintDlg;
}

bool mSettings::isUpdateNotify() { return psettings.updateNotify; }

bool mSettings::isUseToast() { return psettings.useToast; }

bool mSettings::isUseNativePhotoViewer() { return psettings.usePhotoViewer; }

bool mSettings::isRemmberlastFollowupDate() {
  return psettings.remmberLastFollowupDate;
}

int mSettings::getWorkingDays() const { return psettings.workingdays; }

int mSettings::getAutoSaveInterval() { return psettings.autosaveinterval; }
...
bool mSettings::isInLinePatientList() { return psettings.inLinePatientList; }

bool mSettings::isSimpleExamination() { return psettings.simpleExamination; }

int mSettings::getMaxFollowUps() { return psettings.maxFollowUps; }
...
mSettings::dbOptions mSettings::readDbOptions() {
  if (dbOptionsInit) return dboptions;
...
mStyler::Style &mSettings::getStyle() { return styler.getLoadedStyle(); }
...
mSettings::lineStyle mSettings::getLineStylesheet() {
  ...
  linestyle.errorStylesheet =
      QString("QLineEdit{background-color: %1; color:%2;}")
          .arg(errorBackgroundColor)
          .arg(errorTextColor);
  linestyle.warningStylesheet =
      QString("QLineEdit{background-color: %1; color:%2;}")
          .arg(warningBackgroundColor)
          .arg(warningTextColor);
  linestyle.normalStylesheet =
      QString("QLineEdit{background-color: %1; color:%2;}")
          .arg(normalBackgroundColor)
          .arg(normalTextColor);
...
int mSettings::getLastSelectedFollowUpDate() {
  ...
  int lastSelectedFollowUpDate =
      settings
          .value(_lastSelectedFollowUpDate,
                 QVariant(QDate::currentDate().toJulianDay()))
          .toInt();
  settings.endGroup();
