...
genericOnlineSearch::genericOnlineSearch(QWidget *parent)
    : mDialog(parent), ui(new Ui::genericOnlineSearch) {
  ui->setupUi(this);
  connect(ui->label, SIGNAL(leftClicked()), this,
          SLOT(tradeNameDoubleClicked()));
...
void genericOnlineSearch::setModel(QStringList drugs) {
  ...
  move(win->frameGeometry().topLeft() + win->rect().center() -
       this->rect().center());
  ...
  switch (engine) {
    ...
    case 1:
      searchString =
          ...
          "srchcont_rxlist.asp?src=%1&x=0&y=0";
      break;
    case 2:
      searchString =
          "http://google2.fda.gov/"
          "search?q=%1&client=FDAgov&proxystylesheet=FDAgov&output=xml_no_dtd&"
          "site=FDAgov&requiredfields=-archive:Yes&sort=date:D:L:d1&filter=1";
      break;
    case 3:
