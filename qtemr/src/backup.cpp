...
backup::backup(QWidget *parent)
  ...
  connect(ui->closeButton, &QToolButton::clicked, this,
          &backup::onCloseButton_clicked);
  connect(ui->buttonNew, &QToolButton::clicked, this,
          &backup::onButtonNew_clicked);
  connect(ui->buttonDelete, &QToolButton::clicked, this,
          &backup::onButtonDelete_clicked);
  connect(ui->listOfAvailableBackUps, &QTableView::clicked, this,
          &backup::onListOfAvailableBackUps_clicked);
  connect(ui->buttonRestore, &QToolButton::clicked, this,
          &backup::onButtonRestore_clicked);

  ui->compatibility->setText(QString("Only Backups compatible with database "
                                     "version [%1] will be displayed below.")
                                 .arg(databaseVersion));
...
void backup::onCloseButton_clicked() { this->close(); }
...
QString backup::getZipMD5(const QString &zipFile) {
  ...
  QString md5 = QString(
      QCryptographicHash::hash(*zipfileContents, QCryptographicHash::Md5)
          .toHex());
...
bool backup::createBackup() {
  ...
  int backupcount =
      static_cast<int>(QDateTime::currentDateTime().toMSecsSinceEpoch() / 1000);
...
void backup::getFileContents(const QString &fileName,
...
void backup::onButtonRestore_clicked() {
  ...
  if (zipMD5 == savedMD5) {
    ...
    QMessageBox::information(
        nullptr, "Warning",
        "Press OK to close Application now to avoid data loss.",
        QMessageBox::Ok);
