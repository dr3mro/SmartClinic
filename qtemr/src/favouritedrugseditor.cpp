...
favouriteDrugsEditor::favouriteDrugsEditor(QWidget *parent)
    : mDialog(parent), ui(new Ui::favouriteDrugsEditor) {
  ui->setupUi(this);
  setModal(true);
  ...
  connect(ui->tradeName, SIGNAL(textChanged(QString)), this,
          SLOT(addButtonToggler(QString)));
  connect(ui->dose, SIGNAL(textChanged(QString)), this,
          SLOT(addButtonToggler(QString)));
  connect(ui->tradeName, SIGNAL(textChanged(QString)), this,
          SLOT(setFilter(QString)));
  connect(ui->closeButton, SIGNAL(clicked(bool)), this, SLOT(close()));
  connect(ui->dose, SIGNAL(returnPressed()), this,
          SLOT(on_ButtonNew_clicked()));
...
void favouriteDrugsEditor::on_ButtonDel_clicked() {
  ...
  int row = index.row();
  QString selectedDrug = ui->tableView->model()
                             ->data(index.sibling(row, 0), Qt::DisplayRole)
                             .toString();
...
void favouriteDrugsEditor::on_ButtonNew_clicked() {
  ...
  if (list.contains(tradeName, Qt::CaseInsensitive)) {
    msgbox.information(nullptr, "Error",
                       "Error adding this item, alreay exisits!");
    return;
  } else {
    if (!sqlextra->addDefaultDrugDose(tradeName, dose)) {
      msgbox.information(nullptr, "Error",
                         "Error adding this item, unknown Error!");
      return;
    } else {
      model->select();
...
void favouriteDrugsEditor::dataChanged(QModelIndex topLeft,
  ...
  if (length < 2) {
    msgbox.information(nullptr, "Error",
                       "Error modifying this item, insufficient length!");
    model->revertAll();
  } else {
    model->submitAll();
  }
