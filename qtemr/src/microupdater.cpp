...
microUpdater::microUpdater(QObject* parent)
    : QObject(parent),
  ...
  connect(updateInfoGraber, SIGNAL(returnDownloadedData(QByteArray)), this,
          SLOT(notifyUser(QByteArray)));
}
...
bool microUpdater::isUpdateAvailable(QByteArray ba) {
  ...
  if (latestVersion != ignoredVersion &&
      QVersionNumber::fromString(latestVersion) >
          QVersionNumber::fromString(APPVERSION))
    return true;
  else
    return false;
}
...
void microUpdater::notifyUser(QByteArray ba) {
  if (parent()->objectName() == "miniUpdater") emit viewDownloadedData(ba);

  if (isUpdateAvailable(ba) && parent()->objectName() == "MainWindow") {
    QMessageBox msgBox;

    msgBox.setText(QString("New Update is available to version: %1, Do you "
                           "want to update now?")
                       .arg(latestVersion));

    QAbstractButton* pButtonYes =
        (QAbstractButton*)msgBox.addButton(tr("Yes"), QMessageBox::YesRole);
    QAbstractButton* pButtonNo =
        (QAbstractButton*)msgBox.addButton(tr("No"), QMessageBox::NoRole);
    QAbstractButton* pButtonIgnore =
        (QAbstractButton*)msgBox.addButton(tr("Skip"), QMessageBox::RejectRole);
