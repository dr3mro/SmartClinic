...
void mLineEdit::makeEditable(bool b) { setReadOnly(!b); }

void mLineEdit::addUniqueItem2Model(QString item, QString objName) {
  if (objectName() != objName) return;
...
void mLineEdit::toggleonTextChangedSignal(bool c) {
  if (windowName != "visitsBox") return;
  ...
  if (c) {
    ...
    disconnect(this, SIGNAL(cursorPositionChanged(int, int)), this,
               SLOT(onTextChanged()));
...
void mLineEdit::keyPressEvent(QKeyEvent *e) {
  if (e->modifiers() == Qt::SHIFT && e->key() == Qt::Key_Delete &&
    ...
    QAbstractProxyModel *t_proxyModel =
        (QAbstractProxyModel *)mcompleter->popup()->model();
    QString currentText =
        mcompleter->popup()->currentIndex().data(Qt::DisplayRole).toString();
    ...
    QItemSelectionModel *sm =
        (QItemSelectionModel *)mcompleter->popup()->selectionModel();
    sm->select(mcompleter->completionModel()->index(0, 0),
