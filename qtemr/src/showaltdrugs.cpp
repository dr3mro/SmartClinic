...
showAltDrugs::showAltDrugs(QWidget *parent)
    : QWidget(parent), ui(new Ui::showAltDrugs) {
  ui->setupUi(this);
  resize(680, 480);
...
void showAltDrugs::setLabel(QString label) { ui->filters->setText(label); }
...
void showAltDrugs::on_tableView_activated(const QModelIndex &index) {
  ...
  int reply = msgbox.question(
      this, "Confirmation",
      QString("Are you sure you want to replace <b> %1 </b> with <b> %2 </b>")
          .arg(drugsTableCurrentSelectedDrugName)
          .arg(tradeName),
