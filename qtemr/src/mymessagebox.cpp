...
int myMessageBox::question(QWidget *parent, const QString &title,
                           const QString &text, const StandardButton button0,
                           const StandardButton button1, bool DefaultNo) {
  messagebox = new mMessageBox(this);
  messagebox->setParent(parent);
  messagebox->setWindowFlags(this->windowFlags() | Qt::WindowStaysOnTopHint |
...
int myMessageBox::question(QWidget *parent, const QString &title,
                           ...
                           StandardButton button1, StandardButton button2) {
  messagebox = new mMessageBox(this);
  messagebox->setParent(parent);
  messagebox->setWindowFlags(this->windowFlags() | Qt::WindowStaysOnTopHint |
                             Qt::FramelessWindowHint);
...
int myMessageBox::information(QWidget *parent, const QString &title,
                              const QString &text) {
  messagebox = new mMessageBox(this);
  messagebox->setParent(parent);
  messagebox->setWindowFlags(this->windowFlags() | Qt::WindowStaysOnTopHint |
