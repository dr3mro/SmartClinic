...
void ZipReaderPrivate::scanFiles()
{
    ...
    if (comment_length != i)
        mDebug() << "QZip: failed to parse zip file.";
    ...
    for (i = 0; i < num_dir_entries; ++i) {
        ...
        if (read < (int)sizeof(CentralFileHeader)) {
            mDebug() << "QZip: Failed to read complete header, index may be incomplete";
            break;
        }
        ...
        if (header.file_name.length() != l) {
            mDebug() << "QZip: Failed to read filename from zip index, index may be incomplete";
            break;
        }
        l = readUShort(header.h.extra_field_length);
        header.extra_field = device->read(l);
        if (header.extra_field.length() != l) {
            mDebug() << "QZip: Failed to read extra field in zip file, skipping file, index may be incomplete";
            break;
        }
        l = readUShort(header.h.file_comment_length);
        header.file_comment = device->read(l);
        if (header.file_comment.length() != l) {
            mDebug() << "QZip: Failed to read read file comment, index may be incomplete";
            break;
        }
       ...
       ulong len = contents.length();
        ...
        do {
            ...
            case Z_OK:
                ...
                break;
            case Z_MEM_ERROR:
                mDebug() << "QZip: Z_MEM_ERROR: Not enough memory to compress file, skipping";
                data.resize(0);
                break;
            case Z_BUF_ERROR:
                len *= 2;
