        ...
        if ( xmlread.name() == "item" )
        {
            ...
            if ( ! attributes.empty() )
            {
                if ( attributes.value("dbVersion").toDouble() ==  version)
                {
                    ...
                    item4->setData(QVariant(attributes.value("row").toInt()), Qt::DisplayRole);
...
bool dataIOhelper::add2BackUpXML(int row,QString date, QString zipPath, QString MD5)
{
    double version = dbVersion;
...
QString dataIOhelper::setCurrentFolder(bool forceChange)
{
    ...
    if( currentAppPath.isEmpty() || forceChange)
    {
        ...
        if(!forceChange)
            QMessageBox::information(0,
                                 "Setup",
                                 QString("This is the first time you run %1, Please configure your data location, "
                                         "if you cancel it will load on current executable folder and you will "
                                         "be asked the next time %1 is launched.").arg(APPNAME),
                                 QMessageBox::Ok);

        newAppPath = QFileDialog::getExistingDirectory(0,"Open Directory",
                                                     currentAppPath.isEmpty() ? QCoreApplication::applicationDirPath().replace('/','\\'):currentAppPath,
    ...
    {
        QMessageBox::information(0,"Message",QString("%1 will now exit to change data path, please relaunch %1.").arg(APPNAME),QMessageBox::Ok);
        QFile lockFile(".lock");
        lockFile.remove();
        qApp->quit();
    }
