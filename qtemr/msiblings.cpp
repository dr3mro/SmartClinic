...
mSiblings::mSiblings(QWidget *)
{
    ...
    QStringList labels = QStringList() << "DateOfBirth" <<  "TOD" <<  "Gender" <<"PlaceOfBirth" << "Complications" << "Term";
...
void mSiblings::clear()
{
    model->removeRows(0,model->rowCount());
}

void mSiblings::removeRow(int row)
{
    model->removeRow(row);
}

bool mSiblings::save(int ID)
{
    return sqlbase->saveSiblings(ID,model);
}
...
QStandardItemModel *mSiblings::getModel()
{
    return model;
}
